...
dw_table_path <- app_config$dw_table_path
...
server <- function(input, output, session) {
  ...
  apply_role_selection_state <- function(state = NULL) {
    ...
    sync_missing_measurement_inputs(submitted_state)
    ...
    if (length(current_measurements) > 0) {
      message(sprintf(
        ...
        named_override_log_text(current_measurements)
      ))
    }
    validation_message <- role_assignment_validation(dependent_names(), independent_names())
  ...
  apply_variable_selection_state <- function(state = NULL) {
    ...
    sync_table_state(submitted_state)
    sync_missing_measurement_inputs(submitted_state)
    selection_state <- variable_selection_state(selected_names(), available_variable_names())
    ...
    finish_variable_selection(selected)
    invisible(TRUE)
  }
  ...
  prepare_analysis_result <- function() {
    ...
    prepare_regression_analysis_results(
      ...
      variable_info = info,
      reference_values = regression_reference_values_static(category_label_values()),
      boot_r = input$boot_r,
