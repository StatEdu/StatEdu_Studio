...
Development QA status: Core SEM/CFA automation, native CFA/SEM model-file selection and analysis entry, packaged ordinal WLSMV/theta execution, multigroup invariance/reliability/HTMT export, nondefault BCa/percentile intervals, background progress, independent cancellation, accurate cancellation messaging, release structure, and lifecycle cleanup pass. The pending rows above are the remaining focused packaged or external-program checks and must not be represented as completed.
