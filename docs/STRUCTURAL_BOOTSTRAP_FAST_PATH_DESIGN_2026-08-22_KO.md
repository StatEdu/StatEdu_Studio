...
### 2. 추적 가능한 fixture 동등성

- CFA/PLS clean fixture와 SHA-256으로 고정한 `statedu_dmc_75_v1` SEM fixture에서 같은 index를 strict와 fast에 넣는다.
- `statedu_dmc_75_v1`은 의도적으로 admissible/Heywood 표본을 함께 만든다. R 4.5.3,
  seed 20260822의 24회 mask는 14 valid/10 invalid로 고정하며, optimized production과
  public strict mask가 position별로 완전히 같아야 한다.
- 핵심 검사에 걸리지 않는 clean fixture도 strict/fast 유효 mask가 같아야 한다.
...
위 PoliticalDemocracy 수치는 융합 전의 역사적 shadow audit이며 현재 release fixture
결과가 아니다. 현재 core는 `statedu_dmc_75_v1`의 hash와 14/24 position mask를
별도로 고정하고 optimized production mask까지 직접 동일성 검사한다.
...
주의: 아래 `PoliticalDemocracy` 행은 2026-08-22 당시의 비추적 로컬 CSV로 얻은
역사적 성능 증거다. 현재 release gate와 benchmark는 이를 입력으로 재사용하지 않으며,
R 4.5.3·고정 RNG/seed·canonical SHA-256 계약의 `statedu_dmc_75_v1`을 사용한다.
따라서 아래 시간과 valid 수를 새 fixture의 결과로 오표기하거나 재사용해서는 안 되고,
설치본 승인 전 현재 benchmark를 다시 실행해 새 증거를 생성해야 한다.
