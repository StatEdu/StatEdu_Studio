...
Historical-artifact boundary (2026-08-26): this checklist does not cover the
PLS/PLSc moderation, moderated-mediation, multigroup, export/round-trip, and
bundled-runtime changes made after the recorded 2026-08-23 build. Those changes
require a new installer identity and a fresh copy of this checklist. The
private SmartPLS evidence required for a new release run is also absent on the
current validation workstation, so the newer source state is not eligible for
packaged or public sign-off yet.
