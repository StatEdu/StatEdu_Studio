...
HTML 저장 결과는 앱 화면의 결과표를 연구 보고서나 논문 작성에 옮기기 쉽게 정리한 것이다. PDF, Excel, Word 결과 저장은 공개판에서는 숨기고 1.3.0 공개 버전에 제공할 예정이지만, 비공개 개발자판에서는 검증을 위해 개방한다. 공개판 그림은 300 dpi이며 개발자판과 Pro는 600 dpi 출력을 지원한다. 저장된 표는 분석 판단을 자동으로 대체하지 않는다.
...
1.2.4-dev의 CFA, SEM/CB-SEM, PLS-SEM 메뉴는 독립 관측 횡단자료만 지원한다.
...
선택하되 저장된 프로젝트의 명시적 선택은 유지한다. 1.2.4부터
