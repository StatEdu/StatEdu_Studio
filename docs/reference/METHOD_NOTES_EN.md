...
Public 1.2 scope note: these notes describe workflows exposed in the public 1.2 interface. HTML result saving and 300 dpi figure-file saving are available. PDF, Excel, and Word result saving are deferred publicly until version 1.3.0, while non-public development builds expose those formats and 600 dpi output for validation. Mplus/latent add-ons, license activation, and paid-edition gating are not public 1.2 features. Sample-size calculator entries for GEE, LMM, GLMM, survival, cluster, or SEM/CFA are planning calculators only.
...
Saved HTML output preserves the main result tables, method notes, warnings, skipped-result messages, and footnotes. PDF, Excel, and Word saving are available in non-public development builds but remain planned for public version 1.3.0. Before using saved output in a report:
...
The CFA, SEM/CB-SEM, and PLS-SEM menus in 1.2.4-dev support independent
...
a new analysis, while an explicitly saved choice is retained. In 1.2.4,
