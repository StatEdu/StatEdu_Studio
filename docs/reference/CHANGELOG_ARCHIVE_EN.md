...
- Public PDF, Word and Excel saving; 300 dpi transparent figures in Free; improved tables, canvas exports and validation documentation. Meta-analysis and within-subject treatment repeated-measures ANOVA are excluded from this installer.


## Unreleased changes - 2026-09-11

- Restored localized PDF covers with StatEdu branding and clarified the public 1.3.0 document-export / later Pro roadmap.
- Improved portrait regression and mediation tables, grouped residual diagnostic plots, and limited ordered notes to displayed statistics and the final hierarchical model.
- Renamed the mediation/moderation canvas menu and removed the legacy regression submenu.
- Separated analysis commands from execution and moved canvas actions and export controls below the variable panel.
- Standardized structural main/appendix table fonts, removed nested zoom, and fixed overlapping HTML headers.
- Unified canvas PNG export around the displayed DOM, preserving model geometry and text wrapping; HTML/PDF reports append the displayed result model.
- Applied transparent PNG backgrounds and edition-based 300 dpi (Free) / 600 dpi (development, Pro), including PNG resolution metadata.

## Unreleased changes - 2026-09-05

- Added meta-analysis workflows and input templates, and one-group repeated-measures ANOVA.
- Expanded category-label handling and refined data and analysis screens.
- Improved PLS bootstrap worker scheduling and added serial/parallel reproducibility checks.
- Fixed LMM and GLMM predictor loss when long formulas wrap across multiple lines, including GLMM count-family screening.
- Added regression coverage for predictor retention, meta-analysis, repeated-measures ANOVA, and category-label output.

## Earlier development changes

- Kept PDF, Excel, and Word result saving hidden in public releases until the 1.3.0 roadmap while exposing all three formats in non-public development builds.
- Kept public figure output at 300 dpi and enabled 600 dpi output in development and Pro editions.
- Added per-latent indicator-distance controls (1.0x, 1.5x, 1.7x, and 2.0x) to the SEM canvas and preserved the selected scale in saved model files.
- Set figure-file output to 300 dpi by default and restricted the separate 600 dpi option to the Pro edition.
- Reduced first-load time across analysis menus by deferring structural-model handlers and caching shared result-table labels without changing analysis output.
- Accelerated penalized-regression selection bootstrapping with an exact adaptive serial/parallel path; the default remains 500 resamples and same-seed result objects remain identical.
- Fixed Latent Mplus BCH reporting so full-sample and moderator-stratified runs use distinct data/input/output paths, Table 6 retains BCH means and standard errors, and saved full-sample Wald tests cannot be replaced by a later subgroup output.
- Added explicit BCH overall degrees of freedom, detailed pairwise Wald-test output, modal-profile sparse-cell diagnostics, correct full-sample A3/A4 summaries, and distinct naive versus primary R3STEP table sources.
