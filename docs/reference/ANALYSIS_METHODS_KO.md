...
- 공개판은 HTML 결과 저장, 300 dpi 그림 파일 저장, Result 탭 추가를 지원한다. PDF, Excel, Word 결과 저장은 공개판에서 숨기고 1.3.0 공개 버전에 제공할 예정이다. 비공개 개발자판에서는 세 형식과 600 dpi 출력을 개방하며 Pro도 600 dpi를 지원한다.
...
public 1.2는 아래 분석 목록에 평가자간 일치도, 혼합 반복측정 ANOVA, 매개·조절효과을 포함한다. HTML 결과 저장, 300 dpi 그림 파일 저장, Result 탭 추가는 공개 범위이다. PDF/Excel/Word 결과 저장은 공개판에 노출하지 않고 1.3.0 공개 버전에 제공할 예정이지만 비공개 개발자판에서는 개방한다. license activation, paid-edition gating, Mplus/latent add-on은 public 1.2 화면에 노출하지 않는다.
...
- 결과는 화면 출력과 HTML 저장으로 연결된다. PDF/Excel/Word 결과 저장은 공개판에서 숨기고 비공개 개발자판에서 개방한다.
...
출력에는 모형 개요, 가정 검토표, 반복측정 검정표, 집단/시점 요약, 사후비교표, 경고 또는 skipped-model 안내, 현재 제공되는 HTML/그림/Result 추가 controls가 포함된다.
...
- 조건부 효과 그림과 결과 그림 저장.
- 현재 릴리스의 HTML/그림 저장 및 Result 탭 추가.

## 매개·조절효과

`매개·조절효과`은 캔버스에서 변수를 배치하고 경로를 그린 뒤, 인식된 구조를 매개·조절 분석 엔진으로 실행한다.
...
- warnings, skipped-model diagnostics, 현재 릴리스의 HTML/그림/Add result 출력이 포함된다.
...
- 이분형·다항형·순서형 종속변수에 대한 survey-weighted 로지스틱 회귀. 모형 선택의 기본값은 기존과 같은 이분형이다.
- 다항형은 `svyVGAM::svy_vglm`과 `VGAM::multinomial`로 추정하며, 변수 설정의 기준범주 또는 첫·마지막 범주를 선택한다. 기준범주별 비교표를 출력한다.
- 순서형은 `survey::svyolr`의 비례오즈 모형이다. 범주 설정 순서(없으면 요인 수준 또는 값 순서)를 사용하고 반전할 수 있다. OR은 높은 범주에 속할 누적 오즈를 나타내며 절단점은 별도 부록에 표시한다. 선택 가능한 평행기울기 진단은 누적 이분형 로짓을 적층하여 원래 조사설계의 ID·복제가중치와 절단점 간 공분산을 유지하고, 기울기 동일성을 설계기반 Wald F로 검정한다. 일반 Brant 검정이나 우도비 검정이 아니며, 추정 불가를 별도로 표시한다. 유의하지 않은 결과가 비례오즈 가정을 입증하지는 않는다.
- 계수와 오즈비, 신뢰구간, Wald 검정.
- 다항·순서형은 조사설계 자유도를 이용한 계수별 Wald F 검정과 t 신뢰구간, 전체 기울기의 결합 Wald F 검정을 제공한다. Taylor 및 복제가중치 설계를 사용한다.
- pseudo R-squared 등 기술적 적합 지표는 이분형에서 제공한다.
- 가중 N, 설계 df, 모형 적합 요약.
- 다항·순서형에는 관측된 종속변수 범주가 3개 이상 필요하다. 완전사례 처리 후 범주가 사라지거나 모형행렬이 계수 부족인 경우 분석을 제한한다.
...
- 다집단 잠재곱 모형에서는 원지표를 집단 안에서 각각 중심화하고 곱지표를
  집단별로 생성한다. Bootstrap은 관측된 집단별 표본수 `n_g`를 유지해
  집단 안에서 층화 재표집하며, 매 반복마다 재표집된 원지표를 집단별로
  다시 중심화하고 모든 곱지표를 새로 만든다.
- 원래 구성개념(상호작용요인 제외)은 configural/metric gate를 통과해야
  한다. 이후 공동 구조모형에서는 상호작용요인 적재량을 포함한 모든
  요인적재량을 집단 간 동일하게 제약하고 구조회귀는 집단별로 자유롭게
  둔다. Gate 실패를 우회하는 자동 부분불변성 재적합은 수행하지 않는다.
- 잠재 상호작용의 비교 대상은 비표준화 상호작용계수 B이다. 집단별 B,
  전체 집단 omnibus 검정과 모든 집단쌍의 B 차이를 출력한다. 표준화
  상호작용계수는 기술값이며 집단 간 동일성 검정의 추정대상이 아니다.
- 적격한 각 간접경로에서 집단별 moderated-mediation index는 상호작용 B와
  나머지 비표준화 간접경로 계수의 곱이다. 집단별 index, 전체 집단 index
  검정과 모든 집단쌍의 index 차이를 함께 출력한다.
- 층화 bootstrap CI와 경험적 양측 p값을 집단별 index 및 집단쌍 index
  차이의 주 추론으로 사용한다. Delta-method CI와 공동모형 covariance를
  이용한 Wald omnibus·집단쌍 검정은 보조 모형기반 결과다. 모형기반 BH는
  상호작용 omnibus, 상호작용 집단쌍, index omnibus, index 집단쌍에서
  각각 수행한다. Bootstrap p값도 집단별 상호작용, 상호작용 집단쌍 차이,
  집단별 index, index 집단쌍 차이별로 각각 보정하며 서로 다른 결과군을
  하나로 합치지 않는다.
- 공동모형 미수렴·부적합, 사용할 수 있는 모수 covariance 부재 또는 필수
  계수 누락 시 추론을 억제한다. 고정 0 또는 전부 고정된 구성요소는 점추정만
  유지하고 검정하지 않는다. Bootstrap 추론에는 유효 반복이 2회 이상이고
  요청수의 50% 이상이어야 한다. 50% 이상 80% 미만은 Caution, 80% 이상은
  Adequate, 50% 미만은 Unreliable로 표시하고 CI와 p값을 억제한다.
- Audit에서는 bootstrap 실행 기록과 추론 사용 가능 여부를 분리한다. 실행은
  완료되었지만 사용할 수 없는 경우 재현성을 위해 실행 사실은 보존하되 CI와
  p값을 억제하고 bootstrap 주 추론으로 표현하지 않는다.
- 이 절의 곱지표·공통요인 불변성·Wald 검정 규칙은 CB-SEM에 적용된다.
  PLS/PLSc 잠재조절은 아래 PLS-SEM 절의 구성점수 기반 절차를 사용하며,
  CB-SEM 잠재곱 결과와 같은 추정대상으로 해석하지 않는다.
...
- PLSpredict는 훈련 fold에서만 산출한 평균으로 결측 입력을 대체한 뒤,
  주 분석과 같은 PLS 또는 PLSc 측정·구조모형을 다시 적합하고 수렴·해의
  허용성을 확인한다. holdout의 원 결측 결과 셀은 RMSE/MAE 계산에서
  제외하며 유효 평가 표본 수를 기록한다. 직접 선행변수 방식의 PLS 예측을
  같은 fold의 선형모형 기준과 비교한다. 기본 반복분할은 10회이며 단일
  분할이나 평균오차 하나만으로 외부 예측력을 주장하지 않는다.
- R², f², VIF, loading, AVE와 HTMT의 관행적 절단값은 참고값이며 자동
  문항삭제·경로채택·타당도 합격 판정이 아니다.

### PLS MICOM과 다집단 구조효과

- MICOM은 PLS 합성점수의 불변성을 검정한다. MICOM 근거를 PLSc
  공통요인 불변성에 확장하지 않으며, PLSc 다집단 추론은 명시적으로
  차단한다.
- 1단계는 모든 집단에서 같은 지표, 모형, 자료처리와 PLS 알고리즘 설정을
  사용했는지 항목별로 점검한다. 2단계는 집단쌍별로 그 두 집단만 합쳐
  원래 집단 크기를 보존한 순열을 수행하며, 합성점수 상관의 부호를 유지한다.
  구성개념×집단쌍 검정군에는 Holm 보정으로 family-wise error rate를
  통제한다.
- 3단계 평균·분산 비교는 한 번 적합한 pooled PLS 합성점수를 고정한 뒤
  집단표지만 순열한다. 평균차와 log 분산비의 central equal-tail 순열구간과
  plus-one 양측 p값을 보고하며, 2단계가 통과한 경우에만 완전/부분
  불변성을 분류한다.
- 세 집단 이상에서는 모든 집단쌍을 각각 평가한다. 일부 집단쌍만 MICOM을
  통과하면 그 집단쌍의 구조효과 비교만 허용하고 나머지 쌍의 추론은
  억제한다. 현재 전체 집단 omnibus PLS-MGA 검정은 제공하지 않으므로
  결과는 pairwise 비교임을 명시한다.
- 집단별 PLS bootstrap은 직접효과, 특정 간접효과, 총간접효과와 총효과를
  같은 정규화 경로 레지스트리로 다시 계산한다. 집단쌍 차이는 집단 1에서
  집단 2를 뺀 값으로 보고하고 효과군별 raw, BH 및 Holm 보정 p값을 함께
  제공한다. 유효 공통 draw가 요청수의 80% 미만이면 그 집단쌍의 CI와
  p값을 억제한다.
- N 30은 MICOM의 보편적 최소 표본 기준으로 사용하지 않는다. 작은 집단은
  실행을 중단하지 않고 N, 결측률, 유효 재표집률과 검정력·안정성 경고를
  기록한다.

### PLS/PLSc 결측자료 처리

- PLS/PLSc는 선택 옵션 없이 `seminr::mean_replacement`와 동일한 지표별
  평균 대체를 고정적으로 사용한다. 본 분석에서는 전체 분석표본에서
  관측된 지표값의 산술평균으로 해당 지표의 결측 셀을 대체하며, 지표
  결측만을 이유로 사례를 삭제하지 않는다.
- 결과와 Audit JSON에는 원 분석 N, 완전사례 N, 평균 대체 사례 수,
  평균 대체 셀 수·비율, 지표별 결측률과 대체 평균을 기록한다.
- bootstrap은 원자료의 사례를 재표집한 뒤 각 재표집 안에서 지표 평균을
  다시 계산하여 결측 셀을 대체한다. 따라서 본 분석과 bootstrap은 같은
  평균대체 원칙을 사용하지만 대체 평균값 자체는 재표집마다 달라질 수 있다.
- 평균 대체는 분산과 연관성을 왜곡할 수 있으므로 결측 원인과 규모를
  보고한다. 결측이 실질적이면 완전사례 분석 또는 연구설계에 맞게
  정당화된 대안 처리와의 민감도 분석을 별도로 검토한다.

### PLS/PLSc bootstrap

- PLS/PLSc bootstrap 기본값은 5,000회이며 1,000/5,000/10,000/20,000/50,000회 중
  하나를 명시적으로 선택한다.
...
- 한 반복의 필수 경로·loading·weight·HTMT·효과가 모두 유한할 때만
  whole draw를 유효로 센다.
- 직접효과는 설정된 각 구조경로의 계수이다. 특정 간접효과는 예측변수에서
  결과변수까지 이어지는 각 단순 비순환 매개경로의 경로계수 곱이며, 경로마다
  하나의 추정량으로 보고한다. 총간접효과는 같은 예측변수-결과변수 쌍의 모든
  특정 간접효과를 합한 값이고, 총효과는 직접효과와 총간접효과의 합이다.
  각 유효 재표집에서 이 네 효과를 원자료와 같은 방식으로 다시 계산한다.
- 효과표의 95% percentile 신뢰구간은 R `quantile` type 7을 사용한다. 경험적
  양측 p값은 유효 재표집 효과의 부호 개수에 plus-one 보정을 적용한
  `2 * (min(n[θ* <= 0], n[θ* >= 0]) + 1) / (R_valid + 1)`이며 1을 넘지
  않게 제한한다.
- BH 보정은 직접효과, 특정 간접효과, 총간접효과, 총효과의 네 family에서
  각각 별도로 수행하며 같은 추정량을 한 family 안에 중복시키지 않는다.
- 유효 draw가 요청수의 80% 미만이면 bootstrap SE, CI, t, p와 유의성
  표현을 억제하고 실패유형을 결과와 Audit에 기록한다.
- bootstrap을 실행하지 않으면 네 효과의 점추정 β는 계속 표시하되 Boot SE,
  CI, t, p, BH p는 공란으로 두고 `Not requested`/점추정 전용 상태를 명시한다.

### PLS 잠재조절과 조절된 매개효과

- 잠재조절은 PLS 구성점수에 대한 two-stage 방법을 기본으로 하며,
  product-indicator와 orthogonalized 방법도 명시적으로 선택할 수 있다.
  상호작용만 넣고 필요한 주효과를 누락하지 않도록 강한 위계 원칙을 적용한다.
- 각 bootstrap 반복에서 측정모형, 구성점수, 상호작용항과 구조모형을 모두
  다시 추정한다. 고정된 원자료 구성점수나 미리 계산한 상호작용열만
  재표집하지 않는다.
- 비표준화 상호작용계수, 구성점수 평균과 평균 ± 1 SD에서의 단순기울기,
  조건부 간접효과와 조절된 매개효과 지수를 출력한다. 조절된 매개효과는
  상호작용 경로계수와 이후 간접경로 계수의 곱으로 정의한다.
- 상호작용, 조절된 매개효과 지수와 조건부 간접효과의 BH 보정은 서로 다른
  검정군에서 각각 수행한다. 유효 whole draw가 요청수의 80% 미만이면 CI와
  p값을 억제하고 점추정과 실패 진단만 유지한다.
- PLSc를 선택하더라도 생성된 상호작용항에는 일관성 보정을 적용하지 않는다.
  따라서 해당 상호작용 및 조절된 매개효과는 PLS 구성점수 척도의 결과이며
  PLSc 공통요인 상호작용으로 해석하지 않는다.
- 다집단 비교는 집단쌍별 MICOM 부분 측정불변성 통과 후에만 허용한다.
  집단별 상호작용·조절된 매개효과와 집단쌍 차이를 보고하고, 각 차이
  검정군 안에서 raw, BH 및 Holm p값을 제공한다. 세 집단 이상의 전체
  omnibus 조절효과 검정은 현재 제공하지 않는다.

## 구조모형 공통 출력과 제한

- 사용자 label을 화면과 결과표에 우선 적용하되 syntax와 Audit에는 재현을
  위해 원 변수명과 내부 product 명세를 보존한다.
- Audit schema 1.8은 앱·R·패키지 버전, 모형·코드 fingerprint, estimator,
  sampling design, seed, RNG, CI 방법, quantile type, 유효 draw와 실패를
  기록한다. 다집단 잠재조절에서는 집단별 중심화·곱지표 생성 audit 행,
  metric 및 상호작용요인 적재량 제약 정책, 집단별 상호작용·index와 대조,
  층화 bootstrap 진단도 함께 저장한다. Schema 1.8은 schema 1.7에서 도입한
  재현성 필드를 버리지 않고 확장한다.
...
- 공개판에서는 HTML 저장을 지원한다. PDF, Excel, Word 결과 저장은 공개판에서 숨기되 비공개 개발자판에서 개방하며, 공개 제공은 1.3.0에 제공할 예정이다.
