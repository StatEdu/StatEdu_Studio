...
Public 1.2 exposes the analysis and calculator workflows listed below, including Inter-rater Agreement, Longitudinal / Panel Models, Mixed Repeated-Measures ANOVA, and Mediation / Moderation Effects. HTML result saving, 300 dpi figure-file saving, and the in-app Result collection are public. PDF, Excel, and Word result saving remain hidden publicly until version 1.3.0; non-public development builds expose those formats and 600 dpi output. License activation, paid-edition gating, and Mplus/latent add-ons are not exposed in the public 1.2 interface. Planning calculators may include GEE, LMM, GLMM, survival, cluster, and SEM/CFA entries; those are calculator workflows, not public 1.2 Analysis menus.
...
- HTML result export and figure-file saving where applicable.
...
Results are shown on screen and can be saved through the public HTML result workflow.
...
Outputs include the model overview, assumption-review table, repeated-measures test table, group/time summaries, post-hoc tables, warnings or skipped-model notes, and the current HTML/figure/Add-result controls where applicable.
...
- HTML, figure, and Result-tab output in the current release.

## Mediation / Moderation Effects

The **Mediation / Moderation Effects** menu provides a canvas for drawing a mediation/moderation model and sending the recognized structure to the same analysis engine.
...
- In a multi-group latent-product model, source indicators are centered and
  products are generated separately within each group. Bootstrap resampling is
  stratified by group at the observed group sizes; every replicate re-centers
  the resampled source indicators within group and regenerates all products.
- The original (non-interaction) factors must pass the configural/metric gate.
  The joint structural model then constrains all factor loadings, including the
  interaction-factor loadings, equal across groups while leaving structural
  regressions group-specific. Automatic partial-invariance refitting is not
  used to bypass a failed gate.
- The latent-interaction estimand is the unstandardized interaction coefficient
  B. Group-specific B values, an all-group omnibus test, and group-pair
  differences are reported; standardized interaction coefficients are
  descriptive and are not used as the cross-group equality estimand.
- For every eligible indirect path, the group-specific index of moderated
  mediation is the product of the interaction B and the remaining
  unstandardized indirect-path coefficients. The app reports group indices,
  an all-group index test, and every group-pair index difference.
- Stratified bootstrap intervals and empirical two-sided p values are primary
  for the group indices and their pairwise differences. Delta-method intervals
  and joint-model Wald omnibus/pairwise tests remain auxiliary model-based
  results. Model-based BH adjustment is performed separately for interaction
  omnibus tests, interaction pair contrasts, index omnibus tests, and index
  pair contrasts. Bootstrap p values are likewise adjusted separately for
  group interaction estimates, interaction pair differences, group indices,
  and index pair differences; unrelated result families are never pooled.
- Inferential results are suppressed when a required joint model does not
  converge, is inadmissible, lacks a usable parameter covariance matrix, or a
  required coefficient is unavailable. A fixed-zero or wholly fixed component
  retains its point value but receives no inferential test. Bootstrap inference
  additionally requires at least two valid draws and at least 50% of requested
  draws; 50% to less than 80% is marked Caution, at least 80% Adequate, and
  fewer than 50% Unreliable with intervals and p values suppressed.
- The Audit records bootstrap execution separately from inference usability.
  A completed but unusable run remains recorded for reproducibility, but its
  confidence intervals and p values are suppressed and it is never described
  as bootstrap-primary evidence.
- The product-indicator, common-factor invariance, and Wald-test rules in this
  section apply to CB-SEM. PLS/PLSc latent moderation uses the construct-score
  procedure documented below and is not the same estimand as a CB-SEM latent
  product.
...
- PLSpredict imputes missing inputs from training-fold means only, refits the
  same PLS or PLSc measurement and structural estimator within each training
  fold, and applies the convergence and admissibility gate. Originally missing
  holdout outcomes are excluded from RMSE/MAE and the effective evaluation N is
  retained. Direct-antecedent PLS predictions are compared with a linear-model
  benchmark fitted on the same fold. Ten repeated splits are the default; one
  split or one mean error is insufficient evidence of external predictive
  performance.
...
### PLS MICOM and Multi-Group Structural Effects

- MICOM tests invariance of PLS composite scores. Its evidence is not extended
  to PLSc common-factor invariance, and PLSc multi-group inference is explicitly
  blocked.
- Step 1 audits identical indicators, model specification, data treatment, and
  PLS algorithm settings across groups. Step 2 pools only the two groups in each
  comparison, preserves their original sizes during permutation, and retains
  the sign of the composite-score correlation. Holm adjustment controls the
  family-wise error rate across the construct-by-pair family.
- Step 3 fixes composite scores from one pooled PLS fit and permutes only the
  group labels. Mean differences and log variance ratios use central equal-tail
  permutation intervals and plus-one two-sided p values. Full versus partial
  invariance is classified only after Step 2 passes.
- With three or more groups, every pair is evaluated separately. If only some
  pairs pass MICOM, structural-effect inference is admitted for those pairs and
  suppressed for the others. This release does not provide an omnibus PLS-MGA
  test, so the output must be identified as pairwise inference.
- Within-group PLS bootstrap refits direct, specific indirect, total indirect,
  and total effects against one canonical path registry. A contrast is group 1
  minus group 2; raw, BH-adjusted, and Holm-adjusted p values are reported within
  each effect family. Pairwise intervals and p values are suppressed when fewer
  than 80% of requested draw positions are jointly valid.
- N = 30 is not treated as a universal MICOM minimum. A small group does not
  stop estimation; N, missingness, valid resampling rate, and a stability/power
  warning are recorded instead.

### PLS/PLSc Missing-Data Handling

- PLS/PLSc uses one fixed, `seminr::mean_replacement`-compatible indicator-mean
  replacement policy. For the main fit, every missing indicator cell is
  replaced by that indicator's arithmetic mean over the observed analysis
  cases; no case is dropped solely because a model indicator is missing.
- Results and Audit JSON record the original analysis N, complete-case N,
  rows and cells receiving replacement, the cell percentage, indicator-level
  missingness, and each replacement mean.
- Bootstrap first resamples cases from the original analysis data and then
  recomputes indicator means inside every resample. The main fit and bootstrap
  therefore use the same replacement principle, although the actual means can
  vary across resamples.
- Mean replacement can distort variances and associations. Report the amount
  and likely mechanism of missingness and consider a complete-case or otherwise
  justified sensitivity analysis when missingness is material.

### PLS/PLSc Bootstrap

- Bootstrap defaults to 5,000 resamples and may be explicitly set to 1,000,
  5,000, 10,000, 20,000, or 50,000 resamples.
...
- A direct effect is the coefficient of each specified structural path. A
  specific indirect effect is the product of coefficients along one simple,
  acyclic mediating path from predictor to outcome and is reported once per
  path. The total indirect effect is the sum of all specific indirect effects
  for a predictor-outcome pair; the total effect is its direct effect plus its
  total indirect effect. All four effects are recomputed in every valid draw.
- Effect-table 95% percentile intervals use R `quantile` type 7. The two-sided
  empirical sign p value uses the plus-one rule
  `2 * (min(n[theta* <= 0], n[theta* >= 0]) + 1) / (R_valid + 1)`, capped at 1.
- BH adjustment is applied in four separate families: direct, specific
  indirect, total indirect, and total effects. An estimand appears only once
  within its family.
...
- Without bootstrap, point estimates β for all four effects remain visible,
  while Boot SE, interval, t, p, and BH p fields are blank and explicitly
  marked `Not requested`/point-estimate only.

### PLS Latent Moderation and Moderated Mediation

- The default latent-moderation procedure is two-stage estimation on PLS
  construct scores. Product-indicator and orthogonalized procedures may also
  be selected explicitly. Strong hierarchy is enforced so an interaction is
  not estimated without its required main effects.
- Every bootstrap replicate re-estimates the measurement model, construct
  scores, interaction term, and structural model. The procedure does not merely
  resample fixed original-data scores or a precomputed interaction column.
- Output includes the unstandardized interaction coefficient, simple slopes at
  the construct-score mean and mean plus/minus one standard deviation,
  conditional indirect effects, and indices of moderated mediation. An index
  is the interaction-path coefficient multiplied by the remaining downstream
  indirect-path coefficients.
- BH adjustment is performed separately for interaction effects, indices of
  moderated mediation, and conditional indirect effects. If fewer than 80% of
  requested whole draws are valid, intervals and p values are suppressed while
  point estimates and failure diagnostics remain available.
- When PLSc is selected, the generated interaction term is not consistency-
  corrected. Its interaction and moderated-mediation results therefore remain
  PLS construct-score estimands and must not be interpreted as PLSc common-
  factor interactions.
- Multi-group contrasts require pair-specific partial measurement invariance
  under MICOM. Group interaction/index estimates and admitted pairwise
  differences are reported with raw, BH-adjusted, and Holm-adjusted p values
  within each contrast family. An all-group omnibus moderation test for three
  or more groups is not currently provided.
...
- User labels have priority in displayed and exported tables. Raw variable
  ...
  for reproducibility.
- Audit schema 1.8 records app/R/package versions, model and code fingerprints,
  estimator, sampling design, seed, RNG, CI method, quantile type, valid draws,
  and failures. For multi-group latent moderation it also records group-specific
  centering/product-generation audit rows, the metric and interaction-loading
  constraint policy, group interaction/index estimands and contrasts, and
  stratified-bootstrap diagnostics. Schema 1.8 extends rather than discards the
  reproducibility fields introduced by schema 1.7.
...
Results can be added to the Result tab and saved as HTML. PDF, Excel, and Word are enabled in non-public development builds but deferred in public releases until version 1.3.0. Saved output includes the relevant tables, notes, warnings, skipped-analysis messages, selected methods, effect sizes, and confidence intervals where available.

Figures can be saved from analyses that produce figure output. Figure files use 300 dpi by default; only Pro offers the separate 600 dpi option.
