...
- 공개판 PDF·Word·Excel 저장과 Free 300 dpi 투명 배경 그림 저장을 제공합니다. 결과표·캔버스 저장·검증 문서를 개선했습니다. 메타분석과 동일 대상 내 처치 반복측정 분산분석은 이번 설치본에서 제외합니다.


## 미출시 변경 사항 - 2026-09-11

- UI 언어에 따른 PDF 표지와 StatEdu 로고·전체 이름 표시를 복구하고, 공개판 1.3.0 문서 저장 / 이후 Pro 로드맵을 정리했습니다.
- 회귀·매개조절 표의 세로 출력을 개선하고 잔차 진단 그림 두 개를 한 페이지로 묶었습니다. 주석은 출력된 통계량에 맞추며 위계 모형의 마지막 표에 모았습니다.
- 매개·조절효과 메뉴 이름을 정리하고 기존 회귀 하위 메뉴를 제거했습니다.
- 분석 명령어·실행 버튼을 분리하고 캔버스 분석과 저장 버튼을 변수 패널 아래로 이동했습니다.
- 구조방정식 본표·부록표의 글꼴과 확대 배율을 통일하고 HTML 헤더 겹침을 수정했습니다.
- 화면의 모형 도형·문자·화살표를 그대로 PNG로 저장하며 HTML/PDF 마지막에 결과 모형을 추가했습니다.
- PNG 배경을 투명하게 하고 Free 300 dpi / 개발자·Pro 600 dpi를 픽셀 해상도와 파일 메타데이터에 반영했습니다.

## 미출시 변경 사항 - 2026-09-05

- 메타분석 기능과 입력 템플릿, 단일집단 반복측정 ANOVA를 추가했습니다.
- 범주 라벨 처리를 확장하고 데이터·분석 화면을 개선했습니다.
- PLS 부트스트랩 작업자 배분을 개선하고 직렬·병렬 결과 재현성 검증을 추가했습니다.
- 긴 모형식이 여러 줄로 나뉠 때 LMM·GLMM 독립변수가 누락되는 오류를 수정했습니다. GLMM 분포 비교 경로에도 적용했습니다.
- 독립변수 유지, 메타분석, 반복측정 ANOVA, 범주 라벨 출력에 대한 회귀 검증을 추가했습니다.

## 이전 개발 변경 사항

- PDF, Excel, Word 결과 저장은 1.3.0 로드맵까지 공개판에서 숨기고 비공개 개발자판에서는 세 형식을 모두 개방했습니다.
- 공개판 그림 저장은 300 dpi로 유지하고 개발자판과 Pro 버전에서는 600 dpi 출력을 지원하도록 정리했습니다.
- 구조방정식 모형 편집기에서 잠재변수별 측정변수 거리를 1.0배, 1.5배, 1.7배, 2.0배로 조절하고 모형 파일에 저장할 수 있도록 추가했습니다.
- 구조모형 handler의 지연 등록과 공통 결과표 label cache를 적용해 분석 결과를 바꾸지 않고 전체 분석 메뉴의 첫 로딩 시간을 줄였습니다.
- 벌점회귀 선택 bootstrap에 결과 동일성이 검증된 적응형 직렬·병렬 경로를 적용했습니다. 기본 반복 수는 500회로 유지되며 같은 seed의 결과 객체는 동일합니다.
- SEM 구조효과 사례재표집 부트스트랩의 기본 신뢰구간을 bias-corrected(BC)로 변경하고 percentile 방식을 선택지로 유지했으며, 선택 방법을 결과와 audit 내보내기에 기록하도록 했습니다.
- 반영형·형성형 혼합모형에서 반영형 측정 부분의 PLS SRMR, d_G, d_ULS가 출력되도록 수정하고, 형성형 구성개념이 있으면 PLSc가 적용되지 않음을 명시했습니다. exact-fit 추론은 일반 모수 부트스트랩과 별개입니다.
