...
- 가정 검토: 종속변수 family/link 적합성, working correlation 타당성, 군집 내 상관, 과분산. Count에서는 Poisson dispersion ratio를 보고하고 과분산이 크면 negative binomial을 최종 family로 선택한다.
- 결측값: 일반 GEE는 likelihood 기반 FIML/MAR 방법이 아니다. complete-case 또는 complete-subject가 기본 처리이며, MAR dropout 가능성이 있으면 MI, IPW, WGEE를 민감도 분석으로 함께 검토한다.
...
- 결측값: LMM의 likelihood 기반 추정은 outcome missingness가 관측된 정보에 조건부로 MAR라고 볼 수 있을 때 available observations를 사용할 수 있다. 공변량 결측이나 dropout 메커니즘이 중요하면 MI 또는 IPW 민감도 분석을 추가한다.
...
GLMM은 binary, count, skewed positive outcome처럼 Gaussian LMM이 맞지 않는 반복측정 종속변수에서 subject-specific 효과를 추정한다(McCullagh & Nelder, 1989; Bates et al., 2015). 앱은 Binary(logit), Gamma(log), Count(log link)를 제공하며, Count에서는 Poisson dispersion screening 후 Poisson 또는 negative binomial을 최종 fitted family로 선택한다.
...
- 가정 검토: family/link 적합성, convergence/singular fit, random-effect structure, within-subject correlation, overdispersion. Count에서는 Poisson/NB screening의 dispersion ratio와 가능한 AIC/BIC 정보를 함께 확인한다.
- 결측값: LMM과 같이 likelihood 기반 available-observation 접근은 MAR 가정 아래 사용할 수 있다. 다만 binary/count 자료에서 dropout과 outcome 과정이 강하게 연결되어 있으면 MI 또는 IPW 민감도 분석을 보고한다.
...
- 표준오차: 앱은 group-clustered HC1 공분산을 사용한다. cross-sectional dependence가 뚜렷하면 Driscoll-Kraay 계열 표준오차 같은 대안을 고려한다.
