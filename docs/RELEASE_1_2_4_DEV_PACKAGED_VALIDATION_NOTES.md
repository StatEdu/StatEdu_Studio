...
Historical-artifact boundary (2026-08-26): this record remains valid only for
the exact 2026-08-23 installer identified below. Later source changes to
PLS/PLSc moderation, moderated mediation, multigroup inference, export/result
round-trip, and bundled validation packages have not been packaged into that
artifact. Their source-level validation must not be merged into this older
package record. A new clean build, checksum record, packaged-runtime smoke, and
manual QA record are required before those changes receive packaged status.

The private SmartPLS bundle used by this historical run is not present on the
current validation workstation. Consequently its required evidence gate has
not been rerun for the newer source state and remains blocked until the
original `STATEDU_SMARTPLS_EVIDENCE_ROOT` contents are supplied.
