...
| 결과 전환 | 계산 완료 뒤 결과표·결과 모형이 20초 이상 늦거나 표시되지 않음 | 반복 설정 I/O, 불필요한 worker 모듈, 결과/원본 snapshot 연결 누락 | `validate_mediation_moderation_runtime.R`: 각 worker 결과 RDS 읽기 1초 이내, 첫 fresh worker의 결과 읽기·job 정리 직후이자 두 번째 worker 시작 전에 사용자 캔버스 HTML을 단 한 번 uncached 생성하여 5초 이내인지 확인한다. 각 worker process 종료와 임시 job 폴더 재귀 정리도 확인; `validate_custom_model_canvas.R`: 결과 snapshot·UI 전환 계약 |
...
| 구조모형 기본 설정 | 새 CFA·SEM·PLS-SEM에서 표집구조가 미선언으로 시작하거나 SEM 간접효과 bootstrap이 꺼져 있음 | 공통 설정 UI와 실행 시 fallback의 기본값 불일치 | `validate_cfa_ui.R`·`validate_sem_canvas.R`: 기본 표집구조는 `독립 관측 횡단자료`, SEM 경로·간접·총효과 기본값은 5,000회, 선택지는 1,000 / 5,000 / 10,000 / 20,000 / 50,000이며 30,000은 없음. CFA·PLS-SEM의 기존 기본값은 유지 |
...
| SmartPLS 비공개 증적 | `STATEDU_CSEM_VALIDATION_MODE=optional`만 설정하면 비공개 SmartPLS 증적 검사도 선택 모드가 된다고 오인 | cSEM 패키지 검사와 벤더 증적 검사의 환경변수를 혼동 | SmartPLS 검사는 별도의 `STATEDU_SMARTPLS_EVIDENCE_MODE`를 사용한다. 설치·릴리스 게이트는 `required`와 절대경로 `STATEDU_SMARTPLS_EVIDENCE_ROOT`를 강제하고, 일반 전체 개발 검사만 명시적인 `optional`에서 증적 루트 미설정 SKIP을 허용한다. SKIP은 동등성 통과로 기록하지 않음 |
...
아래 `PoliticalDemocracy` 행은 2026-08-22 당시 비추적 로컬 CSV의 역사적 기준선이다.
현재 추적 가능한 계약은 `statedu_dmc_75_v1`(R 4.5.3, 고정 RNG/seed,
canonical SHA-256)이며, 다음 설치본은 현재 benchmark를 재실행해 이 행을 새 증거로
교체하기 전까지 과거 수치를 현재 fixture 결과로 간주하지 않는다.
