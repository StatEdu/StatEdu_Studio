...
- Kaplan–Meier, Cox 및 경쟁위험 결과의 HTML 저장과 Result 결과 모음 등록을 확인했다. PDF·Excel writer의 파일 생성·형식 검증은 향후 1.3.0 이후 기능을 위한 내부 회귀검증이며 현재 UI에는 노출하지 않는다.
...
- Kaplan–Meier, Cox, 경쟁위험 공개판 화면에는 HTML·그림·Result 추가 동작만 노출한다. 비공개 개발자판에서는 PDF·Excel·Word를 함께 노출한다.
...
- 실제 Shiny 테스트 세션에서 공개판은 HTML·그림·Result 추가만 활성화되고, 개발자판은 PDF·Excel·Word까지 활성화되는지 확인한다.
- Cox 결과를 격리된 임시 Result 저장소에 추가한 뒤 JSON 재개방, 결과 제목 및 내장 정적 HTML을 확인하여 Result 탭 연결을 검증한다.
- 문서상 공개판 범위와 공통 저장 정책을 대조하여 공개판은 HTML·300 dpi 그림·Result 추가/이력을 제공하고, 개발자판은 PDF·Excel·Word 및 600 dpi 출력을 개방하도록 수정했다. Pro도 600 dpi를 지원한다.
