...
이 문서는 **StatEdu Studio** 버전 1.3.0을 실제로 사용하는 절차를 설명합니다. 앱 실행, 데이터 열기, 변수 선택, 분석 실행, 결과 저장처럼 사용자가 화면에서 따라 해야 하는 작업 흐름을 다룹니다. 구현된 분석 기법 목록은 `ANALYSIS_METHODS_KO.md`, 방법 선택 기준과 해석상 주의점은 `METHOD_NOTES_KO.md`를 참고합니다.

## 1. 앱 실행

**StatEdu Studio**는 Windows PC에서 로컬로 실행되는 Shiny 앱입니다. 데이터는 사용자의 PC에서 분석되며 외부 서버로 전송되지 않습니다.

설치 후 시작 메뉴의 StatEdu Studio Dev를 실행합니다. 데이터 파일을 열고 변수명·측정수준·결측값과 범주 코딩을 확인합니다. 케이스 선택과 자료 분할이 활성화되어 있으면 분석에 적용되는 범위를 먼저 확인합니다. 분석에 필요한 변수와 옵션을 지정하고 실행합니다. UI 언어를 바꿔도 사용자 변수명과 데이터 값은 원문을 유지합니다.
...
public 1.2 분석 메뉴에는 빈도분석/기술통계, 교차표 분석, t-test / ANOVA, paired test, ANCOVA, 비모수 검정, 비모수 대응검정, 상관분석, 신뢰도, 평가자간 일치도, 요인분석, 주성분분석, 회귀분석, 매개·조절, 매개·조절효과, GLM, 로지스틱 회귀, 반복측정 ANOVA, 종단/패널 모형, 복합표본분석이 포함됩니다.
...
`매개·조절효과` 메뉴에서는 캔버스에 변수를 배치하고 화살표로 연결해 같은 분석 엔진에 전달한다. 캔버스 모형은 현재 지원하는 매개·조절 모형 번호와 일치해야 분석할 수 있으며, 실행 후에는 결과 모형 그림과 계수 라벨을 확인할 수 있다.
...
- `복합표본 로지스틱 회귀분석`: 옵션의 `로지스틱 모형`에서 이분형·다항형·순서형을 선택한다. 다항형은 종속변수 기준범주를 선택하고, 순서형은 지정된 범주 순서를 확인하거나 반전한다. 설계기반 오즈비·신뢰구간·Wald 검정을 제공하며 pseudo R-squared는 이분형에 제공한다. 다항·순서형은 완전사례 수, 변수별 결측 수, 결과 범주의 가중 분포와 변수별 공동검정을 함께 출력한다. 순서형은 조사설계를 반영한 평행기울기 진단을 제공하므로 공통 오즈비를 해석하기 전에 경고를 확인한다. 유의하지 않은 진단이 비례오즈를 입증하지는 않는다. `결과 추가`로 원형 결과를 보존하고 결과 화면에서 Word·HWPX로 저장할 수 있다.
...
버전 1.3.0 기준으로 표본수와 효과크기 메뉴가 별도 상위 메뉴로 제공된다. 이 메뉴는 연구계획 단계에서 필요한 최소 표본 수 `n`, 이미 정한 표본 수에서의 검정력, 그리고 표본수 계산에 넣을 효과크기를 계산하기 위한 도구다.
...
## 11. StatEdu Studio 1.3.0 — 사용자 안내서
...
CFA 화면에서 측정변수를 잠재요인에 배정하고 요인 간 관계를 지정합니다. 자료와 지원 옵션에 맞는 추정법·결측 처리·표준화·부트스트랩 설정을 선택합니다. 실행 후 식별·수렴·부적합해 경고를 먼저 읽고 적재량, 적합도, 신뢰도·AVE·HTMT를 확인합니다. 집단 비교에서는 지원되는 측정동일성 설정과 제약을 확인합니다.

### 11.3 SEM

SEM 캔버스에서 측정모형과 구조경로를 구성하고 직접·간접효과에 필요한 경로를 확인합니다. 추정법과 부트스트랩 옵션을 지정한 뒤 실행합니다. 경로계수·신뢰구간·적합도와 유효 반복 수를 함께 보고합니다. 분석 후 경로나 노드를 변경하면 기존 결과는 무효화되므로 다시 실행합니다. 화면 배치와 모형 파일을 저장해 재사용할 수 있습니다.

### 11.4 PLS-SEM / PLSc

PLS-SEM 화면에서 구성개념과 지표를 배정하고 반영형 Mode A 또는 형성형 Mode B를 선택합니다. PLSc는 지원되는 반영형 모형에 적용합니다. 경로와 부트스트랩·예측·집단 비교 옵션을 확인하고 실행합니다. 적재량 또는 가중치, 공선성, 신뢰도·타당도와 구조경로를 함께 검토합니다. 요청 반복 수와 유효 반복 수를 구분하며 추정 거부와 불충분한 반복 경고를 무시하지 않습니다.

### 11.5 생존분석

생존분석 설정에서 시간·사건 변수, 사건/검열 코드, 시간 단위와 필요한 집단·설명변수를 지정합니다. 추천 분석과 지원 조건을 확인해 Kaplan–Meier·생명표·RMST·Cox 또는 경쟁위험으로 이동합니다. RMST는 제한 시점 τ를 확인하고 Cox는 비례위험 진단을 검토합니다. 경쟁위험에서는 관심 사건과 다른 사건을 구분하며 CIF, 원인별 HR, Fine–Gray SHR을 같은 지표로 해석하지 않습니다.
