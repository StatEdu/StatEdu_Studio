...
PLSpredict는 Direct Antecedents 방식으로 k-fold 교차검증을 독립적으로 반복하며, UI 기본값은 10회이다. 각 반복의 seed를 기준 seed에서 순차적으로 생성하고 Audit에 기준 seed, fold 수, 반복 수와 반복별 결과를 기록한다. 지표별 RMSE/MAE에 대해 PLS와 선형모형 기준값의 반복 평균 차이, 분할 간 표준편차, PLS 오차가 더 낮았던 반복 비율을 함께 보고한다.
