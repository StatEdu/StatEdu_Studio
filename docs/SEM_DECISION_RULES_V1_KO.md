...
- PLSc는 모든 구성개념이 `commonFactor + reflective`로 명세된 경우에만 실행한다. composite, formative 또는 미확정 구성개념이 하나라도 포함된 혼합 모형에는 전체모형 consistency correction을 적용하지 않고 실행을 차단한다.
...
CB-SEM 직접·간접·총효과는 선택적으로 사례 재표집 percentile bootstrap 95% CI와 양측 bootstrap p 값을 보고한다. 각 효과별 요청 반복 수, 유효 반복 수, 유효율과 상태를 함께 기록하며 미수렴 또는 부적합 해는 유효 반복에서 제외한다. 유효율이 80% 미만이면 주의, 50% 미만이면 신뢰할 수 없는 결과로 표시한다. 기본 delta-method 구간과 bootstrap 구간을 혼동하지 않도록 bootstrap 실행 시 비표준화 효과의 CI와 p 값을 bootstrap 결과로 교체한다.

조절된 매개효과에서는 조절경로의 상호작용계수와 나머지 간접경로 계수의 곱으로 index of moderated mediation를 반복마다 다시 계산한다. index의 percentile CI와 bootstrap p를 유의 여부와 관계없이 보고하며, Johnson-Neyman 결과도 index의 사전 유의성 필터로 숨기지 않는다.
...
Q²는 생략 예측오차와 기준을 비교하는 표본 내 예측 관련성 지표이다. 0 초과는 `기술적 참고`이며 강한 표본외 예측력의 증거로 자동 판정하지 않는다. 0 이하 값은 해당 내생 구성개념의 예측 관련성을 검토해야 한다는 신호로 유지한다. 최종 예측력 평가는 반복 PLSpredict의 PLS–LM 차이, 분할 간 변동성, PLS 우세 반복 비율과 실제 적용 맥락의 손실함수를 함께 사용한다.
...
Audit schema 1.1은 분석자료와 검증자료에 대해 행·열 수, 변수명·자료형 및 직렬화된 내용의 SHA-256 fingerprint를 저장한다. 원자료 값은 manifest에 포함하지 않지만 동일한 자료·행 순서·변수 속성으로 실행했는지 fingerprint로 대조할 수 있다. 명세 fingerprint와 별도로 핵심 분석 함수 본문의 code fingerprint, Git commit·branch·dirty 상태, 운영체제·locale·RNG 종류와 주요 패키지 버전을 기록한다.
