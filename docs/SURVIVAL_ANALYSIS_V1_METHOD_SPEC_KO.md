...
공개판 UI에는 HTML 저장, 300 dpi 그림 파일 저장, Result 결과 모음만 노출한다. 비공개 개발자판에서는 PDF·Excel·Word 저장과 600 dpi 그림 출력을 검증 목적으로 개방한다. PDF·Excel·Word의 공개 제공은 1.3.0 공개 버전 또는 그 이후를 위한 계약이다.
...
- 공개판 생존 결과 화면은 HTML, 그림(해당 시), Result 추가 및 감사 저장을 제공하고 PDF·Excel·Word는 렌더링하지 않는다. 비공개 development edition에서는 PDF·Excel·Word를 렌더링한다.
- 공개판 그림 파일은 300 dpi이며 development 및 Pro edition은 600 dpi 출력을 지원한다.
