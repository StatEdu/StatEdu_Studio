...
Summary: all 27 effect-size comparison items matched the reference definition. SEM/CFA quantities are retained as sample-size planning diagnostics, not as reportable Effect Size menu outputs. For independent t-test conversion, StatEdu Studio uses the equal-n exact formula `2t/sqrt(df + 2)`, which matches the G*Power convention rather than the `effectsize::t_to_d` default approximation `2t/sqrt(df_error)`. Cramer's V is compared against the unadjusted definition (`adjust = FALSE`) used by StatEdu Studio.
