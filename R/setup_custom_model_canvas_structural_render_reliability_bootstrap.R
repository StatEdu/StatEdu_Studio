...
output[[paste0(prefix, "_result_reliability_bootstrap")]] <- renderUI({
  ...
  tagList(
    ...
    tags$p(class = "structural-result-note", if (ko) paste0("구간은 사례 재표집", if (identical(reliability_ci_method, "bca")) "과 leave-one-out 잭나이프 가속도" else " 백분위수 부트스트랩", "을 사용하고, 선택된 ", if (identical(bundle$validity_formula, "model_implied")) "모형-함의 모수" else "표준화 적재량", " AVE/CR 공식을 적용합니다. 이 출력에서 McDonald's omega total은 CR과 같은 적합된 congeneric scoring 공식에 따릅니다.") else paste0("Intervals use case resampling", if (identical(reliability_ci_method, "bca")) " with leave-one-out jackknife acceleration" else " percentile bootstrap", " and the selected ", if (identical(bundle$validity_formula, "model_implied")) "model-implied parameter" else "standardized-loading", " AVE/CR formula. McDonald's omega total follows the same fitted congeneric scoring formula as CR in this output.")),
