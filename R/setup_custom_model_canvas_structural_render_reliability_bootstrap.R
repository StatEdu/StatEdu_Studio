...
output[[paste0(prefix, "_result_reliability_bootstrap")]] <- renderUI({
  ...
  if (isTRUE(bundle$cfa_bootstrap_canceled) && is.null(values)) return(tags$p(class = "structural-result-note", if (ko) "AVE·신뢰도 부트스트랩이 사용자 요청으로 중단되었습니다. 점추정값과 기본 분석 결과는 유지됩니다." else "The AVE/reliability bootstrap was stopped by the user. Point estimates and base-model results remain available."))
