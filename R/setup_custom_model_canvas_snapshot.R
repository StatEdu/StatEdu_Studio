...
custom_model_canvas_snapshot_spec <- function(snapshot, selected_names, language = statedu_initial_language(), two_moderator_model = "3") {
  ...
  if (any(vapply(nodes, function(node) isTRUE(node$unassigned), logical(1)))) {
    stop(custom_model_canvas_text(language, "Assign data variables to the empty boxes.", "빈 측정변수에 왼쪽 데이터 변수를 끌어 넣으세요."))
  }
  ...
  names(nodes) <- node_id
  if (any(vapply(nodes, function(node) isTRUE(node$autoRole), logical(1)))) {
    paths <- Filter(function(edge) !identical(edge$kind, "covariance"), edges)
    sources <- vapply(paths, custom_model_canvas_record_value, character(1), key = "from")
    targets <- vapply(paths, custom_model_canvas_record_value, character(1), key = "to")
    if (any(!c(sources, targets) %in% node_id)) stop("연결된 변수를 찾을 수 없습니다. / Invalid path endpoint.")
    remaining <- node_id
    topology_order <- character(0)
    while (length(remaining)) {
      roots <- setdiff(remaining, targets[sources %in% remaining & targets %in% remaining])
      if (!length(roots)) stop("순환 경로는 매개·조절효과에서 지원하지 않습니다. / Cyclic paths are not supported.")
      topology_order <- c(topology_order, roots)
      remaining <- setdiff(remaining, roots)
    }
    path_ids <- vapply(paths, custom_model_canvas_record_value, character(1), key = "id")
    moderators <- vapply(Filter(function(link) custom_model_canvas_record_value(link, "toEdge") %in% path_ids, moderations), custom_model_canvas_record_value, character(1), key = "from")
    for (id in node_id) {
      nodes[[id]]$role <- if (id %in% moderators) "moderator" else if (id %in% targets) {
        if (id %in% sources) "mediator" else "dependent"
      } else if (id %in% sources) "independent" else "observed"
    }
    inferred <- vapply(nodes, custom_model_canvas_record_value, character(1), key = "role")
    if (any(inferred == "observed")) stop("연결되지 않은 측정변수가 있습니다. 화살표를 연결하거나 제거하세요. / Connect or remove unconnected variables.")
    if (!any(inferred == "dependent")) stop("종속변수로 향하는 화살표를 연결하세요. / Connect paths to an outcome.")
    if (sum(inferred == "moderator") > 2L) stop("조절변수는 최대 2개까지 지원합니다. / At most two moderators are supported.")
  }
  ...
  if (exists("topology_order", inherits = FALSE)) {
    mediator_nodes <- mediator_nodes[order(match(names(mediator_nodes), topology_order))]
    # Preserve graph order in the existing serial-mediator ordering helper.
    for (index in seq_along(mediator_nodes)) mediator_nodes[[index]]$x <- index
  }
