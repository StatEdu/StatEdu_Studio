...
structural_canvas_register_moderation_outputs <- function(output, prefix, fit_result, app_language_fn = NULL,
  ...
  output[[paste0(prefix, "_result_moderation_jn")]] <- renderUI({
    ...
    number <- if (is.function(table_number_fn)) table_number_fn("moderation_jn") else NA_character_
    title <- if (length(number) && !is.na(number) && nzchar(as.character(number))) {
      paste0("Table ", number, ". Johnson-Neyman regions of significance")
    } else {
      "Johnson-Neyman regions of significance"
    }
    div(class = "result-section regression-result-panel structural-main-result-panel structural-moderation-jn-result",
      h4(title),
      structural_canvas_basic_html_table(
        table,
        role = "main",
        orientation = "auto",
        note = "Note. Regions are shown only for statistically detectable interaction or moderated-mediation effects. Latent-moderator values use the lavaan regression factor-score scale.",
