...
survival_ui_text <- function(text, language = statedu_initial_language()) {
  ...
  statedu_localized_text(language, value[["en"]], value[["ko"]])
}
...
survival_coerce_duration <- function(values) {
  if (typeof(values) %in% c("integer", "double") && is.null(attributes(values))) {
    numeric_values <- as.numeric(values)
    missing <- is.na(values)
    return(list(values = numeric_values, missing = missing,
      invalid_encoding = !missing & is.na(numeric_values)))
  }
...
survival_normalize_event_map <- function(values, event_map = NULL, event_of_interest = "1") {
  # A supplied map does not use observed codes; preserve dispatch for special inputs.
  if (!(typeof(values) %in% c("integer", "double") && is.null(attributes(values)) &&
      identical(class(event_map), "data.frame") && all(c("raw_value", "role") %in% names(event_map)))) {
...
survival_preflight <- function(data, settings) {
  ...
  reason_counts <- lengths(reasons)
  excluded <- reason_counts > 0
  exclusion_labels <- rep("", length(reasons))
  single_reason <- which(reason_counts == 1L)
  if (length(single_reason)) exclusion_labels[single_reason] <- unlist(reasons[single_reason], use.names = FALSE)
  multiple_reasons <- which(reason_counts > 1L)
  if (length(multiple_reasons)) exclusion_labels[multiple_reasons] <- vapply(reasons[multiple_reasons], paste, collapse = ";", character(1))
...
survival_preflight_stop <- function(preflight) {
  ...
  stop(structure(list(message = message, call = NULL, issues = blocking),
    class = c("statedu_survival_preflight_error", "error", "condition")))
}
...
survival_rank_counts <- function(data, time, event, group_codes, times, k, entry) {
  if (!identical(class(data), "data.frame") || !is.character(entry) || is.object(entry) ||
      length(entry) != 1L || is.na(entry) || anyNA(group_codes) ||
      as.double(k) * length(times) > 2e6) return(NULL)
  exits <- data[[time]]
  events <- data[[event]]
  if (!is.numeric(exits) || is.object(exits) || !is.null(dim(exits)) || any(!is.finite(exits)) ||
      length(exits) != length(group_codes) || !is.logical(events) || is.object(events) ||
      !is.null(dim(events)) || length(events) != length(exits) || anyNA(events)) return(NULL)
  entries <- if (nzchar(entry)) data[[entry]] else NULL
  if (nzchar(entry) && (!is.numeric(entries) || is.object(entries) || !is.null(dim(entries)) ||
      length(entries) != length(exits) || any(!is.finite(entries)) || any(entries > exits))) return(NULL)
  rows <- split(seq_along(exits), factor(group_codes, levels = seq_len(k)))
  risk_counts <- event_counts <- matrix(0L, nrow = k, ncol = length(times))
  for (i in seq_len(k)) {
    index <- rows[[i]]
    # Strictly earlier exits are removed; delayed entry must precede the event.
    exited <- findInterval(times, sort(exits[index]), left.open = TRUE)
    entered <- if (is.null(entries)) length(index) else findInterval(times, sort(entries[index]), left.open = TRUE)
    risk_counts[i, ] <- entered - exited
    event_counts[i, ] <- tabulate(match(exits[index][events[index]], times), nbins = length(times))
  }
  list(risk = risk_counts, events = event_counts)
}
...
survival_weighted_rank_test <- function(data, time, event, group, method = "logrank", entry = "") {
  ...
  group_codes <- as.integer(group_values)
  counts <- survival_rank_counts(data, time, event, group_codes, times, k, entry)
  time_index <- 0L
  for (event_time in times) {
    ...
    if (is.null(counts)) {
      ...
      n_group <- as.numeric(tabulate(group_codes[risk], nbins = k))
      d_group <- as.numeric(tabulate(group_codes[event_at_time], nbins = k))
    } else {
      n_total <- sum(counts$risk[, time_index])
      d_total <- sum(counts$events[, time_index])
      if (n_total <= 1 || d_total <= 0) next
      n_group <- as.numeric(counts$risk[, time_index])
      d_group <- as.numeric(counts$events[, time_index])
    }
    ...
    if (k >= 20L) {
      # Keep each entry's scalar arithmetic order; small matrices favor the loop.
      value <- outer(-d_total * (n_total - d_total) * n_group, n_group,
        function(x, y) x * y) / (n_total^2 * (n_total - 1))
      diag(value) <- d_total * (n_total - d_total) * n_group *
        (n_total - n_group) / (n_total^2 * (n_total - 1))
      variance <- variance + weight^2 * value
    } else for (i in seq_len(k)) {
      for (j in seq_len(k)) {
        value <- if (i == j) {
...
survival_life_table <- function(data, time, event, group = "", breaks = numeric(0)) {
  ...
  group_conversion_quiet <- TRUE
  group_values <- if (nzchar(as.character(group %||% ""))) {
    withCallingHandlers(as.factor(data[[group]]),
      warning = function(w) group_conversion_quiet <<- FALSE,
      message = function(m) group_conversion_quiet <<- FALSE
    )
  } else NULL
  groups <- if (is.null(group_values)) "All" else levels(droplevels(group_values))
  rows <- list()
  interval_labels <- vector("list", length(breaks) - 1L)
  group_source <- data
  if (identical(class(data), "data.frame") &&
      is.character(time) && length(time) == 1L && time %in% names(data) &&
      is.character(event) && length(event) == 1L && event %in% names(data) &&
      all(vapply(data, function(column) {
        !is.object(column) || identical(class(column), "factor") ||
          identical(class(column), c("ordered", "factor"))
      }, logical(1)))) {
    group_source <- data[, unique(c(time, event)), drop = FALSE]
  }
  for (group_value in groups) {
    current_groups <- if (group_conversion_quiet || identical(group_value, "All")) group_values else as.factor(data[[group]])
    group_data <- if (identical(group_value, "All")) data else group_source[current_groups == group_value, , drop = FALSE]
    survival_value <- 1
    for (index in seq_len(length(breaks) - 1L)) {
      start <- breaks[[index]]
      end <- breaks[[index + 1L]]
      if (!is.object(group_data[[time]])) {
        risk_mask <- if (start == 0) group_data[[time]] >= 0 else group_data[[time]] > start
        in_interval <- risk_mask & group_data[[time]] <= end
        at_risk <- sum(risk_mask)
      } else {
      ...
      interval_label <- interval_labels[[index]]
      if (is.null(interval_label)) {
        label_quiet <- TRUE
        interval_label <- withCallingHandlers(
          sprintf("%s-%s", survival_format_number(start), survival_format_number(end)),
          warning = function(w) label_quiet <<- FALSE,
          message = function(m) label_quiet <<- FALSE
        )
        if (label_quiet) interval_labels[[index]] <- interval_label
      }
  ...
  columns <- lapply(names(rows[[1L]]), function(name) {
    unlist(lapply(rows, `[[`, name), use.names = FALSE)
  })
...
survival_km_plot_data <- function(fit, plot_type = "survival") {
  ...
  if (nrow(curve) > 0) {
    zero_rows <- data.frame(Time = 0, Survival = 1, Lower = 1, Upper = 1,
                            Strata = unique(curve$Strata), stringsAsFactors = FALSE)
...
survival_km_ggplot <- function(result, plot_type = "survival", figure_version = "color") {
  ...
  if (isTRUE(result$show_ci)) {
    missing_ci <- sum(!is.finite(curve$LowerValue) | !is.finite(curve$UpperValue))
    if (missing_ci > 0L) caption <- paste(c(caption, sprintf("Confidence limits unavailable at %d curve points; estimates are retained.", missing_ci)), collapse = "\n")
  }
  ...
  if (isTRUE(result$show_ci) && all(c("LowerValue", "UpperValue") %in% names(curve))) {
    p <- p +
      ggplot2::geom_step(ggplot2::aes(y = LowerValue), linewidth = 0.28, linetype = "22", alpha = 0.45, na.rm = TRUE) +
      ggplot2::geom_step(ggplot2::aes(y = UpperValue), linewidth = 0.28, linetype = "22", alpha = 0.45, na.rm = TRUE)
  }
...
survival_km_posthoc_table <- function(data, time, event, group, formula, test_method = "logrank", entry = "") {
  ...
  names(columns) <- names(rows[[1L]])
  table <- as.data.frame(columns, stringsAsFactors = FALSE)
...
survival_km_crossing_diagnostics <- function(fit, tolerance = 1e-10) {
  ...
  followup_ends <- setNames(vector("list", length(strata_levels)), strata_levels)
  followup_end <- function(level) {
    cached <- followup_ends[[level]]
    if (!is.null(cached)) return(cached)
    quiet <- TRUE
    value <- withCallingHandlers(max(time[strata == level]),
      warning = function(w) quiet <<- FALSE,
      message = function(m) quiet <<- FALSE
    )
    if (quiet) followup_ends[[level]] <<- value
    value
  }
  rows <- lapply(pairs, function(pair) {
    common_followup_end <- min(vapply(pair, followup_end, numeric(1)))
...
prepare_km_single_analysis_result <- function(
  ...
  if (length(attr(data, "statedu_scope_excluded"))) analysis_scope_prepare_variables(data, environment(), c("time", "event", "group", "entry"))
...
survival_rmst_contrasts <- function(estimates) {
  ...
  columns <- if (identical(class(estimates), 'data.frame') &&
    all(vapply(estimates, function(x) is.atomic(x) && is.null(attributes(x)), logical(1)))) unclass(estimates) else NULL
  batch_rows <- !is.null(columns) && all(c("Strata", "Tau", "RMST", "SE") %in% names(columns)) &&
    is.numeric(columns$Tau) && is.numeric(columns$RMST) && is.numeric(columns$SE)
  make_row <- if (batch_rows) function(..., stringsAsFactors = FALSE) list(...) else data.frame
  pairs <- utils::combn(seq_len(nrow(estimates)), 2L, simplify = FALSE)
  critical_value <- stats::qnorm(.975)
  rows <- lapply(pairs, function(index) {
    first <- if (is.null(columns)) estimates[index[[1]], , drop = FALSE] else lapply(columns, `[`, index[[1]])
    second <- if (is.null(columns)) estimates[index[[2]], , drop = FALSE] else lapply(columns, `[`, index[[2]])
    ...
    make_row(
      ...
      SE = se,
      LLCI = difference - critical_value * se,
      ULCI = difference + critical_value * se,
      z = z,
      p = if (is.finite(z)) 2 * stats::pnorm(abs(z), lower.tail = FALSE) else NA_real_,
      Ratio = ratio,
      RatioLLCI = if (is.finite(ratio_se)) exp(log(ratio) - critical_value * ratio_se) else NA_real_,
      RatioULCI = if (is.finite(ratio_se)) exp(log(ratio) + critical_value * ratio_se) else NA_real_,
      stringsAsFactors = FALSE
    )
  })
  out <- if (batch_rows) {
    values <- lapply(seq_along(rows[[1L]]), function(index) unlist(lapply(rows, `[[`, index), use.names = FALSE))
    names(values) <- names(rows[[1L]])
...
survival_cox_collinearity <- function(design) {
  ...
  finite_rows <- if (!is.object(design)) rowSums(!is.finite(design)) == 0 else
    apply(design, 1L, function(row) all(is.finite(row)))
  if (is.object(design) || !all(names(attributes(design)) %in% c("dim", "dimnames")) || !all(finite_rows)) {
  ...
  if (sum(estimable) == 1L) {
    ...
    regression_design <- if (!is.object(estimable_design)) cbind(1, estimable_design) else NULL
    for (position in seq_along(estimable_indexes)) {
      target <- estimable_design[, position]
      predictors <- if (!is.null(regression_design)) regression_design[, -(position + 1L), drop = FALSE] else
        cbind(1, estimable_design[, -position, drop = FALSE])
      fit <- stats::lm.fit(predictors, target)
...
survival_cox_functional_form_data <- function(data, covariates, martingale) {
  ...
  continuous <- covariates[vapply(covariates, function(name) {
    values <- data[[name]]
    if (!is.numeric(values) || is.factor(values)) return(FALSE)
    if (is.null(attributes(values)) && length(values) >= 4L) {
      probe <- values[seq_len(4L)]
      if (all(is.finite(probe)) && !anyDuplicated(probe)) return(TRUE)
    }
    length(unique(values[is.finite(values)])) >= 4L
  }, logical(1))]
  if (!length(continuous)) return(data.frame())
  finite_martingale <- if (is.numeric(martingale) && is.null(attributes(martingale))) is.finite(martingale) else NULL
  rows <- lapply(continuous, function(name) {
    values <- as.numeric(data[[name]])
    keep <- is.finite(values) & (finite_martingale %||% is.finite(martingale))
    complete <- all(keep)
    data.frame(
      `Analysis row` = which(keep),
      Covariate = name,
      Value = if (complete && is.null(attributes(values))) values else values[keep],
      `Martingale residual` = if (complete && is.null(attributes(martingale))) martingale else martingale[keep],
      check.names = FALSE,
      stringsAsFactors = FALSE
    )
  })
  if (is.null(names(rows)) && all(vapply(rows, function(row) .row_names_info(row, 1L) < 0L &&
    all(vapply(row, function(column) is.atomic(column) && is.null(attributes(column)), logical(1))), logical(1)))) {
    columns <- lapply(names(rows[[1L]]), function(name) unlist(lapply(rows, `[[`, name), use.names = FALSE))
    names(columns) <- names(rows[[1L]])
    return(as.data.frame(columns, check.names = FALSE, stringsAsFactors = FALSE))
  }
  do.call(rbind, rows)
}

survival_cox_influence_residuals <- function(fit) {
  quiet <- TRUE
  dfbeta <- withCallingHandlers(tryCatch(stats::residuals(fit, type = "dfbeta"), error = function(e) NULL),
    warning = function(w) quiet <<- FALSE, message = function(m) quiet <<- FALSE)
  reusable <- quiet && identical(class(fit), "coxph") && is.null(fit$weights) &&
    is.null(fit$na.action) && is.matrix(dfbeta) && !is.object(dfbeta) &&
    ncol(dfbeta) > 1L && all(is.finite(dfbeta))
  covariance <- if (reusable) drop(fit$naive.var %||% fit$var) else NULL
  reusable <- reusable && is.matrix(covariance) && !is.object(covariance) &&
    nrow(covariance) == ncol(dfbeta) && ncol(covariance) == ncol(dfbeta) &&
    all(is.finite(covariance)) && all(diag(covariance) > 0)
  dfbetas <- if (reusable) dfbeta %*% diag(sqrt(1 / diag(covariance))) else
    tryCatch(stats::residuals(fit, type = "dfbetas"), error = function(e) NULL)
  list(dfbeta = dfbeta, dfbetas = dfbetas)
}
...
survival_cox_basic_diagnostics <- function(fit, data, covariates) {
  ...
  influence_residuals <- survival_cox_influence_residuals(fit)
  dfbeta <- influence_residuals$dfbeta
  dfbetas <- influence_residuals$dfbetas
...
survival_adjusted_curve_once <- function(fit, data, group, times) {
  ...
  # Equal hazards give the same mean; retain the original calculation and time order.
  distinct_hazards <- unique(cumulative_hazard)
  hazard_index <- match(cumulative_hazard, distinct_hazards)
  ...
  rows <- lapply(levels, function(level) {
    ...
    risk_index <- NULL
    if (all(is.finite(risk)) && anyDuplicated(risk)) {
      distinct_risk <- unique(risk)
      risk_index <- match(risk, distinct_risk)
    }
    survival <- vapply(distinct_hazards, function(hazard) {
      values <- if (is.null(risk_index)) exp(-hazard * risk) else exp(-hazard * distinct_risk)[risk_index]
      # Preserve the mean calculation; skip removal only when no values are missing.
      mean(values, na.rm = anyNA(values))
    }, numeric(1))[hazard_index]
    data.frame(Level = level, Time = times, Survival = survival, stringsAsFactors = FALSE)
  })
  do.call(rbind, rows)
}

survival_bootstrap_intervals <- function(values) {
  if (!is.matrix(values) || !is.numeric(values) || is.object(values) ||
      nrow(values) < 1024L || ncol(values) == 0L || !is.null(names(dimnames(values)))) {
    return(apply(values, 2, stats::quantile, probs = c(.025, .975), na.rm = TRUE, names = FALSE))
  }
  intervals <- vapply(seq_len(ncol(values)), function(index) {
    stats::quantile(values[, index], probs = c(.025, .975), na.rm = TRUE, names = FALSE)
  }, numeric(2))
  colnames(intervals) <- colnames(values)
  intervals
}
  ...
  if (bootstrap_reps > 0L) {
    ...
    if (successful >= minimum_successful) {
      for (level in levels) {
        ...
        # Both endpoints use the same draws and quantile definition.
        intervals <- survival_bootstrap_intervals(values)
        estimate$Lower[target] <- intervals[1L, ]
        estimate$Upper[target] <- intervals[2L, ]
      }
    }
  } else {
    estimate$Lower <- NA_real_
...
prepare_cox_analysis_result <- function(data, time, event, covariates, event_value = "1", variable_info = NULL, reference_values = character(0), adjusted_group = "", adjusted_bootstrap_reps = 2000L, adjusted_times = "", entry = "", start = "", stop = "", subject_id = "", time_origin = "", time_unit = "", event_map = NULL, strata = "", cluster = "", spline_covariate = "", spline_df = 4L, time_varying_covariate = "", time_varying_times = "", ties_method = "efron") {
  if (length(attr(data, "statedu_scope_excluded"))) analysis_scope_prepare_variables(data, environment(), c("time", "event", "covariates", "adjusted_group", "entry", "start", "stop", "subject_id", "strata", "cluster", "spline_covariate", "time_varying_covariate"))
...
survival_fine_gray_engine <- function() {
  original <- cmprsk::crr
  # Opt in only to exact builds already compared with this runtime.
  path <- Sys.getenv("STATEDU_FINE_GRAY_NATIVE_DLL", "")
  if (!nzchar(path) || .Platform$OS.type != "windows" ||
      .Machine$sizeof.pointer != 8L || R.version$arch != "x86_64" ||
      as.character(getRversion()) != "4.5.3" ||
      as.character(utils::packageVersion("cmprsk")) != "2.2.12") return(original)
  engine <- tryCatch(suppressWarnings({
    path <- normalizePath(path, winslash = "/", mustWork = TRUE)
    accepted_builds <- c(
      "46ea81bd3b78a1284e7ce4f78654bea5", # Original validated prototype.
      "cebd7c0594c1cd189cb9983d1df5e16e"  # Reproducible Rtools45 build.
    )
    if (!unname(tools::md5sum(path)) %in% accepted_builds) return(original)
    loaded <- Filter(function(dll) identical(dll[["path"]], path), getLoadedDLLs())
    dll <- if (length(loaded)) loaded[[1L]] else dyn.load(path)
    address <- getNativeSymbolInfo("crrvvc", dll)$address
    engine <- original
    scope <- new.env(parent = environment(original))
    scope$.Fortran <- function(...) {
      args <- list(...)
      if (identical(args[[1L]], "crrvv")) {
        args[[1L]] <- address
        args$PACKAGE <- NULL
      }
      do.call(base::.Fortran, args)
    }
    environment(engine) <- scope
    engine
...
prepare_competing_risk_result <- function(
  ...
  if (length(attr(data, "statedu_scope_excluded"))) analysis_scope_prepare_variables(data, environment(), c("time", "event", "group", "covariates", "censoring_group"))
  previous_contrasts <- getOption("contrasts")
  options(contrasts = c(unordered = "contr.treatment", ordered = "contr.poly"))
  on.exit(options(contrasts = previous_contrasts), add = TRUE)
  ...
  if (regression %in% c("fine_gray", "both")) {
    ...
    fg_gtol <- 1e-6
    fg_maxiter <- 10L
    fg_engine <- survival_fine_gray_engine()
    fg_fit <- if (is.null(censoring_values)) {
      fg_engine(ftime = model_data[[time]], fstatus = status, cov1 = design, failcode = 1L, cencode = 0L, gtol = fg_gtol, maxiter = fg_maxiter)
    } else {
      fg_engine(ftime = model_data[[time]], fstatus = status, cov1 = design, failcode = 1L, cencode = 0L, cengroup = censoring_values, gtol = fg_gtol, maxiter = fg_maxiter)
    }
    fg_fit$call[[1L]] <- quote(cmprsk::crr)
