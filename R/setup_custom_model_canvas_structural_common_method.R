...
structural_canvas_common_method_lookup_label <- function(value, labels, language = "ko") {
  language <- normalize_app_language(language %||% "en")
  if (identical(language, "en")) return(as.character(value %||% ""))
  if (!identical(language, "ko")) {
    labels <- setNames(vapply(names(labels), function(en) statedu_localized_text(language, en), character(1)), names(labels))
  }
...
structural_canvas_common_method_method_label <- function(value, language = NULL) {
  ...
  structural_canvas_common_method_lookup_label(value, labels, language)
}
...
structural_canvas_common_method_guidance_label <- function(value, language = NULL) {
  labels <- {
    c(
      ...
      "Single-factor CFA requires at least three indicators." = "단일요인 CFA에는 관측변수가 최소 3개 필요합니다.",
  ...
  value <- as.character(value %||% "")
  translated <- structural_canvas_common_method_lookup_label(value, labels, language)
  prefixes <- c(
    "Single-factor CFA failed:" = "단일요인 CFA 계산 실패:",
    "Common latent factor model failed:" = "공통잠재요인 모형 계산 실패:"
  )
  for (prefix in names(prefixes)) {
    matched <- startsWith(value, prefix)
    if (any(matched)) translated[matched] <- paste0(
      structural_canvas_common_method_lookup_label(prefix, prefixes, language),
      substring(value[matched], nchar(prefix) + 1L)
    )
  }
  translated
}
...
structural_canvas_common_method_conclusion <- function(common_method, language = NULL) {
  ...
  data.frame(
    Status = structural_canvas_common_method_status_label(status, language),
    Guidance = structural_canvas_common_method_conclusion_guidance_label(guidance, language),
    stringsAsFactors = FALSE,
    check.names = FALSE
  )
}

structural_canvas_common_method_conclusion_guidance_label <- function(value, language = "ko") {
  labels <- c(
    "No common method diagnostics could be computed for the current model." = "현재 모형에서 동일방법편의 진단 결과를 계산하지 못했습니다.",
  ...
  translated <- structural_canvas_common_method_lookup_label(parts, labels, language)
...
structural_canvas_common_method_display_table <- function(common_method, format_values = TRUE, language = NULL) {
  ...
  ko <- identical(normalize_app_language(language %||% "en"), "ko")
  {
    ...
    names(table) <- structural_canvas_common_method_lookup_label(names(table), header_labels, language)
  }
