structural_canvas_multigroup_moderation_bootstrap_eligibility <- function(invariance_result) {
  gate <- invariance_result$product_factor_joint_gate %||% NULL
  if (!is.list(gate) || !isTRUE(gate$passed)) {
    ...
    return(list(
      eligible = FALSE, state = "product_factor_joint_gate_failed",
      reason = paste0(
        "Multi-group latent-moderation bootstrap was not started because ",
        "the joint product-factor model gate failed. ", reason
      )
    ))
  }
  structural_canvas_effect_bootstrap_original_fit_gate(list(
    fit = invariance_result$fits[["Free structural paths"]] %||% NULL
  ))
}
...
structural_canvas_execute_analysis <- function(snapshot, settings = NULL, input, session, dataset_fn, variable_table_fn, analysis_type, prefix, fit_result, app_language_fn = NULL, defer_pls_bootstrap = FALSE, defer_cfa_bootstrap = FALSE) {
  ...
  if (identical(analysis_type, "plssem")) missing <- "mean_replacement"
  ...
  invariance_path_scope <- options$invariance_path_scope %||% "all"
  invariance_selected_path_ids <- options$invariance_selected_path_ids %||% character(0)
  ...
  if(any(vapply(snapshot$nodes %||% list(),function(n)!is.null(n$scoreDesign),logical(1)))) {
    snapshot <- canvas_score_refresh(snapshot, data)
    data <- canvas_score_data(snapshot, data)
    if (!is.null(validation_data)) validation_data <- canvas_score_data(snapshot, validation_data)
  }
  ...
  missing_diagnostics <- if (!identical(analysis_type, "plssem")) {
    ...
    structural_canvas_pls_missing_diagnostics(data, result$observed %||% character(0))
  }
  ...
  if (identical(analysis_type, "cfa") && bollen_stine_bootstrap > 0L) {
    ...
    if (!isTRUE(eligibility$available)) stop(structural_canvas_reporting_text(eligibility$reason, statedu_current_language(app_language_fn)))
  }
  invariance_result <- structural_canvas_run_measurement_invariance(
    ...
    mga_bootstrap_reps = pls_bootstrap,
    mga_seed = pls_seed,
    result_coefficient = result_coefficient,
    result_measurement_coefficient = result_measurement_coefficient,
    invariance_path_scope = invariance_path_scope,
    invariance_selected_path_ids = invariance_selected_path_ids
  )
  ...
  bollen_stine_result <- if (cfa_bootstrap_pending) NULL else structural_canvas_run_bollen_stine_bootstrap(
    analysis_type, bollen_stine_bootstrap, result, bollen_stine_seed, language = statedu_current_language(app_language_fn)
  )
  ...
  pls_modmed_result <- NULL
  if (
    identical(analysis_type, "plssem") &&
      length(result$moderation_definitions %||% list()) &&
      is.list(pls_bootstrap_result) &&
      !is.null(pls_bootstrap_result$statedu_boot_paths)
  ) {
    pls_modmed_result <- structural_canvas_pls_modmed_from_bootstrap(
      result, pls_bootstrap_result,
      estimator = result$estimator %||% estimator
    )
    pls_modmed_result <- structural_canvas_pls_modmed_compact(pls_modmed_result)
  }
  if (is.list(pls_bootstrap_result)) {
    # Raw per-replicate arrays and effect matrices are internal hand-offs only.
    # Persist their summarized tables and validity metadata, not payloads that
    # can grow to hundreds of MB at 50,000 requested resamples.
    pls_bootstrap_result$statedu_boot_paths <- NULL
    pls_bootstrap_result$statedu_moderation_draws <- NULL
    pls_bootstrap_result$statedu_effect_draws <- NULL
  }
  ...
  effect_bootstrap_requested <- analysis_type %in% c("cbsem", "sem") && effect_bootstrap > 0L
  effect_bootstrap_gate <- if (isTRUE(effect_bootstrap_requested)) {
    structural_canvas_effect_bootstrap_original_fit_gate(result)
  } else {
    list(eligible = TRUE, state = "not_requested", reason = "")
  }
  multigroup_moderation_bootstrap_requested <- isTRUE(effect_bootstrap_requested) &&
    isTRUE(invariance_enabled) &&
    identical(as.character(invariance_result$subtype %||% ""), "latent_product_indicator") &&
    length(result$moderation_definitions %||% list()) > 0L
  multigroup_moderation_bootstrap_gate <- if (multigroup_moderation_bootstrap_requested) {
    structural_canvas_multigroup_moderation_bootstrap_eligibility(invariance_result)
  } else {
    list(eligible = FALSE, state = "not_requested", reason = "")
  }
  defer_effect_bootstrap <- isTRUE(effect_bootstrap_requested) && (
    isTRUE(effect_bootstrap_gate$eligible) ||
      isTRUE(multigroup_moderation_bootstrap_gate$eligible)
  )
  pooled_effect_bootstrap_blocked_reason <- if (isTRUE(effect_bootstrap_requested) && !isTRUE(effect_bootstrap_gate$eligible)) {
    as.character(effect_bootstrap_gate$reason %||% "Structural-effect bootstrap requires a converged, admissible original lavaan model.")
  } else ""
  multigroup_moderation_bootstrap_blocked_reason <- if (
    multigroup_moderation_bootstrap_requested &&
      !isTRUE(multigroup_moderation_bootstrap_gate$eligible)
  ) {
    as.character(multigroup_moderation_bootstrap_gate$reason %||%
      "Multi-group latent-moderation bootstrap requires a converged, admissible free multi-group model.")
  } else ""
  # The pooled structural-effect and multi-group latent-moderation passes have
  # independent eligibility and result contracts.  Do not let a blocked
  # multi-group pass suppress an otherwise valid pooled bootstrap (or vice
  # versa) in the saved bundle and result UI.
  effect_bootstrap_blocked_reason <- pooled_effect_bootstrap_blocked_reason
  effect_bootstrap_result <- if (isTRUE(effect_bootstrap_requested) && !isTRUE(defer_effect_bootstrap)) {
    NULL
  } else if (isTRUE(defer_effect_bootstrap)) NULL else {
  ...
  fit_result(list(
    ...
    effect_bootstrap_pending = isTRUE(effect_bootstrap_requested) &&
      isTRUE(effect_bootstrap_gate$eligible),
    effect_bootstrap_pooled_eligible = isTRUE(effect_bootstrap_gate$eligible),
    multigroup_moderation_bootstrap_requested = multigroup_moderation_bootstrap_requested,
    multigroup_moderation_bootstrap_pending = multigroup_moderation_bootstrap_requested &&
      isTRUE(multigroup_moderation_bootstrap_gate$eligible),
    multigroup_moderation_bootstrap_gate = multigroup_moderation_bootstrap_gate,
    multigroup_moderation_bootstrap_blocked_reason = multigroup_moderation_bootstrap_blocked_reason,
    multigroup_moderation_bootstrap_canceled = FALSE,
    multigroup_moderation_bootstrap_error = NULL,
    effect_bootstrap_canceled = FALSE,
    effect_bootstrap_error = NULL,
    effect_bootstrap_blocked_reason = effect_bootstrap_blocked_reason,
    ...
    pls_bootstrap = pls_bootstrap, pls_seed = pls_seed,
    pls_bootstrap_result = pls_bootstrap_result, pls_modmed_result = pls_modmed_result,
    ...
    invariance_enabled = invariance_enabled, invariance_group = invariance_group,
    invariance_path_scope = invariance_path_scope,
    invariance_selected_path_ids = invariance_selected_path_ids,
  ...
  session$sendCustomMessage(
    ...
    local({
      result_snapshots <- structural_canvas_group_result_snapshots(
        snapshot, result$fit, result_coefficient, pls_bootstrap_result,
        result_measurement_coefficient, invariance_result,
        statedu_current_language(app_language_fn),
        effect_bootstrap_state = list(
          requested = effect_bootstrap,
          pending = isTRUE(effect_bootstrap_requested) &&
            isTRUE(effect_bootstrap_gate$eligible), canceled = FALSE,
          blocked_reason = effect_bootstrap_blocked_reason,
          result = effect_bootstrap_result
        )
      )
      list(
