structural_canvas_fit_admissibility <- function(fit, validated_model_df = NULL) {
  converged <- isTRUE(lavaan::lavInspect(fit, "converged"))
  post_check <- isTRUE(lavaan::lavInspect(fit, "post.check"))
  # Prepared bootstrap fits deliberately omit expensive test slots. A caller
  # that already validated the unchanged model/ParTable may therefore supply
  # that model's df. Ordinary callers keep the public fitMeasures() path.
  model_df <- if (is.null(validated_model_df)) {
    tryCatch(
      suppressWarnings(as.numeric(lavaan::fitMeasures(fit, "df")[[1L]])),
      error = function(error) NA_real_
    )
  } else {
    candidate <- suppressWarnings(as.numeric(validated_model_df))
    if (length(candidate) == 1L && is.finite(candidate)) candidate[[1L]] else NA_real_
  }
