...
structural_canvas_quality_display_guidance <- function(item, guidance) {
  guidance_map <- c(
    ...
    "Missing-data method" = "PLS/PLSc는 지표 평균 대체를 사용합니다. 대체 사례·셀 수를 보고하고 bootstrap은 각 재표집 안에서 지표 평균을 다시 계산함을 명시하십시오.",
...
structural_canvas_quality_display_rows <- function(rows, ko = FALSE, language = NULL) {
  if (!is.null(language) && !normalize_app_language(language) %in% c("en", "ko")) {
    # Generated checklist prose can contain math syntax (for example |1|).
    # Translate it here rather than relying on the generic cell classifier.
    for (column in intersect(c("Item", "Guidance", "Priority", "Action"), names(rows))) {
      rows[[column]] <- vapply(as.character(rows[[column]]), function(value) statedu_localized_text(language, value), character(1))
    }
    # These are generated diagnostic flags, not user-supplied labels or model options.
    if ("Value" %in% names(rows)) rows$Value <- vapply(as.character(rows$Value), function(value) {
      if (identical(value, "TRUE")) return(statedu_localized_text(language, "Yes", "예"))
      if (identical(value, "FALSE")) return(statedu_localized_text(language, "No", "아니오"))
      if (value %in% c("mean_replacement", "not recorded", "Not executed", "Executed; no comparable indicator metrics")) {
        return(statedu_localized_text(language, value))
      }
      if (grepl("^[0-9]+/[0-9]+ indicator metrics favor PLS over LM$", value)) {
        counts <- strsplit(sub(" indicator metrics favor PLS over LM$", "", value), "/", fixed = TRUE)[[1L]]
        template <- statedu_localized_text(language, "{count}/{total} indicator metrics favor PLS over LM")
        return(gsub("{total}", counts[[2L]], gsub("{count}", counts[[1L]], template, fixed = TRUE), fixed = TRUE))
      }
      value
    }, character(1))
    return(rows)
...
structural_canvas_quality_status_summary_display <- function(rows, ko = FALSE, language = NULL) {
  if (!is.null(language) && !normalize_app_language(language) %in% c("en", "ko")) {
    tr <- function(text) statedu_localized_text(language, text)
    if (!nrow(rows) || !"Status" %in% names(rows)) return(tr("Quality status: not assessed."))
    levels <- c("OK", "Review", "Reference only", "Descriptive only", "Screen only", "Not assessed")
    counts <- table(factor(rows$Status, levels = levels))
    return(paste0(tr("Quality status"), ": ", paste0(vapply(levels, tr, character(1)), "=", as.integer(counts), collapse = "; "), "."))
  }
  if (!isTRUE(ko)) {
...
structural_canvas_quality_reporting_readiness_display <- function(review_rows, rows, ko = FALSE, language = NULL) {
  if (!is.null(language) && !normalize_app_language(language) %in% c("en", "ko")) {
    tr <- function(text, n = NULL) {
      text <- statedu_localized_text(language, text)
      if (!is.null(n)) text <- gsub("{n}", as.character(n), text, fixed = TRUE)
      text
    }
    if (!nrow(review_rows)) return(tr("Reporting readiness: no quality-review blockers detected."))
    critical_n <- sum(review_rows$Priority == "Critical")
    major_n <- sum(review_rows$Priority == "Major")
    if (critical_n > 0L) return(tr("Reporting readiness: blocked until {n} critical review item(s) are resolved.", critical_n))
    if (major_n > 0L) return(tr("Reporting readiness: requires resolution or explicit justification for {n} major review item(s).", major_n))
    return(tr("Reporting readiness: advisory review item(s) should be documented."))
  }
  if (!isTRUE(ko)) {
...
structural_canvas_lavaan_quality_result_ui <- function(bundle, analysis_type = "cfa", language = statedu_initial_language()) {
  ...
  display_rows <- structural_canvas_quality_display_rows(rows, ko, language)
  display_review_rows <- structural_canvas_quality_display_rows(review_rows, ko, language)
  div(
    class = "result-section regression-result-panel structural-lavaan-quality-result",
    h4(statedu_localized_text(language, "SEM quality checklist", "SEM 품질 체크리스트")),
    ...
    result_note_paragraph(
      class = "structural-result-note",
      statedu_localized_text(language, "This checklist summarizes lavaan SEM/CFA convergence, admissibility, sample adequacy, common-method-bias screens, chi-square/df, robust/scaled fit source, global fit, measurement quality, discriminant-validity risk, and structural explanatory-power conditions for reporting and review.", "이 체크리스트는 lavaan SEM/CFA의 수렴, 해의 허용성, 표본 적절성, 공통방법편향 점검, χ²/df, robust/scaled 적합도 출처, 전역 적합도, 측정 품질, 판별타당도 위험, 구조모형 설명력 조건을 요약합니다.")
    )
  )
}
...
structural_canvas_additional_fit_indices_ui <- function(additional_fit, ko = FALSE, language = if (ko) "ko" else "en") {
  ...
  tagList(lapply(names(tables), function(family) {
    display <- tables[[family]]
    names(display) <- vapply(names(display), structural_canvas_reporting_text, character(1), language = language)
    attr(display, "result_user_columns") <- seq_along(display)
    tags$div(
      class = paste("structural-additional-fit-family", if (family %in% c("incremental", "rmsea", "residual")) "landscape-table-panel"),
      tags$h6(statedu_localized_text(language, structural_canvas_additional_fit_family_label(family), structural_canvas_additional_fit_family_label(family, TRUE))),
      tags$div(
...
structural_canvas_fit_guidance_result_ui <- function(bundle, language) {
  ...
  labels <- if (length(selections) > 1L) c(statedu_localized_text(language, "Research model", "연구모형"), bundle$comparison_label %||% statedu_localized_text(language, "Modified model", "탐색적 수정모형")) else statedu_localized_text(language, "Research model", "연구모형")
  ...
  display_summaries <- vapply(summaries, structural_canvas_appendix_ui_text, character(1), language = language)
  if (!normalize_app_language(language) %in% c("en", "ko")) {
    table$Reference <- vapply(table$Reference, structural_canvas_reporting_text, character(1), language = language)
    attr(table, "result_user_columns") <- c("Model", "Reference")
  } else attr(table, "result_user_columns") <- "Model"
  div(class = "structural-fit-guidance-result",
    tags$h5(statedu_localized_text(language, "Guide for Table 2: Reference-based fit guidance", "표 2 가이드: 기준 기반 적합도 안내")),
    structural_canvas_basic_html_table(
      table,
      class = "table table-striped table-bordered structural-landscape-table structural-fit-guidance-table",
      role = "appendix",
      orientation = "auto",
      language = language
    ),
    tags$p(paste(vapply(names(display_summaries), function(name) paste0(name, ": ", display_summaries[[name]]), character(1)), collapse = " | ")),
    result_note_paragraph(class = "structural-result-note", statedu_localized_text(language, "Reference-only/review labels locate estimates relative to common approximate ranges; they do not accept or reject the model. Also inspect identification, residuals, parameter plausibility, theory, sample characteristics, and plausible alternative models.", "기준 참고/검토 표시는 흔히 쓰는 근사 범위와의 위치를 설명할 뿐 모형의 채택·기각 판정이 아닙니다. 모형 식별, 잔차 진단, 모수 타당성, 이론, 표본 특성, 대안 모형 비교를 함께 검토하십시오.")),
    result_note_paragraph(class = "structural-result-note", statedu_localized_text(language, "Common reference points: CFI/TLI .90/.95, RMSEA .08/.06, and SRMR .10/.08. These are context-dependent reference points, not universal cutoffs.", "흔한 참고점: CFI/TLI .90/.95, RMSEA .08/.06, SRMR .10/.08. 이 값은 맥락 의존적 참고점이며 보편적 절단값이 아닙니다.")),
    if (any(table$Guidance == "Not assessed")) result_note_paragraph(class = "structural-result-note", statedu_localized_text(language, "Fit guidance is not assessed for saturated models (df = 0) or unavailable fit indices.", "포화모형(df = 0)이거나 적합도 지수가 없으면 적합도 안내를 평가하지 않습니다.")),
    if (nrow(additional_fit)) tagList(
      tags$h5(statedu_localized_text(language, "Guide for Table 2: Additional fit indices not shown in Table 2", "표 2 가이드: 표 2에 직접 표시하지 않은 추가 적합도 통계량")),
      structural_canvas_additional_fit_indices_ui(additional_fit, ko, language)
    )
  )
...
structural_canvas_rmsea_tests_result_ui <- function(bundle, language = statedu_initial_language()) {
  ...
  if (!identical(normalize_app_language(language), "en")) {
    display$Model[[1L]] <- statedu_localized_text(language, if (nrow(display) > 1L) "Original model" else "Fitted model", if (nrow(display) > 1L) "원래 모형" else "적합 모형")
    if (nrow(display) > 1L && is.null(bundle$comparison_label)) display$Model[[2L]] <- statedu_localized_text(language, "Modified model", "수정 모형")
  }
  attr(display, "result_user_columns") <- c("Model", "Source")
  tagList(
    tags$h5(statedu_localized_text(language, "Guide for Table 2: RMSEA hypothesis tests", "표 2 가이드: RMSEA 가설검정")),
    structural_canvas_basic_html_table(
      display,
      class = "table table-striped table-bordered structural-landscape-table structural-rmsea-tests-table",
      role = "appendix",
      orientation = "auto",
      language = language
    ),
    result_note_paragraph(class = "structural-result-note", statedu_localized_text(language, "Close-fit tests H0: population RMSEA <= .05; a small p value rejects close fit. Not-close tests H0: population RMSEA >= .08; a small p value rejects poor approximate fit. Interpret both with the RMSEA estimate and confidence interval.", "Close-fit 검정의 귀무가설은 모집단 RMSEA <= .05입니다. 작은 p값은 close fit을 기각합니다. Not-close 검정의 귀무가설은 모집단 RMSEA >= .08입니다. 작은 p값은 poor approximate fit을 기각합니다. 두 검정은 RMSEA 추정값 및 신뢰구간과 함께 해석하십시오.")),
    result_note_paragraph(class = "structural-result-note", statedu_localized_text(language, "Robust or scaled p values are selected to match the reported RMSEA when available. These hypothesis tests are sample-size sensitive and are not standalone model-acceptance rules.", "가능한 경우 보고된 RMSEA와 맞는 robust 또는 scaled p값을 선택합니다. 이 가설검정은 표본크기에 민감하며 독립적인 모형 채택 규칙이 아닙니다."))
  )
...
structural_canvas_information_criteria_result_ui <- function(bundle, language = statedu_initial_language()) {
  ...
  display <- table
  display$Model[[1L]] <- structural_canvas_reporting_text(if (nrow(display) > 1L) "Original model" else "Fitted model", language)
  if (nrow(display) > 1L && is.null(bundle$comparison_label)) display$Model[[2L]] <- structural_canvas_reporting_text("Modified model", language)
  display$`Comparison status` <- vapply(display$`Comparison status`, structural_canvas_reporting_text, character(1), language = language)
  if (!identical(normalize_app_language(language), "en")) display$Admissible <- vapply(ifelse(display$Admissible, "Yes", "No"), structural_canvas_reporting_text, character(1), language = language)
  ...
  attr(display, "result_user_columns") <- c("Model", "Estimator", "Admissible", "Comparison")
  tagList(
    tags$h5(statedu_localized_text(language, "Guide for Table 2: Likelihood-based information criteria", "표 2 가이드: 우도 기반 정보기준")),
    structural_canvas_basic_html_table(
      display,
      class = "table table-striped table-bordered structural-landscape-table structural-information-criteria-table",
      role = "appendix",
      orientation = "auto",
      language = language
    ),
    result_note_paragraph(class = "structural-result-note", statedu_localized_text(language, "Lower AIC, BIC, and adjusted BIC indicate better relative expected fit after penalizing complexity; delta values are relative to the smallest criterion in this displayed set.", "AIC, BIC, adjusted BIC가 낮을수록 복잡도 벌점을 반영한 상대적 기대 적합이 더 좋다는 뜻입니다. Delta 값은 이 표에 표시된 모형 중 가장 작은 기준값을 기준으로 계산합니다.")),
    if (any(grepl("^Not comparable", table$`Comparison status`))) result_note_paragraph(class = "structural-result-note", statedu_localized_text(language, "Delta values were suppressed because analyzed observations, observed variables, estimator family, or model admissibility differed across models.", "분석 관측치, 관측변수, 추정량 계열 또는 모형 허용성이 모형 간에 달라 delta 값을 숨겼습니다.")),
    result_note_paragraph(class = "structural-result-note", statedu_localized_text(language, "Compare information criteria only for models fitted to the same observations and observed variables with the same likelihood and estimator family. They are not absolute goodness-of-fit tests and are not reported for WLSMV models without a comparable likelihood.", "정보기준은 같은 관측치와 관측변수, 같은 likelihood 및 추정량 계열로 적합한 모형끼리만 비교하십시오. 이는 절대적 적합도 검정이 아니며, 비교 가능한 likelihood가 없는 WLSMV 모형에는 보고하지 않습니다."))
  )
...
structural_canvas_bollen_stine_result_ui <- function(bundle, language) {
  ...
  tagList(
    tags$h5(statedu_localized_text(language, "Bollen-Stine bootstrap global-fit test", "Bollen-Stine 부트스트랩 전반적 적합도 검정")),
    structural_canvas_basic_html_table(display, role = "appendix", orientation = "auto", language = language),
    result_note_paragraph(class = "structural-result-note", statedu_localized_text(language, "The bootstrap p value uses the plus-one correction: (1 + bootstrap chi-square values at least as large as observed) / (1 + valid replicates). Valid replicates pass the same convergence, variance, covariance-matrix, df, and latent-correlation admissibility checks as the main CFA. A small p value indicates global model misfit under the exact-fit null.", "부트스트랩 p값은 plus-one 보정식 (1 + 관측값 이상인 부트스트랩 χ² 수) / (1 + 유효 반복 수)을 사용합니다. 유효 반복은 주 CFA와 같은 수렴, 분산, 공분산행렬, 자유도, 잠재상관 허용성 점검을 통과한 반복입니다. 작은 p값은 exact-fit 귀무가설에서 전반적 모형 부적합을 의미합니다.")),
    result_note_paragraph(class = "structural-result-note", statedu_localized_text(language, "Monte Carlo SE and the 95% Wilson interval quantify simulation error from the finite number of valid bootstrap replicates; they are not a confidence interval for a population model parameter.", "Monte Carlo SE와 95% Wilson 구간은 유효 부트스트랩 반복 수가 유한하기 때문에 생기는 시뮬레이션 오차를 나타냅니다. 모집단 모형 모수에 대한 신뢰구간이 아닙니다.")),
    if (any(result$Status != "Adequate")) result_note_paragraph(class = "structural-result-note", statedu_localized_text(language, "Fewer than 80% of requested resamples produced a converged admissible statistic. Treat the bootstrap p value and Monte Carlo interval as unstable; resolve convergence or admissibility problems before reporting.", "요청한 재표집의 80% 미만만 수렴하고 허용 가능한 통계량을 만들었습니다. 부트스트랩 p값과 Monte Carlo 구간을 불안정한 값으로 보고, 보고 전에 수렴 또는 허용성 문제를 해결하십시오.")),
    if (isTRUE(bundle$modified_from_baseline)) result_note_paragraph(class = "structural-result-note", statedu_localized_text(language, "This model was modified using the analyzed data. Its Bollen-Stine result is exploratory and does not provide confirmatory evidence for the data-driven modification.", "이 모형은 분석 자료를 보고 수정한 탐색적 수정 모형입니다. Bollen-Stine 결과는 탐색적 참고값이며 자료 기반 수정에 대한 확인적 근거를 제공하지 않습니다.")),
    result_note_paragraph(class = "structural-result-note", statedu_localized_text(language, "This transformed-data test is available only for complete continuous single-group ML CFA and does not replace approximate fit indices, residual diagnostics, or substantive model evaluation.", "이 transformed-data 검정은 결측이 없는 연속형 단일집단 ML CFA에서만 사용할 수 있으며, 근사 적합도 지수, 잔차 진단, 실질적 모형 평가를 대체하지 않습니다."))
  )
