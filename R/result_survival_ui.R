...
survival_cell_note_marker <- function(table, row_index, column) {
  ...
  # Ordinary marker tables only need the first match, not a copied data frame.
...
survival_simple_table <- function(
  table,
  class = "survival-result-table",
  table_role = "appendix",
  table_language = NULL,
  ...
  role <- result_table_role(table_role, table = table, default = "appendix")
  language <- result_table_language(role, table_language)
  display_table <- if (identical(role, "appendix")) {
    survival_appendix_localize_table(table, language)
  } else {
    table
  }
  # Reuse column formatting within this table; the final row has a distinct border.
  column_classes <- vector("list", length(display_table))
  body_styles <- lapply(seq_along(display_table), function(col_index) {
    lapply(c(FALSE, TRUE), function(last) {
      paste0(result_body_cell_style(col_index == 1L, last), if (col_index == 1L) "" else "text-align:center !important;")
    })
  })
  table_tag <- tags$table(
    class = paste("coefficient-table", class),
    style = result_table_style(font_size = 12, min_width = 0),
    tags$thead(tags$tr(lapply(seq_along(display_table), function(col_index) {
      name <- names(display_table)[[col_index]]
      tags$th(
        class = {
          quiet <- TRUE
          value <- withCallingHandlers(survival_column_class(name),
            warning = function(w) quiet <<- FALSE,
            message = function(m) quiet <<- FALSE)
          # Preserve per-cell diagnostics when formatting was not quiet.
          if (quiet) column_classes[[col_index]] <<- value
          value
        },
    ...
    tags$tbody(lapply(seq_len(nrow(display_table)), function(row_index) {
      tags$tr(lapply(seq_along(display_table), function(col_index) {
        column <- names(display_table)[[col_index]]
        source_column <- names(table)[[col_index]]
        value <- display_table[[col_index]][[row_index]]
        if (is.numeric(value)) value <- survival_format_number(value)
        marker <- survival_cell_note_marker(table, row_index, source_column)
        tags$td(
          class = column_classes[[col_index]] %||% survival_column_class(column),
          style = body_styles[[col_index]][[if (row_index == nrow(display_table)) 2L else 1L]],
          survival_cell_content(value, marker)
        )
      }))
    }))
  )
  intrinsic_width <- result_table_intrinsic_width(display_table, min_width = 0)
...
survival_table_note <- function(text) {
  ...
  result_note_div(class = "survival-table-note result-table-standalone-note", text)
}

survival_appendix_note <- function(language, english, korean) {
  language <- result_appendix_table_language(language)
  survival_table_note(statedu_localized_text(language, english, korean))
}

survival_appendix_localize_table <- function(table, language = NULL) {
  preserve_named_evidence <- function(display) {
    if (is.data.frame(table) && is.data.frame(display) && all(c("Code", "Evidence") %in% names(table))) {
      selected <- table$Code %in% c(
        "zero_event_stratum", "sparse_event_stratum", "categorical_joint_test_not_estimable",
        "fine_gray_categorical_joint_test_not_estimable", "crossing_survival_curves",
        "cause_specific_ph_signal", "cause_specific_categorical_joint_test_not_estimable",
        "fine_gray_residual_time_pattern", "fine_gray_sparse_censoring_stratum",
        "cif_integrity_failure", "gray_test_not_estimable"
      )
      column <- match("Evidence", names(table))
      if (any(selected)) display[[column]][selected] <- table$Evidence[selected]
    }
    display
  }
  if (is.data.frame(table) && all(c("Role", "Label") %in% names(table)) && any(c("Raw value", "Event value") %in% names(table))) {
    lang <- result_appendix_table_language(language)
    roles <- c(event_of_interest = "관심 사건", competing_event = "경쟁사건", censored = "검열", other_state = "기타 상태", exclude = "제외", unknown = "미확인")
    table$Role <- vapply(as.character(table$Role), function(x) {
      if (is.na(x) || !x %in% names(roles) || identical(lang, "en")) return(x)
      if (identical(lang, "ko")) return(unname(roles[[x]]))
      statedu_localized_text(lang, paste("Survival event role", x))
    }, character(1), USE.NAMES = FALSE)
    attr(table, "result_user_columns") <- unique(c(attr(table, "result_user_columns"), "Role", "Raw value", "Event value", "Label"))
  }
  if (is.data.frame(table) && "Exclusion reason" %in% names(table)) {
    reasons <- c(missing_time = "시간 결측", missing_event = "사건 값 결측", missing_group = "집단 값 결측", missing_covariate = "공변량 결측",
      invalid_time_encoding = "시간 값 형식 오류", invalid_time = "유효하지 않은 시간",
      missing_entry = "진입 시간 결측", invalid_time_order = "시간 순서 오류",
      missing_interval = "구간 시간 결측", invalid_interval_time = "유효하지 않은 구간 시간",
      missing_subject_id_value = "대상자 ID 결측", overlapping_interval = "중복된 위험 구간",
      interval_after_event = "사건 발생 후 구간", multiple_subject_events = "대상자별 다중 사건",
      excluded_event_code = "제외하도록 지정한 사건 코드",
      competing_event_requires_competing_risk = "경쟁위험 분석이 필요한 경쟁사건",
      unsupported_other_state = "지원되지 않는 기타 상태")
    table[["Exclusion reason"]] <- vapply(as.character(table[["Exclusion reason"]]), function(x) {
      if (!is.na(x) && x %in% names(reasons)) statedu_localized_text(result_appendix_table_language(language), x, unname(reasons[[x]])) else x
    }, character(1), USE.NAMES = FALSE)
    attr(table, "result_user_columns") <- unique(c(attr(table, "result_user_columns"), "Exclusion reason"))
  }
  localized <- result_appendix_localize_table(table, language)
  if (!is.data.frame(localized) || !identical(result_appendix_table_language(language), "ko")) return(preserve_named_evidence(localized))
  header_labels <- c(
    "Level" = "수준", "Code" = "코드", "Evidence" = "근거", "Guidance" = "안내",
    "Analysis subjects/records" = "분석 대상자/레코드", "Censored records" = "검열 레코드",
    "Censoring proportion" = "검열 비율", "Median potential follow-up (reverse KM)" = "잠재 추적기간 중앙값(역 KM)",
    "Maximum observed time" = "최대 관찰시간", "90th percentile time" = "90백분위 시간",
    "At risk at 90th percentile time" = "90백분위 시점 위험집단 수",
    "Raw value" = "원자료 값", "Role" = "역할", "Label" = "라벨",
    "Source rows" = "원자료 행", "Analysis rows" = "분석 행", "Source subjects" = "원자료 대상자",
    "Analysis subjects" = "분석 대상자", "Events" = "사건", "Competing events" = "경쟁사건", "Censored" = "검열",
    "Method" = "방법", "Distinct event times" = "고유 사건시점 수", "Tied event times" = "동률 사건시점 수",
    "Events at tied times" = "동률 시점 사건 수", "Maximum events at one time" = "단일 시점 최대 사건 수",
    "Proportion of events at tied times" = "동률 시점 사건 비율", "Topic" = "주제",
    "Stratum" = "층", "Records" = "레코드", "Parameters" = "모수 수", "Levels" = "수준 수",
    "Exclusion reason" = "제외 사유", "Review signal" = "검토 신호", "Review status" = "검토 상태",
    "Design column" = "설계행렬 열", "Tolerance" = "허용도", "Review" = "검토",
    "Maximum absolute DFBETA" = "최대 절대 DFBETA", "Maximum absolute DFBETAS" = "최대 절대 DFBETAS",
    "Screening threshold" = "선별 임계값", "Analysis row" = "분석 행",
    "Time function" = "시간 함수", "Interaction B" = "상호작용 B", "Time" = "시간",
    "Event proportion" = "사건 비율",
    "Minimum CIF" = "최소 CIF", "Maximum CIF" = "최대 CIF",
    "Maximum sum of cause-specific CIFs" = "원인별 CIF 합의 최댓값",
    "All CIF values finite" = "모든 CIF 값이 유한함", "CIF values within [0,1]" = "CIF 값이 [0,1] 범위 내",
    "Cause-specific CIF sum <= 1" = "원인별 CIF 합이 1 이하", "Integrity passed" = "무결성 검사 통과",
    "Spline df" = "스플라인 자유도", "Reference value" = "기준값",
    "Nonlinearity LR chi-square" = "비선형성 우도비 카이제곱",
    "Linear-model AIC" = "선형 모형 AIC", "Spline-model AIC" = "스플라인 모형 AIC",
    "Unique target-failure times" = "고유 관심 사건시점 수", "Exploratory p" = "탐색적 p",
    "Holm-adjusted p" = "Holm 보정 p", "Censoring stratum" = "검열 층",
    "Log likelihood" = "로그우도", "Maximum absolute score" = "최대 절대 score",
    "Maximum iterations" = "최대 반복 횟수", "Parameters" = "모수 수",
    "Relative score criterion" = "상대 score 기준", "Convergence tolerance" = "수렴 허용오차",
    "Information rank" = "정보행렬 계수", "Standardized information condition number" = "표준화 정보행렬 조건수",
    "Finite covariance matrix" = "유한 공분산행렬", "Positive standard errors" = "양의 표준오차",
    "Censoring distribution" = "검열분포", "Event proportion" = "사건 비율", "Review reason" = "검토 사유"
  )
  matched <- names(localized) %in% names(header_labels)
  names(localized)[matched] <- unname(header_labels[names(localized)[matched]])
  for (column in names(localized)) {
    if (!is.character(localized[[column]]) && !is.factor(localized[[column]])) next
    values <- as.character(localized[[column]])
    cell_labels <- c(
      "high" = "높음", "review" = "검토", "info" = "정보",
      "Review" = "검토", "No strong signal" = "뚜렷한 신호 없음",
      "Martingale" = "마팅게일", "Deviance" = "편차",
      "Review possible time-varying subdistribution effect" = "시간가변 부분분포 효과 가능성 검토",
      "No monotonic time-pattern signal in this exploratory screen" = "탐색적 선별에서 단조 시간패턴 신호 없음",
      "None" = "없음", "N/A" = "해당 없음", "Model-based" = "모형 기반",
      "Subject-cluster robust" = "대상자 군집 강건", "Complete-case Cox analysis sample" = "Cox 완전사례 분석표본",
      "Companion proportional-hazards model before time-varying extension" = "시간가변 확장 전 동반 비례위험모형",
      "Not estimable" = "추정 불가", "Estimable" = "추정 가능",
      "No events in this group-cause cell" = "이 집단·원인 조합에 사건 없음",
      "Fewer than 5 events in this group-cause cell" = "이 집단·원인 조합의 사건 수가 5건 미만",
      "event_of_interest" = "관심 사건", "competing_event" = "경쟁사건", "censored" = "검열",
      "Time origin" = "시간 원점", "Time unit" = "시간 단위", "Entry variable" = "진입 시간 변수",
      "Interest events" = "관심 사건 수", "Fine-Gray censoring strata" = "Fine-Gray 검열분포 층",
      "Interval start" = "구간 시작", "Interval stop" = "구간 종료", "Subject ID" = "대상자 ID",
      "Stratification variable" = "층화 변수", "Cluster variable" = "군집 변수", "Clusters" = "군집 수",
      "Variance" = "분산", "Source rows" = "원자료 행", "Analysis rows" = "분석 행",
      "Excluded rows" = "제외 행", "Events" = "사건", "Parameters" = "모수 수",
      "Events / parameter" = "모수당 사건 수", "Ties" = "동률 처리",
      "LR chi-square (df)" = "우도비 카이제곱(df)", "LR p" = "우도비 p",
      "Concordance (95% CI)" = "일치도(95% 신뢰구간)", "Package" = "패키지"
    )
    matched_values <- values %in% names(cell_labels)
    values[matched_values] <- unname(cell_labels[values[matched_values]])
    values <- sub(" \\(time-varying HR; see curve\\)$", " (시간가변 HR; 곡선 참조)", values)
    values <- sub(" \\(natural cubic spline; see curve\\)$", " (자연 삼차 스플라인; 곡선 참조)", values)
    separately <- startsWith(values, "Estimated separately within ")
    values[separately] <- paste0("층별로 추정: ", substring(values[separately], nchar("Estimated separately within ") + 1L))
    pooled <- startsWith(values, "One pooled censoring distribution")
    values[pooled] <- sub("^One pooled censoring distribution", "하나의 통합 검열분포", values[pooled])
    localized[[column]] <- values
  }
  attr(localized, "result_table_role") <- "appendix"
  attr(localized, "result_table_language") <- "ko"
  preserve_named_evidence(result_appendix_preserve_data(localized, table))
}

survival_stability_evidence_text <- function(code, evidence, language = statedu_initial_language()) {
  if (identical(code, "few_events_in_stratum")) {
    # Match the final generated suffix; the stratum name may contain the same text.
    parts <- regmatches(evidence, regexec("(?s)^(.*): events = ([0-9]+)$", evidence, perl = TRUE))[[1]]
    if (length(parts) == 3L) return(sprintf(statedu_localized_text(language, "%s: events = %s", "%s: 사건 수 = %s"), parts[2], parts[3]))
    return(evidence)
  }
  if (identical(code, "fine_gray_score_criterion_not_met") && !normalize_app_language(language) %in% c("en", "ko") && startsWith(evidence, "Fine-Gray relative score criterion = ")) {
    values <- strsplit(substring(evidence, nchar("Fine-Gray relative score criterion = ") + 1L), "; tolerance = ", fixed = TRUE)[[1]]
    if (length(values) == 2L) return(sprintf(statedu_localized_text(language, "Fine-Gray relative score criterion = %s; tolerance = %s"), values[1], values[2]))
  }
  sparse_prefixes <- c(few_interest_events = "Interest events = ", few_competing_events = "Competing events = ", zero_group_cause_events = "Zero-event group-cause cells:", sparse_group_cause_events = "Group-cause cells with fewer than 5 events:", low_interest_events_per_covariate = "Interest events/covariate = ")
  sparse_prefixes[["gray_test_not_estimable"]] <- "Non-estimable Gray tests for cause codes:"
  sparse_prefixes[["low_events_per_parameter"]] <- "Events/parameter = "
  sparse_prefixes[["high_vif"]] <- "Maximum design-column VIF = "
  sparse_prefixes[["elevated_vif"]] <- "Maximum design-column VIF = "
  sparse_prefixes[["high_condition_number"]] <- "Design condition number = "
  sparse_prefixes[["dfbetas_influence_signal"]] <- "Influential coefficient rows = "
  sparse_prefixes[["zero_event_stratum"]] <- "No events in strata:"
  sparse_prefixes[["sparse_event_stratum"]] <- "Fewer than 5 events in strata:"
  sparse_prefixes[["adjusted_survival_bootstrap_insufficient"]] <- "Valid adjusted-survival bootstrap replicates = "
  sparse_prefixes[["adjusted_survival_bootstrap_attrition"]] <- "Valid adjusted-survival bootstrap ratio = "
  sparse_prefixes[["breslow_with_substantial_ties"]] <- "Events at tied times = "
  sparse_prefixes[["categorical_joint_test_not_estimable"]] <- "Non-estimable categorical joint tests:"
  sparse_prefixes[["fine_gray_categorical_joint_test_not_estimable"]] <- "Non-estimable Fine-Gray categorical joint tests:"
  sparse_prefixes[["sparse_tail_risk_set"]] <- "At risk near follow-up tail = "
  sparse_prefixes[["high_censoring_proportion"]] <- "Censoring proportion = "
  sparse_prefixes[["crossing_survival_curves"]] <- "Crossing survival curves:"
  sparse_prefixes[["few_robust_variance_clusters"]] <- "Robust-variance clusters = "
  sparse_prefixes[["cause_specific_ph_signal"]] <- "Cause-specific Schoenfeld review signals:"
  sparse_prefixes[["cause_specific_dfbetas_signal"]] <- "Cause-specific influential coefficient rows = "
  sparse_prefixes[["cause_specific_high_vif"]] <- "Maximum cause-specific Cox VIF = "
  sparse_prefixes[["cause_specific_elevated_vif"]] <- "Maximum cause-specific Cox VIF = "
  sparse_prefixes[["cause_specific_high_condition_number"]] <- "Cause-specific Cox design condition number = "
  sparse_prefixes[["fine_gray_high_vif"]] <- "Maximum Fine-Gray VIF = "
  sparse_prefixes[["fine_gray_elevated_vif"]] <- "Maximum Fine-Gray VIF = "
  sparse_prefixes[["fine_gray_high_condition_number"]] <- "Fine-Gray design condition number = "
  sparse_prefixes[["fine_gray_rank_deficient_information"]] <- "Fine-Gray information rank = "
  sparse_prefixes[["fine_gray_ill_conditioned_information"]] <- "Standardized Fine-Gray information condition number = "
  sparse_prefixes[["fine_gray_residual_diagnostic_limited"]] <- "Unique target-failure times available for residual review = "
  sparse_prefixes[["fine_gray_residual_time_pattern"]] <- "Schoenfeld-like residual time-pattern signals:"
  sparse_prefixes[["fine_gray_sparse_censoring_stratum"]] <- "Sparse censoring-distribution strata:"
  sparse_prefixes[["cause_specific_categorical_joint_test_not_estimable"]] <- "Non-estimable cause-specific categorical joint tests:"
  if (!normalize_app_language(language) %in% c("en", "ko") && code %in% names(sparse_prefixes)) {
    prefix <- unname(sparse_prefixes[[code]])
    if (startsWith(as.character(evidence %||% ""), prefix)) return(paste0(statedu_localized_text(language, paste("Survival sparse evidence", trimws(prefix))), if (endsWith(prefix, " ")) " " else "", substring(evidence, nchar(prefix) + 1L)))
  }
  if (identical(code, "cif_integrity_failure") && startsWith(as.character(evidence %||% ""), "CIF integrity failed for groups:")) {
    prefix <- statedu_localized_text(language, "CIF integrity failed for groups:", "CIF 무결성 검사를 통과하지 못한 집단:")
    return(paste0(prefix, substring(evidence, nchar("CIF integrity failed for groups:") + 1L)))
  }
  if (!identical(normalize_app_language(language), "ko")) return(as.character(evidence %||% ""))
  evidence <- as.character(evidence %||% "")
  translate_prefix <- function(english, korean) {
    if (startsWith(evidence, english)) paste0(korean, substring(evidence, nchar(english) + 1L)) else evidence
  }
  prefixes <- c(
    low_events_per_parameter = "Events/parameter = |사건 수/모수 = ",
    few_robust_variance_clusters = "Robust-variance clusters = |강건분산 군집 수 = ",
    high_vif = "Maximum design-column VIF = |설계행렬 열의 최대 VIF = ",
    elevated_vif = "Maximum design-column VIF = |설계행렬 열의 최대 VIF = ",
    high_condition_number = "Design condition number = |설계행렬 조건수 = ",
    dfbetas_influence_signal = "Influential coefficient rows = |영향력 검토 계수 행 수 = ",
    zero_event_stratum = "No events in strata:|사건이 없는 층:",
    sparse_event_stratum = "Fewer than 5 events in strata:|사건이 5건 미만인 층:",
    adjusted_survival_bootstrap_insufficient = "Valid adjusted-survival bootstrap replicates = |유효 보정 생존 부트스트랩 반복 수 = ",
    adjusted_survival_bootstrap_attrition = "Valid adjusted-survival bootstrap ratio = |유효 보정 생존 부트스트랩 비율 = ",
    breslow_with_substantial_ties = "Events at tied times = |동률 시점 사건 비율 = ",
    categorical_joint_test_not_estimable = "Non-estimable categorical joint tests:|추정할 수 없는 범주형 전체 검정:",
    cif_integrity_failure = "CIF integrity failed for groups:|CIF 무결성 검사를 통과하지 못한 집단:",
    few_interest_events = "Interest events = |관심 사건 수 = ",
    few_competing_events = "Competing events = |경쟁사건 수 = ",
    low_interest_events_per_covariate = "Interest events/covariate = |공변량당 관심 사건 수 = ",
    zero_group_cause_events = "Zero-event group-cause cells:|사건이 없는 집단·원인 셀:",
    sparse_group_cause_events = "Group-cause cells with fewer than 5 events:|사건이 5건 미만인 집단·원인 셀:",
    gray_test_not_estimable = "Non-estimable Gray tests for cause codes:|추정할 수 없는 Gray 검정의 원인 코드:",
    cause_specific_categorical_joint_test_not_estimable = "Non-estimable cause-specific categorical joint tests:|추정할 수 없는 원인별 범주형 전체 검정:",
    cause_specific_ph_signal = "Cause-specific Schoenfeld review signals:|원인별 Schoenfeld 검토 신호:",
    cause_specific_dfbetas_signal = "Cause-specific influential coefficient rows = |원인별 영향력 검토 계수 행 수 = ",
    cause_specific_high_vif = "Maximum cause-specific Cox VIF = |원인별 Cox 최대 VIF = ",
    cause_specific_elevated_vif = "Maximum cause-specific Cox VIF = |원인별 Cox 최대 VIF = ",
    cause_specific_high_condition_number = "Cause-specific Cox design condition number = |원인별 Cox 설계행렬 조건수 = ",
    fine_gray_categorical_joint_test_not_estimable = "Non-estimable Fine-Gray categorical joint tests:|추정할 수 없는 Fine-Gray 범주형 전체 검정:",
    fine_gray_high_vif = "Maximum Fine-Gray VIF = |Fine-Gray 최대 VIF = ",
    fine_gray_elevated_vif = "Maximum Fine-Gray VIF = |Fine-Gray 최대 VIF = ",
    fine_gray_high_condition_number = "Fine-Gray design condition number = |Fine-Gray 설계행렬 조건수 = ",
    fine_gray_score_criterion_not_met = "Fine-Gray relative score criterion = |Fine-Gray 상대 score 기준 = ",
    fine_gray_rank_deficient_information = "Fine-Gray information rank = |Fine-Gray 정보행렬 계수 = ",
    fine_gray_ill_conditioned_information = "Standardized Fine-Gray information condition number = |표준화 Fine-Gray 정보행렬 조건수 = ",
    fine_gray_residual_diagnostic_limited = "Unique target-failure times available for residual review = |잔차 검토에 사용할 수 있는 고유 관심 사건시점 수 = ",
    fine_gray_residual_time_pattern = "Schoenfeld-like residual time-pattern signals:|Schoenfeld 유사 잔차 시간패턴 신호:",
    fine_gray_sparse_censoring_stratum = "Sparse censoring-distribution strata:|희소 검열분포 층:",
    crossing_survival_curves = "Crossing survival curves:|교차하는 생존곡선:",
    sparse_tail_risk_set = "At risk near follow-up tail = |추적관찰 말단 위험집단 수 = ",
    high_censoring_proportion = "Censoring proportion = |검열 비율 = "
  )
  if (code %in% names(prefixes)) {
    pair <- strsplit(unname(prefixes[[code]]), "|", fixed = TRUE)[[1]]
    translated <- translate_prefix(pair[[1]], pair[[2]])
    if (identical(code, "fine_gray_score_criterion_not_met")) {
      translated <- sub("; tolerance = ", "; 허용오차 = ", translated, fixed = TRUE)
    }
    return(translated)
  }
  static <- c(
    nonfinite_coefficient = "하나 이상의 계수가 유한하지 않음.",
    nonfinite_vif = "하나 이상의 설계행렬 열 VIF가 유한하지 않음.",
    cause_specific_cox_not_estimable = "하나 이상의 원인별 Cox 계수 또는 불확실성 추정치가 유한하지 않음.",
    cause_specific_model_test_not_estimable = "하나 이상의 원인별 Cox 전체 검정이 유한하지 않음.",
    cause_specific_nonfinite_vif = "하나 이상의 원인별 Cox 설계행렬 열 VIF가 유한하지 않음.",
    fine_gray_not_converged = "Fine-Gray 수렴 표시가 거짓임.",
    fine_gray_not_estimable = "하나 이상의 Fine-Gray 계수 또는 불확실성 추정치가 유한하지 않음.",
    fine_gray_model_test_not_estimable = "Fine-Gray 유사 우도비 검정이 유한하지 않음.",
    fine_gray_nonfinite_vif = "하나 이상의 Fine-Gray 설계행렬 열 VIF가 유한하지 않음.",
    fine_gray_invalid_covariance = "Fine-Gray 공분산행렬이 유한하지 않거나 대각원소가 양수가 아님."
  )
  if (code %in% names(static)) unname(static[[code]]) else evidence
}

survival_main_note <- function(
  format = NULL,
  abbreviations = NULL,
  estimation = NULL,
  reference = NULL,
...
survival_appendix_title <- function(text, language = NULL) {
  text <- as.character(text %||% "")
  if (!identical(result_appendix_table_language(language), "ko")) return(result_appendix_ui_text(text, language))
  labels <- c(
    "Analysis overview" = "분석 개요",
    "Event-code mapping" = "사건 코드 매핑",
    "Data inclusion audit" = "분석 포함 자료 점검",
    "Excluded rows by reason" = "사유별 제외 행",
    "Data review messages" = "자료 검토 메시지",
    "Number at risk" = "위험집단 수",
    "Categorical reference levels and contrast coding" = "범주형 기준수준 및 대비 코딩",
    "Stratum event counts" = "층별 사건 수",
    "Robust-variance cluster summary" = "강건분산 군집 요약",
    "Tied-event summary" = "동률 사건 요약",
    "Supplementary statistics and diagnostics" = "보조 통계량 및 진단",
    "Design-matrix collinearity review" = "설계행렬 다중공선성 검토",
    "Residual distribution review" = "잔차 분포 검토",
    "Influence review" = "영향력 검토",
    "Marginal adjusted survival" = "주변 보정 생존",
    "Estimand contract" = "추정대상 명세",
    "Group-by-cause event counts" = "집단·원인별 사건 수",
    "CIF integrity checks" = "CIF 무결성 점검",
    "Censoring-distribution estimation" = "검열분포 추정",
    "Fine-Gray numerical stability" = "Fine-Gray 수치 안정성",
    "Fine-Gray design-matrix collinearity review" = "Fine-Gray 설계행렬 다중공선성 검토",
    "Proportional subdistribution hazards review" = "비례 부분분포 위험 가정 검토",
    "Cause-specific proportional hazards review" = "원인별 비례위험 가정 검토",
    "Cause-specific collinearity review" = "원인별 다중공선성 검토",
    "Cause-specific residual distribution" = "원인별 잔차 분포",
    "Cause-specific continuous-covariate functional-form review" = "원인별 연속 공변량 함수형태 검토",
    "Cause-specific influence review" = "원인별 영향력 검토",
    "Survival-curve crossing screen" = "생존곡선 교차 선별",
    "Natural cubic spline functional-form analysis" = "자연 삼차 스플라인 함수형태 분석",
    "Time-varying coefficient analysis" = "시간가변 계수 분석",
    "Time-specific hazard ratios" = "시점별 위험비"
  )
  if (text %in% names(labels)) unname(labels[[text]]) else text
}
...
survival_cox_statistic_note <- function(language = statedu_initial_language()) {
  survival_appendix_note(
    language,
    "B = log hazard coefficient; SE = standard error; z = Wald z statistic; p = p value. This supplementary table provides model statistics underlying the hazard-ratio table.",
    "B = 로그 위험계수, SE = 표준오차, z = Wald z 통계량, p = p값입니다. 이 보조표는 위험비 표의 기초 모형 통계량을 제시합니다."
  )
}

survival_cox_localized_method_sentence <- function(result, language) {
  tr <- function(en) statedu_localized_text(language, en)
  present <- function(x) nzchar(as.character(x %||% "")[[1]])
  origin <- if (present(result$time_origin)) result$time_origin else tr("not specified")
  unit <- if (present(result$time_unit)) result$time_unit else tr("not specified")
  method <- tr(if (present(result$start)) "a time-dependent Cox proportional hazards model with subject-cluster robust standard errors" else if (present(result$entry)) "a Cox proportional hazards model with delayed entry" else "a Cox proportional hazards model")
  if (present(result$strata)) method <- paste(method, sprintf(tr("stratified by %s with stratum-specific baseline hazards"), result$strata))
  if (!present(result$start) && present(result$cluster)) method <- paste(method, sprintf(tr("with sandwich robust standard errors clustered by %s"), result$cluster))
  if (present(result$spline_covariate)) method <- paste(method, sprintf(tr("with %s modeled using a natural cubic spline with %d degrees of freedom"), result$spline_covariate, result$spline_df))
  if (present(result$time_varying_covariate)) method <- paste(method, sprintf(tr("with the coefficient for %s varying over log(1 + time)"), result$time_varying_covariate))
  method <- paste(method, sprintf(tr("using the %s method for tied event times"), result$ties %||% "Efron"))
  sprintf(tr("Time was measured from '%s' in units of '%s', and %s was used."), origin, unit, method)
}

survival_noncox_localized_method_sentence <- function(result, language) {
  tr <- function(en) statedu_localized_text(language, en)
  present <- function(x) nzchar(as.character(x %||% "")[[1]])
  origin <- if (present(result$time_origin)) result$time_origin else tr("not specified")
  unit <- if (present(result$time_unit)) result$time_unit else tr("not specified")
  if (identical(result$type, "competing_risk")) {
    parts <- tr("cumulative incidence functions")
    if (present(result$group)) parts <- c(parts, tr("Gray's test"))
    if (is.list(result$cause_specific)) parts <- c(parts, tr("cause-specific Cox regression"))
    if (is.list(result$fine_gray)) parts <- c(parts, tr("Fine–Gray regression"))
    method <- paste(parts, collapse = ", ")
    if (is.list(result$fine_gray) && present(result$censoring_group)) method <- paste(method, sprintf(tr("with censoring distributions estimated separately within %s"), result$censoring_group))
  } else {
    method <- tr(if (identical(result$analysis_method, "life_table")) "the actuarial life-table method" else if (present(result$entry)) "the Kaplan–Meier method with delayed entry" else "the Kaplan–Meier method")
  }
  sprintf(tr("Time was measured from '%s' in units of '%s', and %s was used."), origin, unit, method)
}

survival_method_sentence <- function(result, language = statedu_initial_language()) {
  if (identical(result$type, "cox") && !normalize_app_language(language) %in% c("en", "ko")) return(survival_cox_localized_method_sentence(result, language))
  if (!normalize_app_language(language) %in% c("en", "ko")) return(survival_noncox_localized_method_sentence(result, language))
...
survival_stability_review <- function(result, language = statedu_initial_language(), followup_fn = survival_followup_diagnostics) {
  ko <- identical(normalize_app_language(language), "ko")
  rows <- list()
  add <- function(level, code, evidence, guidance) {
    rows[[length(rows) + 1L]] <<- data.frame(
      Level = level,
      Code = code,
      Evidence = survival_stability_evidence_text(code, evidence, language),
      Guidance = guidance,
      stringsAsFactors = FALSE
    )
  }
  if (identical(result$type, "cox")) {
    ...
    # Restored results may be rendered before an analysis loads S3 methods.
...
survival_reporting_checklist <- function(result, language = statedu_initial_language()) {
  ...
  if (identical(result$type, "competing_risk") && is.list(result$fine_gray)) rows <- rbind(rows, data.frame(
    Item = if (ko) "Fine–Gray 검열분포" else "Fine-Gray censoring distribution",
    Status = review,
    Evidence = if (nzchar(as.character(result$censoring_group %||% ""))) {
      if (ko) paste0("검열분포를 층별로 추정: ", result$censoring_group) else paste("Separate censoring distributions within", result$censoring_group)
    } else if (ko) {
      "전체 분석표본에서 하나의 검열분포 추정; 독립검열 가정 검토"
    } else {
      "One pooled censoring distribution; review the independent-censoring assumption"
    },
    stringsAsFactors = FALSE
  ))
  lang <- normalize_app_language(language)
  if (!lang %in% c("en", "ko")) {
    rows$Item <- vapply(rows$Item, function(x) statedu_localized_text(lang, x), character(1))
    rows$Status <- vapply(rows$Status, function(x) statedu_localized_text(lang, x), character(1))
    if (map_ok) {
      roles <- vapply(map$Role, function(x) statedu_localized_text(lang, paste("Survival event role", x)), character(1))
      rows$Evidence[3] <- paste(map$`Raw value`, roles, sep = "=", collapse = "; ")
    } else rows$Evidence[3] <- statedu_localized_text(lang, "Check unresolved roles")
    rows$Evidence[4] <- sprintf(statedu_localized_text(lang, "N=%s; excluded=%s"), result$n %||% NA, sum(survival_reporting_exclusions(result)$N %||% 0))
    if (!result$type %in% c("cox", "competing_risk")) rows$Evidence[5] <- statedu_localized_text(lang, "Survival / RMST")
    if (nrow(rows) > 6) for (i in 7:nrow(rows)) {
      rows$Evidence[i] <- if (i == 8 && nzchar(as.character(result$censoring_group %||% ""))) {
        sprintf(statedu_localized_text(lang, "Separate censoring distributions within %s"), result$censoring_group)
      } else statedu_localized_text(lang, rows$Evidence[i])
    }
  }
  # Evidence mixes generated prose with user origin/unit, event codes and names.
  # Translate generated components above, then preserve the assembled cells.
  attr(rows, "result_user_columns") <- c("Item", "Status", "Evidence")
  rows
}
...
survival_interpretation_guide <- function(result, language = statedu_initial_language()) {
  ...
  if (!normalize_app_language(language) %in% c("en", "ko")) {
    topics <- vapply(topics, function(x) statedu_localized_text(language, x), character(1), USE.NAMES = FALSE)
    guidance <- vapply(guidance, function(x) statedu_localized_text(language, x), character(1), USE.NAMES = FALSE)
  }
  table <- data.frame(Topic = topics, Guidance = guidance, stringsAsFactors = FALSE)
  attr(table, "result_user_columns") <- c("Topic", "Guidance")
  table
}

survival_reporting_guidance_panel <- function(result, language = statedu_initial_language()) {
  followup <- NULL
  capture_followup <- function(result) {
    quiet <- TRUE
    value <- withCallingHandlers(survival_followup_diagnostics(result),
      warning = function(w) quiet <<- FALSE,
      message = function(m) quiet <<- FALSE)
    # Reuse only within this panel, retaining repeated diagnostics otherwise.
    if (quiet) followup <<- value
    value
  }
  div(class = "result-section regression-result-panel survival-result-panel",
    h3(statedu_localized_text(language, "Reporting checklist and interpretation guide", "보고 체크리스트와 해석 가이드")),
    h4(statedu_localized_text(language, "Analysis stability review", "분석 안정성 검토")),
    survival_simple_table(survival_stability_review(result, language, followup_fn = capture_followup), table_language = language),
    result_note_div(class = "result-note", statedu_localized_text(language, "Thresholds are heuristic screening rules, not automatic analysis-quality pass/fail criteria.", "임계값은 선별용 경험 규칙이며 분석 품질의 자동 합격·불합격 기준이 아닙니다.")),
    h4(statedu_localized_text(language, "Censoring and follow-up diagnostics", "검열 및 추적관찰 진단")),
    survival_simple_table(followup %||% survival_followup_diagnostics(result), table_language = language),
    result_note_div(class = "result-note", statedu_localized_text(language, "Median potential follow-up uses reverse Kaplan–Meier with censoring as the event. A high censoring proportion is not itself evidence of bias; the censoring mechanism requires separate review.", "잠재 추적기간 중앙값은 검열을 사건으로 둔 역 Kaplan–Meier 방법으로 추정합니다. 높은 검열률 자체는 편향의 증거가 아니며 검열기전의 타당성을 별도로 검토해야 합니다.")),
    survival_simple_table(survival_reporting_checklist(result, language), table_language = language),
    h4(statedu_localized_text(language, "Interpretation guide", "해석 가이드")),
    survival_simple_table(survival_interpretation_guide(result, language), table_language = language),
    result_note_div(class = "result-note", statedu_localized_text(language, "This checklist helps prevent reporting omissions; it does not automatically establish design validity or causality.", "체크리스트는 보고 누락을 줄이기 위한 보조도구이며 연구 설계의 타당성이나 인과성을 자동 판정하지 않습니다."))
  )
}
...
survival_km_overview_table <- function(result, language = statedu_initial_language()) {
  ...
  table$Value[1] <- statedu_localized_text(language, table$Value[1], if (identical(result$analysis_method, "life_table")) "생명표" else "Kaplan-Meier")
  if (is.null(result$entry)) table$Value[4] <- statedu_localized_text(language, "None", "없음")
  if (identical(group_variables, "None") && !nzchar(as.character(result$group %||% ""))) table$Value[8] <- statedu_localized_text(language, "None", "없음")
  attr(table, "result_user_columns") <- "Value"
  table
}
...
survival_km_rate_table <- function(result) {
  ...
  data.frame(
    ...
    Survival = vapply(summary_at$surv, survival_format_number, character(1)),
...
survival_km_crossing_note <- function(language = statedu_initial_language()) {
  survival_appendix_note(language,
...
survival_km_result_panel <- function(result, plot_output_ids = "survival_km_plot", title = NULL, language = statedu_initial_language()) {
  ...
  div(
    ...
    div(class = "result-section regression-result-panel survival-result-panel",
      h3(paste0("1. ", survival_appendix_title("Analysis overview", language))),
      survival_simple_table(survival_km_overview_table(result, language), table_language = language),
      result_note_div(class = "result-note", survival_method_sentence(result, language)),
      h4(survival_appendix_title("Event-code mapping", language)), survival_simple_table(survival_reporting_event_map(result), table_language = language),
      if (nrow(data_flow) > 0) tagList(h4(survival_appendix_title("Data inclusion audit", language)), survival_simple_table(data_flow, table_language = language)),
      if (nrow(exclusions) > 0) tagList(h4(survival_appendix_title("Excluded rows by reason", language)), survival_simple_table(exclusions, table_language = language)),
      if (nrow(issues) > 0) tagList(h4(survival_appendix_title("Data review messages", language)), survival_simple_table(issues, table_language = language))
    ),
    if ("survival_time" %in% output_tables) {
      div(class = "result-section regression-result-panel survival-result-panel",
        h3("2. Kaplan-Meier survival time summary"),
        survival_simple_table(
          survival_km_summary_table(result), table_role = "main", table_language = "en",
          note_line = survival_main_note(
            abbreviations = "M = restricted mean; SE = standard error; CI = confidence interval; NE = not estimable; df = degrees of freedom",
            estimation = "M = restricted mean over the default fitted follow-up horizon; prespecified restricted means are reported in the RMST table"
          )
        ),
        if (nrow(rmst_estimates) > 0) tagList(h4("Restricted mean survival time (RMST)"), survival_simple_table(
          rmst_estimates, table_role = "main", table_language = "en",
          note_line = survival_main_note(abbreviations = "RMST = restricted mean survival time; CI = confidence interval", estimation = "RMST is evaluated at the prespecified restriction time")
        )),
        if (nrow(rmst_contrasts) > 0) tagList(h4("RMST contrasts"), survival_simple_table(
          rmst_contrasts, table_role = "main", table_language = "en",
          note_line = survival_main_note(format = "Differences are second level minus first level", estimation = "tau is the prespecified restriction time")
        ))
      )
    },
    if ("survival_table" %in% output_tables) {
      div(class = "result-section regression-result-panel survival-result-panel",
        ...
        survival_simple_table(
          survival_km_rate_table(result), table_role = "main", table_language = "en",
          note_line = survival_main_note(abbreviations = "CI = confidence interval", estimation = "Estimates are Kaplan-Meier survival probabilities at the prespecified times")
        ),
        if (nrow(risk_table) > 0) tagList(h4(survival_appendix_title("Number at risk", language)), survival_simple_table(risk_table, table_language = language))
      )
    },
    if (identical(result$analysis_method, "life_table") && "survival_table" %in% output_tables) {
      div(class = "result-section regression-result-panel survival-result-panel",
        h3("4. Life table"),
        survival_simple_table(survival_life_table_display(result), table_role = "main", table_language = "en")
      )
    },
    if (!is.null(result$logrank)) {
      div(class = "result-section regression-result-panel survival-result-panel",
        h3(survival_km_test_supplement_title("en")),
        survival_simple_table(
          survival_logrank_table(result), table_role = "main", table_language = "en",
          note_line = survival_main_note(abbreviations = "df = degrees of freedom", estimation = "Two-sided log-rank tests compare the survival functions", symbol = "The test is not an effect-size estimate")
        ),
        if (nrow(crossing_summary) > 0) tagList(
          h4(survival_appendix_title("Survival-curve crossing screen", language)),
          survival_simple_table(crossing_summary, table_language = language),
    ...
    if (is.data.frame(result$posthoc) && nrow(result$posthoc) > 0) {
      div(class = "result-section regression-result-panel survival-result-panel",
        h3(survival_ui_text("Post-hoc pairwise comparison", "en")),
        survival_simple_table(
          survival_km_posthoc_display_table(result), table_role = "main", table_language = "en",
          note_line = survival_main_note(multiplicity = "Pairwise p values use the prespecified multiplicity adjustment")
        )
      )
    },
...
survival_km_results_panel <- function(result, plot_output_ids = "survival_km_plot", language = statedu_initial_language()) {
  ...
  div(
    ...
    div(class = "result-section regression-result-panel survival-result-panel survival-wide-result-panel",
      h3("2. Kaplan-Meier survival time summary"),
      survival_simple_table(
        km_summary, table_role = "main", table_language = "en",
        note_line = survival_main_note(
          abbreviations = "M = restricted mean; SE = standard error; CI = confidence interval; NE = not estimable; df = degrees of freedom",
          estimation = "M = restricted mean over the default fitted follow-up horizon; prespecified restricted means are reported in the RMST table"
        )
    ...
    if ("survival_table" %in% output_tables && nrow(rate_summary) > 0) {
      div(class = "result-section regression-result-panel survival-result-panel survival-wide-result-panel",
        h3("3. Survival probabilities at selected time points"),
        survival_simple_table(
          rate_summary, table_role = "main", table_language = "en",
          note_line = survival_main_note(abbreviations = "CI = confidence interval", estimation = "Estimates are Kaplan-Meier survival probabilities at the prespecified times")
        ),
        if (nrow(risk_summary) > 0) tagList(h4(survival_appendix_title("Number at risk", language)), survival_simple_table(risk_summary, table_language = language))
      )
    },
    if (identical(result$analysis_method, "life_table") && "survival_table" %in% output_tables && nrow(life_summary) > 0) {
      div(class = "result-section regression-result-panel survival-result-panel survival-wide-result-panel",
        h3("4. Life table"),
        survival_simple_table(life_summary, table_role = "main", table_language = "en")
      )
    },
    if (nrow(test_summary) > 0) {
      div(class = "result-section regression-result-panel survival-result-panel survival-wide-result-panel",
        h3(survival_km_test_supplement_title("en")),
        survival_simple_table(
          test_summary, table_role = "main", table_language = "en",
          note_line = survival_main_note(abbreviations = "df = degrees of freedom", estimation = "Two-sided tests compare the survival functions", symbol = "Tests are not effect-size estimates")
        ),
        if (nrow(crossing_summary) > 0) tagList(
          h4(survival_appendix_title("Survival-curve crossing screen", language)),
          survival_simple_table(crossing_summary, table_language = language),
        ...
        if (nrow(posthoc_summary) > 0) tagList(
          h4(survival_ui_text("Post-hoc pairwise comparison", "en")),
          survival_simple_table(
            posthoc_summary, table_role = "main", table_language = "en",
            note_line = survival_main_note(multiplicity = "Pairwise p values use the prespecified multiplicity adjustment")
          )
        )
      )
      ...
      lapply(seq_along(items), function(index) {
        ...
        lapply(seq_len(nrow(plot_specs)), function(plot_index) {
          ...
          div(class = "result-section regression-result-panel survival-result-panel survival-plot-result-panel",
            h3(title),
            plotOutput(item_plot_ids[[plot_index]], height = paste0(survival_km_plot_height(item), "px"))
          )
        })
      })
    )
...
survival_cox_result_html_table <- function(result, language = statedu_initial_language(), note_line = NULL) {
  language <- result_main_table_language(language)
  table <- survival_cox_display_parameter_table(result)
  if (!is.data.frame(table) || nrow(table) == 0) return(NULL)
  is_reference <- as.logical(table$Reference %||% rep(FALSE, nrow(table)))
  ...
  contract_table <- data.frame(
    Variable = variables,
    Level = levels,
    HR = vapply(rows, `[[`, character(1), "hr"),
    LLCI = vapply(rows, `[[`, character(1), "llci"),
    ULCI = vapply(rows, `[[`, character(1), "ulci"),
    p = vapply(rows, `[[`, character(1), "p"),
    check.names = FALSE,
    stringsAsFactors = FALSE
  )
  contract <- result_table_contract(
    ...
    role = "main",
    language = language,
    intrinsic_width = result_table_intrinsic_width(contract_table, first_width = 132, default_width = 62, min_width = 590)
  )
  result_table_with_notes(
    result_table_apply_contract(table_tag, contract),
    result_note_tag(note_line %||% survival_main_note(
      abbreviations = "HR = hazard ratio; CI = confidence interval",
      estimation = c(
        "Two-sided p values and 95% confidence intervals are reported; values in parentheses are p values",
        "The likelihood-ratio test evaluates the overall model; PH GLOBAL evaluates the proportional-hazards assumption"
      ),
      reference = "Reference categories have HR = 1.00"
    ))
  )
}
...
survival_cox_collinearity_note <- function(result, language) {
  survival_table_note(sprintf(statedu_localized_text(language,
    "VIF is computed for each Cox design-matrix column; thresholds are heuristic review signals. Condition number = %s.",
    "VIF는 Cox 설계행렬의 각 열에 대해 계산하며 임계값은 경험적 검토 신호입니다. 조건수 = %s."), survival_format_number(result$condition_number)))
}

survival_competing_collinearity_note <- function(result, language, fine_gray = FALSE) {
  en <- if (fine_gray) "Design-matrix condition number: %s. VIF and condition-number thresholds are heuristic screening rules; they do not select or delete covariates automatically." else "Design-matrix condition number: %s. VIF and condition-number thresholds are heuristic screening rules."
  ko <- if (fine_gray) "설계행렬 조건수: %s. VIF와 조건수 임계값은 경험적 선별 규칙이며 공변량을 자동 선택하거나 삭제하지 않습니다." else "설계행렬 조건수: %s. VIF와 조건수 임계값은 경험적 선별 규칙입니다."
  survival_table_note(sprintf(statedu_localized_text(language, en, ko), survival_format_number(result$condition_number %||% NA_real_)))
}
...
survival_cox_collinearity_table <- function(result) {
  ...
  attr(display, "result_user_columns") <- "Design column"
  display
}
...
survival_cox_ph_plot_height <- function(result, minimum = 620) {
  term_count <- if (is.null(result$ph$y)) 0L else ncol(as.matrix(result$ph$y))
  max(minimum, 240 * ceiling(term_count / 2L))
}
...
survival_adjusted_survival_time_table <- function(result) {
  ...
  data.frame(
    ...
    LLCI = vapply(table$Lower, survival_format_number, character(1)),
    ULCI = vapply(table$Upper, survival_format_number, character(1)),
    check.names = FALSE,
    stringsAsFactors = FALSE
  )
}
...
survival_adjusted_survival_contrast_table <- function(result) {
  ...
  data.frame(
    ...
    `Difference LLCI` = vapply(table$`Difference lower`, survival_format_number, character(1)),
    `Difference ULCI` = vapply(table$`Difference upper`, survival_format_number, character(1)),
    Ratio = vapply(table$Ratio, survival_format_number, character(1)),
    `Ratio LLCI` = vapply(table$`Ratio lower`, survival_format_number, character(1)),
    `Ratio ULCI` = vapply(table$`Ratio upper`, survival_format_number, character(1)),
...
survival_cox_results_panel <- function(result, language = statedu_initial_language()) {
  ...
  div(
    ...
    div(class = "result-section regression-result-panel survival-result-panel",
      h3(paste0("1. ", survival_appendix_title("Analysis overview", language))),
      survival_simple_table(survival_cox_overview_table(result), table_language = language),
      result_note_div(class = "result-note", survival_method_sentence(result, language)),
      h4(survival_appendix_title("Data inclusion audit", language)), survival_simple_table(reporting$data_flow, table_language = language),
      h4(survival_appendix_title("Event-code mapping", language)), survival_simple_table(reporting$event_map, table_language = language),
      if (nrow(result$categorical_reference_table %||% data.frame()) > 0) tagList(
        h4(survival_appendix_title("Categorical reference levels and contrast coding", language)),
        survival_simple_table(result$categorical_reference_table, table_language = language),
        survival_appendix_note(language,
          "Treatment contrasts are fixed inside the analysis. The stated reference level remains the denominator regardless of the session-wide R contrasts option.",
          "처치 대비는 분석 내부에서 고정됩니다. 표시된 기준수준은 세션 전체 R 대비 옵션과 관계없이 분모로 유지됩니다."
        )
      ),
      if (nrow(survival_cox_strata_table(result)) > 0) tagList(
        h4(survival_appendix_title("Stratum event counts", language)),
        survival_simple_table(survival_cox_strata_table(result), table_language = language),
        survival_appendix_note(language,
          "The stratification variable defines separate baseline hazards and therefore has no estimated hazard ratio. Sparse or event-free strata require cautious interpretation.",
          "층화 변수는 층별 기저위험을 정의하므로 위험비를 추정하지 않습니다. 희소하거나 사건이 없는 층은 신중하게 해석해야 합니다."
        )
      ),
      if (nrow(survival_cox_cluster_summary_table(result)) > 0) tagList(
        h4(survival_appendix_title("Robust-variance cluster summary", language)),
        survival_simple_table(survival_cox_cluster_summary_table(result), table_language = language),
        survival_appendix_note(language,
          "Sandwich standard errors account for within-cluster correlation. The coefficient estimates are unchanged from the corresponding partial-likelihood model, but inference depends on the number and independence of clusters.",
          "Sandwich 표준오차는 군집 내 상관을 반영합니다. 계수 추정치는 해당 부분우도 모형과 같지만 추론은 군집 수와 군집 간 독립성에 좌우됩니다."
        )
      ),
      h4(survival_appendix_title("Tied-event summary", language)),
      survival_simple_table(survival_cox_ties_summary_table(result), table_language = language),
      survival_appendix_note(language,
        "The ties method is prespecified and reported with the observed extent of tied failures. It is not selected by searching for the smallest p-value.",
        "동률 처리 방법은 사전에 지정하며 관찰된 동률 사건 규모와 함께 보고합니다. 가장 작은 p값을 찾는 방식으로 선택하지 않습니다."
      ),
      if (nrow(exclusions) > 0) tagList(h4(survival_appendix_title("Excluded rows by reason", language)), survival_simple_table(exclusions, table_language = language))
    ),
    div(class = "result-section regression-result-panel survival-result-panel",
      ...
      if (nrow(survival_cox_categorical_joint_test_table(result)) > 0) tagList(
        ...
        survival_simple_table(
          survival_cox_categorical_joint_test_table(result), table_role = "main", table_language = "en",
          note_line = survival_main_note(
            estimation = "Each Wald chi-square jointly tests all non-reference coefficients for one categorical predictor",
            reference = "Reference levels are shown in the hazard-ratio table"
          )
        )
      )
    ),
    ...
    div(class = "result-section regression-result-panel survival-result-panel",
      h3(paste0("3. ", survival_appendix_title("Supplementary statistics and diagnostics", language))),
      survival_simple_table(survival_cox_statistic_table(result), class = "survival-result-table logistic-result-table", table_language = language),
      survival_cox_statistic_note(language),
      h4(statedu_localized_text(language, "Detailed proportional-hazards diagnostics", "비례위험 가정 상세 진단")),
      survival_simple_table(survival_ph_table(result, language), class = "survival-result-table logistic-result-table", table_language = language),
      survival_appendix_note(language, "Covariate-specific Schoenfeld-residual tests are supplementary diagnostics for possible time-varying effects. The main table reports only the overall likelihood-ratio test and the PH GLOBAL test.", "변수별 Schoenfeld 잔차 검정은 시간가변 효과 가능성을 찾기 위한 보조 진단입니다. 본표에는 전체 모형의 Likelihood-ratio 검정과 PH GLOBAL 검정만 제시합니다."),
      if (nzchar(as.character(result$time_varying_covariate %||% ""))) survival_appendix_note(language,
        "For this time-varying-coefficient analysis, the Schoenfeld tests and plots come from the companion proportional-hazards model fitted before adding tt(). They document the diagnostic motivation; they are not post-extension PH tests.",
        "시간가변 계수 분석의 Schoenfeld 검정과 도표는 tt()를 추가하기 전에 적합한 동반 비례위험모형에서 산출됩니다. 이는 진단 근거를 제시하며 확장 모형 적합 후의 비례위험 검정이 아닙니다."
      ),
      if (!is.null(result$ph) && !is.null(result$ph$y)) tagList(
        plotOutput("survival_cox_ph_plot", height = paste0(survival_cox_ph_plot_height(result), "px")),
        survival_appendix_note(language,
          ...
          "평활화한 척도화 Schoenfeld 잔차도는 시간가변 계수 평가를 보조합니다. 공식 p값과 도표를 함께 해석하며 어느 것도 자동 합격·불합격 기준이 아닙니다."
        )
      ),
      h4(survival_appendix_title("Design-matrix collinearity review", language)),
      survival_simple_table(survival_cox_collinearity_table(result), table_language = language),
      survival_cox_collinearity_note(result, language),
      if (nrow(survival_cox_spline_test_display_table(result)) > 0) tagList(
        h4(survival_appendix_title("Natural cubic spline functional-form analysis", language)),
        survival_simple_table(survival_cox_spline_test_display_table(result), table_language = language),
        plotOutput("survival_cox_spline_plot", height = "500px"),
        survival_appendix_note(language,
          "The likelihood-ratio test compares the prespecified natural-cubic-spline model with the corresponding linear-term model. The HR curve is relative to the sample median and spans the 5th to 95th percentiles. Spline basis coefficients are intentionally omitted because their individual HRs are not substantively interpretable; evaluate the joint test and curve together.",
          "우도비 검정은 사전 지정 자연 삼차 스플라인 모형과 해당 선형항 모형을 비교합니다. HR 곡선은 표본 중앙값을 기준으로 5백분위부터 95백분위까지 제시합니다. 개별 스플라인 기저계수의 HR은 실질적으로 해석하기 어려워 생략하므로 전체 검정과 곡선을 함께 평가하세요."
        )
      ),
      if (nrow(survival_cox_time_varying_test_display_table(result)) > 0) tagList(
        h4(survival_appendix_title("Time-varying coefficient analysis", language)),
        survival_simple_table(survival_cox_time_varying_test_display_table(result), table_language = language),
        h4(survival_appendix_title("Time-specific hazard ratios", language)),
        survival_simple_table(survival_cox_time_specific_hr_table(result), table_language = language),
        plotOutput("survival_cox_time_varying_plot", height = "500px"),
        survival_appendix_note(language,
          "The model is β(t) = β + γ log(1 + time). The interaction test evaluates γ = 0; report the time-specific HR curve and confidence intervals rather than a single constant HR. The plotted range uses the 5th to 95th percentiles of observed event times, plus requested reporting times.",
          "모형은 β(t) = β + γ log(1 + time)입니다. 상호작용 검정은 γ = 0을 평가하며 하나의 고정 HR 대신 시점별 HR 곡선과 신뢰구간을 보고합니다. 도표 범위는 관찰 사건시점의 5~95백분위와 요청한 보고 시점을 사용합니다."
        )
      )
    ),
    div(class = "result-section regression-result-panel survival-result-panel",
      h3(paste0("4. ", survival_appendix_title("Residual distribution review", language))),
      survival_simple_table(survival_cox_residual_table(result), table_language = language),
      survival_appendix_note(language,
      ...
      if (is.data.frame(result$functional_form_data) && nrow(result$functional_form_data) > 0) tagList(
        ...
        survival_appendix_note(language,
          ...
          "각 연속 공변량에 대한 Martingale 잔차의 loess 평활선은 함수형태 선별 도표입니다. 체계적인 곡률이 보이면 사전 지정 변환이나 스플라인을 고려하고 실질적 결론을 비교하세요."
        )
      )
    ),
    div(class = "result-section regression-result-panel survival-result-panel",
      h3(paste0("5. ", survival_appendix_title("Influence review", language))),
      survival_simple_table(survival_cox_influence_table(result), table_language = language),
      survival_appendix_note(language,
        "Standardized DFBETAS are compared with the heuristic 2/sqrt(N) screening threshold. Signals identify observations for sensitivity review; observations are not deleted automatically.",
        "표준화 DFBETAS는 경험적 2/sqrt(N) 선별 임계값과 비교합니다. 신호는 민감도 검토 대상을 표시하며 관측치를 자동으로 삭제하지 않습니다."
      )
    ),
    if (is.list(result$adjusted_survival)) div(class = "result-section regression-result-panel survival-result-panel",
      h3(paste0("6. ", survival_appendix_title("Marginal adjusted survival", language))),
      survival_simple_table(survival_adjusted_survival_overview_table(result), table_language = language),
      survival_appendix_note(language,
        "Each group curve averages model-based survival over the complete-case analysis sample's observed covariate distribution. It is not a curve for one mean-covariate subject.",
        "각 집단 곡선은 완전사례 분석표본의 관찰 공변량 분포에 대해 모형 기반 생존을 평균합니다. 평균 공변량을 가진 한 대상자의 곡선이 아닙니다."
      ),
      if (nrow(survival_adjusted_survival_time_table(result)) > 0) tagList(
        h4("Adjusted survival at specified times"),
        survival_simple_table(
          survival_adjusted_survival_time_table(result), table_role = "main", table_language = "en",
          note_line = survival_main_note(abbreviations = "CI = confidence interval", estimation = "Marginal survival is standardized over the complete-case covariate distribution")
        )
      ),
      if (nrow(survival_adjusted_survival_contrast_table(result)) > 0) tagList(
        h4("Pairwise adjusted-survival contrasts"),
        survival_simple_table(
          survival_adjusted_survival_contrast_table(result), table_role = "main", table_language = "en",
          note_line = survival_main_note(
            format = "Difference is second level minus first level; ratio is second level divided by first level",
            estimation = "Confidence intervals are pointwise percentile-bootstrap intervals"
          )
        )
      ),
...
survival_competing_overview_table <- function(result, language = statedu_initial_language()) {
  ...
  if (!nzchar(result$group)) table$Value[8] <- statedu_localized_text(language, "None", "없음")
  if (!nzchar(as.character(result$censoring_group %||% ""))) table$Value[9] <- statedu_localized_text(language, "Pooled", "통합")
  table$Value[10] <- statedu_localized_text(language, table$Value[10], if (nzchar(result$group)) "누적발생함수 / Gray 검정" else "누적발생함수")
  attr(table, "result_user_columns") <- "Value"
  table
}
...
survival_censoring_group_display_table <- function(result, language = statedu_initial_language()) {
  ...
  if (!nzchar(as.character(result$censoring_group %||% ""))) table$`Censoring stratum` <- statedu_localized_text(language, "All analysis records", "전체 분석 관측치")
  reasons <- c("Fewer than 10 analysis records" = "분석 관측치 10개 미만", "No censored observations" = "검열 관측치 없음", "Fewer than 5 censored observations" = "검열 관측치 5개 미만")
  table$`Review reason` <- vapply(strsplit(table$`Review reason`, "; ", fixed = TRUE), function(parts) paste(vapply(parts, function(x) if(x %in% names(reasons)) statedu_localized_text(language, x, reasons[[x]]) else x, character(1)), collapse = "; "), character(1))
  table$`Review signal` <- ifelse(table$`Review signal`, statedu_localized_text(language, "Yes", "예"), statedu_localized_text(language, "No", "아니요"))
  attr(table, "result_user_columns") <- c("Censoring stratum", "Review reason", "Review signal")
  table
}

survival_censoring_group_note <- function(result, language) {
  if (nzchar(as.character(result$censoring_group %||% ""))) return(survival_table_note(sprintf(statedu_localized_text(language,
    "The censoring distribution was estimated separately within %s using crr(cengroup=...). This variable should be prespecified from the design, not selected from observed p-values.",
    "검열분포는 crr(cengroup=...)을 사용하여 %s의 층별로 추정했습니다. 이 변수는 관찰 p값으로 선택하지 말고 연구설계에서 사전 지정해야 합니다."), result$censoring_group)))
  survival_appendix_note(language,
    "One censoring distribution was estimated across the analysis sample. Fine-Gray validity requires independent censoring conditional on the modeled structure; consider prespecified cengroup strata when censoring distributions differ across known groups.",
    "분석표본 전체에서 하나의 검열분포를 추정했습니다. Fine-Gray 타당성은 모형 구조를 조건으로 한 독립검열을 요구하므로 알려진 집단 간 검열분포가 다르면 사전 지정 cengroup 층을 고려하세요.")
}
...
survival_cif_integrity_display_table <- function(result, language = statedu_initial_language()) {
  table <- result$cif_integrity
  if (!is.data.frame(table) || !nrow(table)) return(data.frame())
  for (name in c("Minimum CIF", "Maximum CIF", "Maximum sum of cause-specific CIFs")) {
    table[[name]] <- vapply(table[[name]], survival_format_number, character(1))
  }
  for (name in c("All CIF values finite", "CIF values within [0,1]", "Cause-specific CIF sum <= 1", "Integrity passed")) {
    table[[name]] <- ifelse(is.na(table[[name]]), NA_character_, ifelse(table[[name]], statedu_localized_text(language, "Yes", "예"), statedu_localized_text(language, "No", "아니요")))
  }
  table
}
...
survival_competing_risk_table_plot <- function(result) {
  ...
  rows <- do.call(rbind, lapply(groups, function(group) {
    ...
    followup <- if (identical(class(subset), "data.frame")) unclass(subset)[[result$time]] else NULL
    counts <- if (length(times) >= 32L && length(followup) >= 1000L &&
                  is.numeric(followup) && !is.object(followup) && is.null(dim(followup)) &&
                  all(is.finite(times)) && all(is.finite(followup))) {
      # Count strictly earlier times; tied observations remain at risk.
      as.integer(length(followup) - findInterval(times, sort(followup), left.open = TRUE))
    } else vapply(times, function(value) sum(subset[[result$time]] >= value, na.rm = TRUE), integer(1))
    data.frame(Time = times, Strata = group, N = counts, stringsAsFactors = FALSE)
  }))
...
survival_km_plot_height <- function(result, minimum = 650) {
  groups <- length(unique(as.character(result$summary_at$strata %||% "All")))
  minimum + max(0L, groups - 2L) * 36
}

survival_draw_plot_with_risk_table <- local({
  draw <- function(main_plot, risk_plot, newpage = TRUE) {
    if (is.null(risk_plot)) return(print(main_plot))
    # Both panels must map a given follow-up time to the same horizontal position.
    time_range <- ggplot2::ggplot_build(main_plot)$layout$panel_params[[1]]$x.range
    main_plot <- main_plot + ggplot2::coord_cartesian(xlim = time_range, expand = c(left = FALSE, right = FALSE))
    risk_plot <- risk_plot + ggplot2::coord_cartesian(xlim = time_range, expand = c(left = FALSE, right = FALSE))
    ...
    # Physical text size does not shrink with a narrow/high-DPI Shiny device.
    # Reserve room for every risk row and wrap the caption at the device width.
    device_height <- grid::convertHeight(grid::unit(1, "npc"), "pt", valueOnly = TRUE)
    caption_text <- paste(caption, collapse = "\n")
    caption_height <- 0
    if (has_caption) {
      grid::pushViewport(grid::viewport(gp = grid::gpar(fontsize = 8.5)))
      available_width <- grid::convertWidth(grid::unit(1, "npc"), "pt", valueOnly = TRUE) - 18
      lines <- unlist(lapply(strsplit(caption_text, "\n", fixed = TRUE)[[1]], function(paragraph) {
        words <- strsplit(paragraph, " ", fixed = TRUE)[[1]]
        wrapped <- character(0)
        line <- ""
        for (word in words) {
          candidate <- if (nzchar(line)) paste(line, word) else word
          width <- grid::convertWidth(grid::stringWidth(candidate), "pt", valueOnly = TRUE)
          if (nzchar(line) && width > available_width) {
      ...
      caption_height <- max(device_height * 0.34 / 5.54, length(lines) * 11 + 8)
    }
    risk_rows <- length(unique(as.character(risk_plot$data$Strata)))
    fixed_risk_height <- sum(grid::convertHeight(risk_grob$heights, "pt", valueOnly = TRUE))
    risk_height <- max(device_height * 1.12 / if (has_caption) 5.54 else 5.2,
                       fixed_risk_height + max(1L, risk_rows) * 14)
    heights <- grid::unit.c(grid::unit(1, "null"), grid::unit(risk_height, "pt"))
    if (has_caption) heights <- grid::unit.c(heights, grid::unit(caption_height, "pt"))
    layout <- grid::grid.layout(length(heights), 1, heights = heights)
  ...
  function(main_plot, risk_plot, newpage = TRUE) {
    # Avoid first-draw JIT work; always restore the caller's compiler setting.
    previous_jit <- compiler::enableJIT(0)
    on.exit(invisible(compiler::enableJIT(previous_jit)), add = TRUE)
    draw(main_plot, risk_plot, newpage)
  }
})
...
survival_competing_results_panel <- function(result, plot_output_id = "survival_competing_plot", language = statedu_initial_language()) {
  ...
  div(
    ...
    div(class = "result-section regression-result-panel survival-result-panel",
      h3(paste0("1. ", survival_appendix_title("Analysis overview", language))),
      survival_simple_table(survival_competing_overview_table(result, language), table_language = language),
      result_note_div(class = "result-note", survival_method_sentence(result, language)),
      h4(survival_appendix_title("Data inclusion audit", language)), survival_simple_table(reporting$data_flow, table_language = language),
      h4(survival_appendix_title("Event-code mapping", language)),
      survival_simple_table(reporting$event_map, table_language = language),
      if (nrow(result$categorical_reference_table %||% data.frame()) > 0) tagList(
        h4(survival_appendix_title("Categorical reference levels and contrast coding", language)),
        survival_simple_table(result$categorical_reference_table, table_language = language),
        survival_appendix_note(language,
          "Treatment contrasts are fixed inside cause-specific Cox and Fine-Gray regression. The stated reference level remains the denominator regardless of the session-wide R contrasts option.",
          "처치 대비는 원인별 Cox 및 Fine-Gray 회귀 내부에서 고정됩니다. 표시된 기준수준은 세션 전체 R 대비 옵션과 관계없이 분모로 유지됩니다."
        )
      ),
      h4(survival_appendix_title("Estimand contract", language)),
      survival_simple_table(result$estimand_table %||% data.frame(), table_language = language),
      h4(survival_appendix_title("Group-by-cause event counts", language)),
      survival_simple_table(survival_competing_event_count_display_table(result), table_language = language),
      h4(survival_appendix_title("CIF integrity checks", language)),
      survival_simple_table(survival_cif_integrity_display_table(result, language), table_language = language),
      if (nrow(reporting$exclusions)) tagList(
        h4(survival_appendix_title("Excluded rows by reason", language)),
        survival_simple_table(reporting$exclusions, table_language = language)
      )
    ),
    div(class = "result-section regression-result-panel survival-result-panel",
      h3("2. Cumulative incidence at selected time points"),
      survival_simple_table(
        survival_competing_rate_table(result), table_role = "main", table_language = "en",
        note_line = survival_main_note(abbreviations = "CIF = cumulative incidence function; CI = confidence interval", estimation = "CIF accounts for competing events; 1-KM is not used")
      )
    ),
    if (nrow(survival_gray_display_table(result)) > 0) div(class = "result-section regression-result-panel survival-result-panel",
      h3("3. Gray test"),
      survival_simple_table(
        survival_gray_display_table(result), table_role = "main", table_language = "en",
        note_line = survival_main_note(estimation = "Gray's test compares cumulative incidence functions across groups", symbol = "The test is not an effect-size estimate")
      )
    ),
    if (is.list(result$cause_specific)) div(class = "result-section regression-result-panel survival-result-panel",
      ...
      survival_simple_table(
        survival_cause_specific_coef_table(result), table_role = "main", table_language = "en",
        note_line = survival_main_note(
          abbreviations = "HR = cause-specific hazard ratio; CI = confidence interval",
          estimation = "Competing events leave the risk set at occurrence",
          reference = "Reference categories have HR = 1",
          symbol = "The raw engine coefficient table is retained in the audit export"
        )
      ),
      h4("Cause-specific overall model tests"),
      survival_simple_table(
        survival_cox_model_test_table(result$cause_specific), table_role = "main", table_language = "en",
        note_line = survival_main_note(estimation = "Likelihood-ratio, Wald, and score tests evaluate the joint coefficient null", symbol = "These are not model-fit indices")
      ),
      if (nrow(survival_cox_categorical_joint_test_table(result$cause_specific)) > 0) tagList(
        ...
        survival_simple_table(
          survival_cox_categorical_joint_test_table(result$cause_specific), table_role = "main", table_language = "en",
          note_line = survival_main_note(estimation = "Wald tests jointly evaluate the non-reference coefficients for each categorical predictor")
        )
      ),
      h4(survival_appendix_title("Cause-specific proportional hazards review", language)),
      survival_simple_table(survival_cause_specific_ph_table(result), table_language = language),
      if (!is.null(result$cause_specific$ph)) plotOutput("survival_cause_specific_ph_plot", height = paste0(survival_cox_ph_plot_height(result$cause_specific), "px")),
      survival_appendix_note(language,
        "Schoenfeld results and residual plots are review signals, not an automatic proportional-hazards pass/fail decision.",
        "Schoenfeld 결과와 잔차도는 검토 신호이며 비례위험 가정의 자동 합격·불합격 판정이 아닙니다."
      ),
      h4(survival_appendix_title("Cause-specific collinearity review", language)),
      survival_simple_table(survival_cox_collinearity_table(result$cause_specific), table_language = language),
      survival_competing_collinearity_note(result$cause_specific, language),
      h4(survival_appendix_title("Cause-specific residual distribution", language)),
      survival_simple_table(survival_cox_residual_table(result$cause_specific), table_language = language),
      if (is.data.frame(result$cause_specific$functional_form_data) && nrow(result$cause_specific$functional_form_data)) tagList(
        h4(survival_appendix_title("Cause-specific continuous-covariate functional-form review", language)),
        plotOutput("survival_cause_specific_functional_plot", height = "500px"),
        survival_appendix_note(language,
          "The Martingale-residual smoother is a descriptive functional-form diagnostic; it does not select a transformation automatically.",
          "Martingale 잔차 평활선은 기술적 함수형태 진단이며 변환을 자동 선택하지 않습니다."
        )
      ),
      h4(survival_appendix_title("Cause-specific influence review", language)),
      survival_simple_table(survival_cox_influence_table(result$cause_specific), table_language = language),
      survival_appendix_note(language,
        "Standardized DFBETAS above 2/sqrt(n) are observations for sensitivity review, not automatic deletion rules.",
        "표준화 DFBETAS가 2/sqrt(n)를 넘는 관측치는 민감도 검토 대상이며 자동 삭제 기준이 아닙니다."
      )
    ),
    if (is.list(result$fine_gray)) div(class = "result-section regression-result-panel survival-result-panel",
      ...
      survival_simple_table(
        survival_fine_gray_coef_table(result), table_role = "main", table_language = "en",
        note_line = survival_main_note(
          abbreviations = "sHR = subdistribution hazard ratio; CIF = cumulative incidence function; CI = confidence interval",
          reference = "Reference categories have sHR = 1",
          symbol = c("sHR is not labeled HR and is not an individual risk ratio", "The raw engine coefficient table is retained in the audit export")
        )
      ),
      h4("Fine-Gray overall model test"),
      survival_simple_table(
        survival_cox_model_test_table(result$fine_gray), table_role = "main", table_language = "en",
        note_line = survival_main_note(estimation = "The pseudo likelihood-ratio test evaluates the joint coefficient null", symbol = "It is not a conventional likelihood fit index")
      ),
      if (nrow(survival_cox_categorical_joint_test_table(result$fine_gray)) > 0) tagList(
        ...
        survival_simple_table(
          survival_cox_categorical_joint_test_table(result$fine_gray), table_role = "main", table_language = "en",
          note_line = survival_main_note(estimation = "Model-based Wald tests jointly evaluate the non-reference coefficients for each categorical predictor")
        )
      ),
      h4(survival_appendix_title("Censoring-distribution estimation", language)),
      survival_simple_table(survival_censoring_group_display_table(result, language), table_language = language),
      survival_censoring_group_note(result, language),
      h4(survival_appendix_title("Fine-Gray numerical stability", language)),
      survival_simple_table(survival_fine_gray_optimizer_table(result), table_language = language),
      h4(survival_appendix_title("Fine-Gray design-matrix collinearity review", language)),
      survival_simple_table(survival_cox_collinearity_table(result$fine_gray), table_language = language),
      survival_competing_collinearity_note(result$fine_gray, language, fine_gray = TRUE),
      h4(survival_appendix_title("Proportional subdistribution hazards review", language)),
      survival_simple_table(survival_fine_gray_residual_table(result), table_language = language),
      ...
      survival_appendix_note(language,
        "The crr Schoenfeld-like residual plot is the primary descriptive lack-of-fit review. Spearman correlations and Holm-adjusted p-values are exploratory monotonic-trend screens, not a formal cox.zph-equivalent test and not an automatic assumption pass/fail decision.",
...
survival_saved_plot_specs <- function(result, km_plot_ids = NULL, language = statedu_initial_language()) {
  ...
  if (result$type %in% c("km", "km_multi")) {
    ...
    for (item_index in seq_along(items)) {
      ...
      for (plot_index in seq_len(nrow(plot_specs))) {
        local({
          ...
          }, height = survival_km_plot_height(current_item, minimum = 720))
        })
      }
    }
    ...
    if (!is.null(result$ph) && !is.null(result$ph$y)) add("survival_cox_ph_plot", "Scaled Schoenfeld-residual plots", function() survival_cox_ph_plot(result), height = survival_cox_ph_plot_height(result, 760))
    ...
    add("survival_competing_plot", "Cumulative incidence curves", function() {
      survival_draw_plot_with_risk_table(survival_competing_ggplot(result), survival_competing_risk_table_plot(result), newpage = FALSE)
    })
    if (is.list(result$cause_specific) && !is.null(result$cause_specific$ph)) add("survival_cause_specific_ph_plot", "Cause-specific scaled Schoenfeld-residual plots", function() survival_cox_ph_plot(result$cause_specific), height = survival_cox_ph_plot_height(result$cause_specific, 760))
...
survival_inline_saved_plots <- function(panel, plot_specs) {
  ...
  for (spec in plot_specs) {
    ...
    # Each spec draws on a fresh PNG device; survival specs can skip clearing it.
...
write_survival_results_html <- function(result, file, language = statedu_initial_language()) {
  write_result_html_document(saved_survival_results_html(result, language), file, useBytes = TRUE, language = language)
  invisible(file)
}
...
{
    save_screen_excel_file(saved_survival_results_html(result = result, language = language), file)
}
