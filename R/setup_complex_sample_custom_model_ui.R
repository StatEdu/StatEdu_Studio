...
complex_sample_run_custom_model <- function(data, snapshot, design_spec, confidence = 0.95) {
  ...
  list(overview = overview, syntax = syntax, coefficients = coefficient_table, effects = effects, fits = fits, graph = graph,
       design = built, design_note = complex_sample_design_note(built, role = "main", language = "en"),
...
complex_sample_custom_model_result_ui <- function(result, language = statedu_initial_language()) {
  ...
  appendix_table <- function(table) result_appendix_localize_table(table, ui_language)
  main_table <- function(table) {
  ...
  coefficient_note <- result_sci_note_text(
    ...
    estimation = "Path coefficients use survey-weighted linear regression; indirect and conditional effects use replicate-weight covariance estimation",
    reference = "Term coding and effect definitions follow the analysis syntax"
  )
  main_table_fn <- function(table) coefficient_html_table(
    table,
    table_role = "main",
    table_language = "en",
    note_line = coefficient_note
  )
  design_note <- complex_sample_design_note(result$design, role = "appendix", language = ui_language)
  syntax <- result$syntax
  attr(syntax, "result_user_columns") <- "Syntax"
  equation_rows <- which(syntax$Type == "Survey regression")
  attr(syntax, "result_user_cells") <- cbind(equation_rows, rep(match("Equation", names(syntax)), length(equation_rows)))
  tagList(
    analysis_result_table_section(custom_model_canvas_text(ui_language, "Model overview", "모형 개요"), appendix_table(result$overview), table_fn = model_overview_html_table),
    analysis_result_table_section(custom_model_canvas_text(ui_language, "Analysis syntax", "분석 구문"), appendix_table(syntax), table_fn = model_overview_html_table),
    analysis_result_table_section("Survey-weighted path coefficients", main_table(result$coefficients), table_fn = main_table_fn),
    analysis_result_table_section("Direct, indirect, and conditional effects", main_table(result$effects), table_fn = main_table_fn),
    complex_sample_appendix_diagnostics_section(
      items = "Survey design",
      details = design_note,
      language = ui_language,
      title_en = "Complex-sample design",
      title_ko = "복합표본 설계"
    )
  )
}
...
register_complex_sample_custom_model_handlers <- function(
  ...
  execute_command_model <- function(current_snapshot, scope_dispatch = TRUE) {
    force(current_snapshot)
    if (scope_dispatch) return(analysis_scope_run(session, "complex_custom_model_canvas",
      function() execute_command_model(current_snapshot, FALSE), "complex_custom_model_canvas_results", "complex-custom-model-canvas-root"))
    current_snapshot <- analysis_scope_model_snapshot(current_snapshot,dataset_fn())
    analysis <- tryCatch(
  ...
  observeEvent(input$complex_custom_model_canvas_run_confirm, execute_command_model(input$complex_custom_model_canvas_run_confirm), ignoreInit = TRUE)
  register_canvas_analysis_commands(
    input, output, session, "complex_custom_model_canvas_state", "complex-custom-model-canvas-root",
    ...
    run_fn = execute_command_model, extra_states = list(design = design_state)
  )
}
