...
structural_canvas_cfa_bootstrap_total <- function(args) {
  ...
  htmt_total <- as.integer(args$htmt_bootstrap %||% 0L)
  if (htmt_total > 0L) {
    if (identical(structural_canvas_bootstrap_ci_method(args$htmt_ci_method), "bca")) htmt_total <- htmt_total + nrow(args$data)
    higher <- structural_canvas_higher_htmt_data(args$fit, args$data)
    if (isTRUE(higher$available)) htmt_total <- htmt_total + 2L * (as.integer(args$htmt_bootstrap) +
      if (identical(structural_canvas_bootstrap_ci_method(args$htmt_ci_method), "bca")) higher$n else 0L)
  }
  sum(c(reliability_total, as.integer(args$bollen_stine_bootstrap %||% 0L), htmt_total), na.rm = TRUE)
}
...
structural_canvas_cfa_bootstrap_job_value <- function(args, progress_file) {
  ...
  if (args$htmt_bootstrap > 0L) {
    ...
    if (length(factors) >= 2L) {
      ...
      output$htmt_bootstrap_result <- structural_canvas_htmt_bootstrap_with_higher(
        fit, data, indicators, reps = args$htmt_bootstrap, confidence = .95,
