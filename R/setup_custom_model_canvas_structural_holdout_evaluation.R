...
structural_canvas_holdout_model_comparison <- function(original_syntax, modified_syntax, validation_data, estimator = "MLR", missing = "fiml", std_lv = FALSE, ci_level = .90, ml_likelihood = "normal") {
  ...
  list(table = table, changes = changes, fits = fits, difference = difference,
    admissibility_reasons = lapply(admissibility, function(item) item$reasons %||% character(0)),
    validation_n_raw = nrow(validation_data), validation_n_used = used_n, estimator = estimator)
}
