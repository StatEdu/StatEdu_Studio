...
structural_canvas_run_htmt_bootstrap <- function(analysis_type, htmt_bootstrap, result, data, htmt_seed, ordered, htmt_threshold, htmt_ci_method) {
  ...
  if (analysis_type %in% c("cfa", "cbsem", "sem") && htmt_bootstrap > 0L) {
    ...
    if (length(factor_names_for_htmt) >= 2L) {
      ...
      htmt_bootstrap_result <- structural_canvas_with_progress(
        message = "Estimating HTMT bootstrap confidence intervals",
        value = 0,
        {
          structural_canvas_inc_progress(.1, detail = paste0(htmt_bootstrap, " case-resampling replicates"))
          value <- structural_canvas_htmt_bootstrap_with_higher(
            result$fit, data, indicators_for_htmt, reps = htmt_bootstrap, confidence = .95,
          ...
          structural_canvas_inc_progress(.9, detail = "Preparing interval estimates")
          value
        }
      )
    }
