...
structural_canvas_reliability_estimates <- function(fit, formula_mode = "standardized") {
  # Only est.std is consumed here. Keep the original inference path for
  # exceptional fits (especially bootstrap CIs, which can emit warnings).
  estimates_only <- tryCatch({
    inherits(fit, "lavaan") &&
      identical(as.character(fit@version[[1L]]), "0.7-2") &&
      identical(as.character(utils::packageVersion("lavaan")), "0.7.2") &&
      isTRUE(fit@optim$converged) && fit@Data@ngroups == 1L &&
      fit@Data@nlevels == 1L &&
      fit@Options$se %in% c("standard", "robust.huber.white", "robust.sem", "none") &&
      !isTRUE(fit@Options$se.delta.second.order) &&
      !any(fit@ParTable$op %in% c(":=", "==", "<", ">")) &&
      !isTRUE(fit@Model@eq.constraints) &&
      is.matrix(fit@vcov$vcov) && all(is.finite(fit@vcov$vcov)) &&
      all(vapply(fit@Model@GLIST[names(fit@Model@GLIST) %in% c("theta", "psi")],
        function(x) all(is.finite(diag(x)) & diag(x) > 0), logical(1)))
  }, error = function(error) FALSE)
  standardized <- if (isTRUE(estimates_only)) {
    lavaan::standardizedSolution(fit, se = FALSE, zstat = FALSE, pvalue = FALSE, ci = FALSE)
  } else {
  ...
  # Reuse only quiet accessor results, within this fit and invocation. Lazy
  # evaluation retains the original order and repeats diagnostic-emitting calls.
  model_values <- list()
  reuse_model_value <- function(name, value) {
    if (!is.null(model_values[[name]])) return(model_values[[name]])
    quiet <- TRUE
    result <- withCallingHandlers(value,
      warning = function(w) quiet <<- FALSE,
      message = function(m) quiet <<- FALSE
    )
    if (quiet) model_values[[name]] <<- result
    result
  }
  rows <- lapply(factors, function(factor) {
    ...
    if (identical(formula_mode, "model_implied")) {
      parameters <- reuse_model_value("parameters", lavaan::parameterEstimates(fit))
      raw <- parameters$est[parameters$op == "=~" & parameters$lhs == factor & parameters$rhs %in% indicators]
      latent_variance <- reuse_model_value("latent_variances", diag(as.matrix(lavaan::lavInspect(fit, "cov.lv"))))[[factor]]
