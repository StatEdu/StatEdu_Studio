logistic_cached_null <- function(cache, family, data, dependent, fit) {
  if (!is.environment(cache)) return(fit())
  y <- data[[dependent]]
  rows <- row.names(data)
  entry <- cache[[family]]
  if (!is.null(entry) && identical(entry$y, y, num.eq = FALSE) && identical(entry$rows, rows)) return(entry$model)
  quiet <- TRUE
  model <- withCallingHandlers(fit(), warning = function(w) quiet <<- FALSE, message = function(m) quiet <<- FALSE)
  if (quiet) cache[[family]] <- list(y = y, rows = rows, model = model)
  model
}
...
compact_analysis_blocks <- function(block1, block2 = character(0), block3 = character(0), block4 = NULL) {
  blocks <- list(block1, block2, block3)
  if (!is.null(block4)) blocks <- c(blocks, list(block4))
  count <- length(blocks)
  blocks <- lapply(blocks, function(x) unique(as.character(x %||% character(0))))
  compacted <- Filter(function(block) length(block) > 0, blocks)
  compacted <- c(compacted, rep(list(character(0)), count - length(compacted)))
  stats::setNames(compacted, paste0("block", seq_len(count)))
}
...
logistic_vif_summary <- function(formula, data) {
  ...
  logistic_vif_maximum(vif)
}

logistic_vif_maximum <- function(vif) {
...
logistic_vif_by_predictor <- function(formula, data, predictors) {
  ...
  vif <- coefficient_collinearity(mm)$vif
  logistic_vif_predictor_values(vif, predictors)
}

logistic_vif_predictor_values <- function(vif, predictors) {
  predictors <- as.character(predictors %||% character(0))
  values <- stats::setNames(rep(NA_real_, length(predictors)), predictors)
...
logistic_vif_diagnostics <- function(formula, data, predictors) {
  # Share only within this model step. Preserve the old second call when the
  # first calculation warns, so singular/degenerate fits retain their warnings.
  warned <- FALSE
  vif <- withCallingHandlers({
    mm <- tryCatch(stats::model.matrix(formula, data = data), error = function(e) NULL)
    if (is.null(mm)) numeric(0) else coefficient_collinearity(mm)$vif
  }, warning = function(w) warned <<- TRUE)
  list(
    maximum = logistic_vif_maximum(vif),
    by_predictor = if (warned) logistic_vif_by_predictor(formula, data, predictors) else logistic_vif_predictor_values(vif, predictors)
  )
}
...
logistic_cumulative_probability <- function(probabilities) {
  # Keep each row's cumsum order while avoiding apply's large matrix rearrangement.
  if (identical(class(probabilities), c("matrix", "array")) &&
      is.double(probabilities) && nrow(probabilities) >= 50000L &&
      ncol(probabilities) >= 20L) {
    values <- t(vapply(seq_len(nrow(probabilities)), function(index) {
      cumsum(probabilities[index, ])
    }, numeric(ncol(probabilities))))
    dimnames(values) <- dimnames(probabilities)
    return(values)
  }
  t(apply(probabilities, 1L, cumsum))
}
...
logistic_apparent_performance <- function(model, data, dependent, method) {
  ...
  if (identical(method, "Ordinal logistic regression")) {
    cumulative_probability <- logistic_cumulative_probability(probabilities)[, -length(levels), drop = FALSE]
...
fit_logistic_model <- function(data, dependent, predictors, measurement, ordinal_mode = c("auto", "ordinal", "multinomial"), parallel = NULL, null_cache = NULL) {
  ordinal_mode <- match.arg(ordinal_mode)
  model_data <- data
  form <- make_formula(dependent, predictors)
  null_formula <- stats::reformulate("1", response = dependent)
  if (identical(measurement, "binary")) {
    # Tighten IRLS convergence so the covariance (and Wald tests) stabilizes too.
    model <- stats::glm(form, data = model_data, family = stats::binomial(), control = stats::glm.control(epsilon = 1e-14, maxit = 100))
    null_model <- logistic_cached_null(null_cache, "binary", model_data, dependent, function() stats::glm(null_formula, data = model_data, family = stats::binomial(), control = stats::glm.control(epsilon = 1e-14, maxit = 100)))
  ...
  if (identical(measurement, "ordered")) {
    if (identical(ordinal_mode, "multinomial")) {
      ...
      multi_null <- logistic_cached_null(null_cache, "multinomial", model_data, dependent, function() nnet::multinom(null_formula, data = model_data, trace = FALSE))
    ...
    ordinal_null <- logistic_cached_null(null_cache, "ordinal", model_data, dependent, function() ordinal::clm(null_formula, data = model_data, link = "logit", Hess = TRUE))
  ...
  null_model <- logistic_cached_null(null_cache, "multinomial", model_data, dependent, function() nnet::multinom(null_formula, data = model_data, trace = FALSE))
  model$call$formula <- form
  null_model$call$formula <- null_formula
  model$call$data <- model_data
  null_model$call$data <- model_data
...
prepare_logistic_analysis_results <- function(
  ...
  if (length(attr(data, "statedu_scope_excluded"))) analysis_scope_prepare_variables(data, environment(), c("dependents", "block1", "block2", "block3"))
  ...
  for (dependent in dependents) {
    ...
    for (step_index in seq_along(steps)) {
      ...
      fit <- tryCatch(
        fit_logistic_model(prep$data, dependent, predictors, analysis_measurement, ordinal_mode = ordinal_mode, parallel = ordinal_parallel, null_cache = null_cache),
        error = function(e) e
      )
      ...
      vif_diagnostics <- logistic_vif_diagnostics(make_formula(dependent, predictors), prep$data, predictors)
      max_vif <- vif_diagnostics$maximum
      predictor_vif <- vif_diagnostics$by_predictor
