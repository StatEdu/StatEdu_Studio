...
output[[paste0(prefix, "_result_validity_note")]] <- renderUI({
  ...
  tagList(
    tags$p(class = "structural-result-note", if (ko) "괄호 안 대각값은 sqrt(AVE), 하삼각 값은 잠재상관입니다. Fornell–Larcker 비교가 기준 아래여도 판별타당도의 확립을 뜻하지 않으며 HTMT 구간, 교차적재, 국소적합, 이론과 경쟁 측정모형을 함께 검토해야 합니다." else "Diagonal values are sqrt(AVE) and lower-triangle values are latent correlations. A Fornell-Larcker comparison below its reference does not establish discriminant validity; review HTMT intervals, cross-loadings, local fit, theory, and competing measurement models."),
    ...
    if (length(auto_single_residuals)) tags$p(class = "structural-result-note", paste0(if (ko) "주의: 단일지표 요인 지표(" else "Caution: single-indicator factor indicators (", paste(auto_single_residuals, collapse = ", "), if (ko) ")는 오차분산 0으로 자동 고정되어 완전측정 가정으로 식별되었습니다." else ") were automatically identified by fixing their residual variances to zero, which is a perfect-measurement assumption.")),
