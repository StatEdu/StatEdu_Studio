...
output[[paste0(prefix, "_result_validity_note")]] <- renderUI({
  ...
  tagList(
    ...
    tags$p(class = "structural-result-note", if (ko) "본표는 투고용 요약을 위해 AVE, CR, Cronbach's alpha, McDonald's omega total 중심으로 표시합니다. 지표 수, Fornell-Larcker 판정 문구, 세부 guidance는 본표에서 제외했습니다." else "For manuscript reporting, this table focuses on AVE, CR, Cronbach's alpha, and McDonald's omega total. Indicator count, Fornell-Larcker criterion labels, and detailed guidance are omitted from the main table."),
    tags$p(class = "structural-result-note", if (ko) "AVE ≥ .50, CR/Cronbach's alpha/omega total ≥ .70은 자주 쓰는 기술적 기준이지만 보편적 합격/불합격 규칙은 아니며 구성개념 범위, 문항 수, 모형 허용성, 연구 목적과 함께 해석해야 합니다." else "Common descriptive guidelines include AVE ≥ .50 and CR/Cronbach's alpha/omega total ≥ .70. These are heuristics rather than universal pass/fail rules and should be interpreted with construct breadth, item count, model admissibility, and study purpose."),
