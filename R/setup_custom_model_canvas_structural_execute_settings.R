structural_canvas_execute_settings <- function(settings, input, prefix) {
  ...
  supports_invariance_paths <- prefix %in% c("structural_cbsem", "structural_sem", "structural_plssem")
  invariance_path_scope <- if (supports_invariance_paths) {
    structural_canvas_invariance_path_scope(
      settings$invariance_path_scope %||% input[[paste0(prefix, "_invariance_path_scope")]] %||% "all"
    )
  } else {
    "all"
  }
  invariance_selected_path_ids <- if (supports_invariance_paths) {
    structural_canvas_invariance_selected_path_ids(
      settings$invariance_selected_path_ids %||% input[[paste0(prefix, "_invariance_selected_path_ids")]] %||% character(0)
    )
  } else {
    character(0)
  }
  ...
  micom_permutations <- suppressWarnings(as.integer(settings$micom_permutations %||% input[[paste0(prefix, "_micom_permutations")]] %||% 5000L))
  if (is.na(micom_permutations) || !micom_permutations %in% c(5000L, 10000L, 20000L, 50000L)) micom_permutations <- 5000L
  ...
  list(
    ...
    estimator_recommendation_confirmed = isTRUE(settings$estimator_recommendation_confirmed %||% input[[paste0(prefix, "_estimator_recommendation_confirmed")]] %||% identical(prefix, "structural_plssem")),
    ...
    # CFA, SEM/CB-SEM, and PLS-SEM are single-level independent
    # cross-sectional routes. Dependent-data SEM belongs in future dedicated
    # multilevel, complex-survey, or longitudinal analysis menus.
    ...
    moderation_method = {
      pls_route <- identical(prefix, "structural_plssem")
      default <- if (pls_route) "two_stage" else "all_pairs_dmc"
      value <- as.character(settings$moderation_method %||% input[[paste0(prefix, "_moderation_method")]] %||% default)
      allowed <- if (pls_route) {
        c("two_stage", "product_indicator", "orthogonal")
      } else {
        c("all_pairs_dmc", "matched_pair_dmc", "all_pairs_mean_centered")
      }
    ...
    # PLS/PLSc has one explicit missing-data contract. Keep legacy saved
    # settings from silently reporting lavaan's FIML/listwise labels.
    missing = if (identical(prefix, "structural_plssem")) {
      "mean_replacement"
    } else {
      settings$missing %||% input[[paste0(prefix, "_missing")]] %||% "fiml"
    },
