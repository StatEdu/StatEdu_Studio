...
structural_canvas_reliability_bootstrap_chunk_size <- function(value = NULL, reps, workers) {
  ...
  if (is.null(value)) {
    value <- if (workers <= 1L) {
      ...
      # The fixed-index pool sends only compact integer matrices. Batches of up
      # to 250 amortize Windows cluster scheduling while still updating a
      # 5,000-draw progress display twenty times.
      max(workers * 4L, min(250L, reps))
    }
  }
...
# The production CFA bootstrap runs in an isolated callr process. Install the
# immutable data/model context once in each reusable PSOCK worker, then send
# only compact row-index blocks for subsequent chunks. This avoids repeatedly
# serializing hundreds of resampled data frames through lavaanList on Windows.
structural_canvas_reliability_bootstrap_worker_install_context <- function(context) {
  assign(".statedu_cfa_reliability_fixed_index_context", context, envir = .GlobalEnv)
  list(installed = TRUE, rows = nrow(context$data), columns = ncol(context$data))
}

structural_canvas_reliability_bootstrap_worker_cleanup <- function() {
  if (exists(
    ".statedu_cfa_reliability_fixed_index_context",
    envir = .GlobalEnv, inherits = FALSE
  )) {
    rm(".statedu_cfa_reliability_fixed_index_context", envir = .GlobalEnv)
  }
  if (exists(
    ".statedu_cfa_lavaan_metadata_fast_path_state",
    envir = .GlobalEnv, inherits = FALSE
  )) {
    state <- get(
      ".statedu_cfa_lavaan_metadata_fast_path_state",
...
structural_canvas_reliability_bootstrap_fixed_index_worker <- function(block) {
  context <- get(
    ".statedu_cfa_reliability_fixed_index_context",
    envir = .GlobalEnv, inherits = FALSE
  )
  if (!is.matrix(block$indices) || ncol(block$indices) != length(block$positions)) {
    return(list(
      positions = block$positions, items = NULL, failed = TRUE,
      error = "CFA fixed-index block is malformed"
    ))
  }
  ...
  for (column in seq_len(ncol(block$indices))) {
    ...
    if (inherits(candidate, "error") || !inherits(candidate, "lavaan")) {
      items[[column]] <- list(status = "unknown", value = NULL)
      next
    }
    screening <- tryCatch(
      context$extract_fit(candidate, character(0), list(), context$model_df),
      error = function(error) error
    )
    items[[column]] <- context$classify_result(screening, function() {
      context$reliability_fast(candidate, context$formula_mode)
    })
  }
...
structural_canvas_reliability_bootstrap <- function(syntax, data, reps = 500L, confidence = .95, seed = default_seed(), estimator = "ML", missing = "fiml", std_lv = FALSE, ordered = character(0), formula_mode = "standardized", original_fit = NULL, ci_method = "bias_corrected", progress = NULL, cancel = NULL, ml_likelihood = "normal", workers = NULL, chunk_size = NULL, return_draws = FALSE) {
  ...
  fit_fixed_indices <- function(positions, frame_indices) {
    if (!fixed_index_active || !length(positions) ||
        length(frame_indices) != length(positions)) return(NULL)
    block_count <- min(length(positions), max(selected_workers, selected_workers * 4L))
    ...
    blocks <- lapply(local_positions, function(local_index) {
      list(
        positions = as.integer(positions[local_index]),
        indices = do.call(cbind, frame_indices[local_index])
      )
    })
    pieces <- tryCatch(
      parallel::clusterApplyLB(
        cluster, blocks, structural_canvas_reliability_bootstrap_fixed_index_worker
      ),
      error = function(error) NULL
    )
  ...
  fit_frames <- function(frames, first_position, positions = NULL, frame_indices = NULL) {
    ...
    values <- NULL
    if (fixed_index_active && length(positions) && is.list(frame_indices)) {
      fixed_started <- Sys.time()
      values <- fit_fixed_indices(positions, frame_indices)
      fixed_index_seconds <<- fixed_index_seconds + as.numeric(difftime(
        Sys.time(), fixed_started, units = "secs"
      ))
      fixed_index_blocks <<- fixed_index_blocks + 1L
      if (is.null(values)) fixed_index_fallbacks <<- fixed_index_fallbacks + 1L
    }
    if (is.null(values) && use_isolated_fast_screen && selected_workers > 1L) {
      values <- fit_fast_frames(frames, first_position)
    }
    if (is.null(values)) {
  ...
  if (use_isolated_fast_screen && selected_workers > 1L && reps > 1L) {
    ...
    on.exit({
      try(parallel::clusterCall(
        cluster, structural_canvas_reliability_bootstrap_worker_cleanup
      ), silent = TRUE)
    ...
    all_worker_fast_paths_applied <-
      length(cluster_metadata_fast_path) == selected_workers &&
    ...
    fixed_index_supported <- all_worker_fast_paths_applied &&
      identical(template_version, "0.7-2") && identical(template_se, "standard") &&
      identical(template_estimator, "ML") && !template_categorical &&
      ...
      !isTRUE(getOption("statedu.internal.disable_cfa_reliability_fixed_index", FALSE))
    if (fixed_index_supported) {
      ...
      fixed_index_worker_context <- tryCatch(
        parallel::clusterCall(
          cluster, structural_canvas_reliability_bootstrap_worker_install_context,
          fixed_context
        ),
        error = function(error) list()
      )
      fixed_index_active <- length(fixed_index_worker_context) == selected_workers &&
  ...
  for (positions in make_chunks(reps, first = 2L)) {
    ...
    values <- fit_frames(
      frames, positions[[1L]], positions = positions,
      frame_indices = sample_indices[positions]
    )
  ...
  if (identical(ci_method, "bca")) {
    ...
    for (positions in make_chunks(n)) {
      ...
      jackknife_values[positions] <- fit_frames(
        frames, reps + positions[[1L]], positions = positions,
        frame_indices = lapply(positions, function(index) seq_len(n)[-index])
      )
