...
structural_file_icon <- function(kind) {
  ...
  if (kind %in% c("open", "load")) {
    return(svg(
      tags$path(d = "M3 7.5V6a2 2 0 0 1 2-2h5l2 2h6a2 2 0 0 1 2 2v1.5"),
  ...
  if (identical(kind, "resultOpen")) {
    return(svg(
      tags$path(d = "M3 7.5V6a2 2 0 0 1 2-2h5l2 2h6a2 2 0 0 1 2 2v1.5"),
      tags$path(d = "M3.5 9.5h17l-2.2 9H5.7z"),
      tags$path(d = "M8 16v-2M12 16v-4M16 16v-3")
    ))
  }
  if (identical(kind, "resultSave")) {
    return(svg(
      tags$path(d = "M4 3.5h10l4 4V20H4z"),
      tags$path(d = "M14 3.5v4h4"),
      tags$path(d = "M7 15v-3M10 15V9M13 15v-2"),
      tags$path(d = "M18 12v7"),
      tags$path(d = "m15.5 16.5 2.5 2.5 2.5-2.5")
    ))
  }
  svg(
