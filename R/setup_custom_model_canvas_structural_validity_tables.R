...
structural_canvas_validity_result_table <- function(kind, bundle, snapshot, fit, ko, fmt, display_name) {
  if (!identical(kind, "validity")) return(NULL)
  ...
  for (row in seq_along(latent_names)) {
    ...
    table[row, 1L] <- display_name(latent_name)
    for (column in seq_along(latent_names)) {
      if (row == column) {
        table[row, column + 1L] <- if (single_indicator && !constrained_single) {
          "(N/A‡)"
        } else if (!is.finite(ave_value) || ave_value < 0) {
          "(N/A†)"
        } else {
          paste0("(", format_decimal3(sqrt(ave_value)), if (ave_value > 1) "†" else if (constrained_single) "¶" else "", ")")
        }
      }
    ...
    table[row, ncol(table) - 1L] <- if (single_indicator) {
      if (constrained_single) "N/A¶" else "N/A‡"
    } else {
      paste0(format_decimal3(alpha_value), alpha_marker)
    }
