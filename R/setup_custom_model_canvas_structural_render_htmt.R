...
structural_canvas_register_htmt_outputs <- function(output, prefix, fit_result, result_table = NULL, app_language_fn = NULL) {
  render_htmt_tables <- function(include_details = FALSE) {
    ...
    if (is.null(sample_covariance)) {
      return(tags$p(class = "structural-result-note", if (ko) "다집단 표본통계에 대한 HTMT는 현재 결과 화면에 표시하지 않습니다." else "HTMT is not currently displayed for multigroup sample statistics."))
    }
    ...
    tagList(
      ...
      tags$p(class = "structural-result-note", if (ko) "HTMT는 문항 상관의 절댓값을 사용합니다. 선택 기준보다 낮은 값은 'Below reference'로 표시되며 판별타당도 합격을 뜻하지 않습니다." else "HTMT uses absolute item correlations. Values below the selected threshold are marked 'Below reference', which is not a standalone discriminant-validity pass."),
      ...
      if (bootstrap_reps > 0L && is.null(bootstrap_table) && isTRUE(bundle$cfa_bootstrap_pending)) tags$p(class = "structural-result-note", if (ko) "HTMT 부트스트랩을 백그라운드에서 계산하고 있습니다. 완료되면 이 결과표가 자동으로 갱신됩니다." else "HTMT bootstrap intervals are being computed in the background. This result table will update automatically when complete."),
      if (bootstrap_reps > 0L && is.null(bootstrap_table) && bootstrap_canceled) tags$p(class = "structural-result-note", if (ko) "HTMT 부트스트랩이 사용자 요청으로 중단되었습니다. 점추정값과 기본 분석 결과는 유지됩니다." else "The HTMT bootstrap was stopped by the user. Point estimates and base-model results remain available."),
      if (bootstrap_reps > 0L && is.null(bootstrap_table) && !isTRUE(bundle$cfa_bootstrap_pending) && !bootstrap_canceled) tags$p(class = "structural-result-note", if (ko) "선택된 지표로부터 HTMT 부트스트랩 신뢰구간을 계산하지 못했습니다." else "HTMT bootstrap confidence intervals could not be estimated from the selected indicators."),
      if (bootstrap_incomplete) tags$p(class = "structural-result-note", if (ko) "일부 부트스트랩 표본은 허용 가능한 상관행렬을 만들지 못했습니다. 순서형 범주가 없거나 희소한 경우가 흔한 원인입니다. 유효 반복 수가 줄어든 구간은 신중하게 해석하십시오." else "Some bootstrap resamples could not produce an admissible correlation matrix, commonly because an ordered category was absent or sparse. Interpret intervals with reduced valid-replicate counts cautiously."),
      ...
      if (bootstrap_caution) tags$p(class = "structural-result-note", if (ko) "HTMT 부트스트랩 상태 Caution은 요청 표본의 50% 이상 80% 미만만 유효했다는 뜻입니다. 구간은 불안정할 수 있으므로 유효 반복 수를 함께 보고하십시오." else "HTMT bootstrap status Caution means that 50% to less than 80% of requested resamples were valid. Treat the interval as unstable and report the valid-replicate count."),
      if (bootstrap_unreliable) tags$p(class = "structural-result-note", if (ko) "HTMT 부트스트랩 상태 Unreliable은 요청 표본의 50% 미만만 유효했다는 뜻입니다. 신뢰한계와 기준 판단은 평가하지 않으며, 변별타당도 근거로 사용하면 안 됩니다." else "HTMT bootstrap status Unreliable means that fewer than 50% of requested resamples were valid. Confidence limits and threshold decisions are not assessed and should not be used as discriminant-validity evidence."),
      if (!is.null(bootstrap_table)) tags$p(class = "structural-result-note", paste0(
        if (ko) {
          paste0(htmt_ci_label, " 구간은 사례 재표집", if (identical(htmt_ci_method, "bca")) "과 leave-one-out 잭나이프 가속도" else "", if (length(bundle$ordered %||% character(0))) " 및 각 표본에서 재추정한 polychoric 상관" else "", "에 기반합니다. 'Upper < threshold'는 선택한 .85/.90 기준에 대한 단측 95% 상한을 사용합니다. 'Upper < 1'은 양측 95% 구간이 1을 제외하는지 나타냅니다. 이 판단은 점추정 기준과 분리해 보고합니다.")
        } else {
          paste0("The ", htmt_ci_label, " interval is based on case resampling", if (identical(htmt_ci_method, "bca")) " with leave-one-out jackknife acceleration" else "", if (length(bundle$ordered %||% character(0))) " with polychoric correlations re-estimated in each resample" else "", ". 'Upper < threshold' uses the one-sided 95% upper confidence limit for the selected .85/.90 criterion. 'Upper < 1' indicates whether the two-sided 95% interval excludes 1. These are intentionally reported separately from the point-estimate criterion.")
        }
      ))
    )
  }
