...
structural_canvas_register_htmt_outputs <- function(output, prefix, fit_result, result_table = NULL, app_language_fn = NULL) {
  render_htmt_tables <- function(include_details = FALSE) {
    ...
    tagList(
      ...
      if (bootstrap_reps > 0L && is.null(bootstrap_table) && !isTRUE(bundle$cfa_bootstrap_pending)) tags$p(class = "structural-result-note", if (ko) "선택된 지표로부터 HTMT 부트스트랩 신뢰구간을 계산하지 못했습니다." else "HTMT bootstrap confidence intervals could not be estimated from the selected indicators."),
