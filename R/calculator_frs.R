...
frs_result <- function(data, selected) {
  ...
  result <- source
  result[["FRScore"]] <- score
  result[["CVD10"]] <- cvd10
  result[["G.CVD10"]] <- g_cvd10
  result[["Heartage"]] <- frs_heart_age(score, sex)
  result
}
...
frs_calculator_tab_panel <- function() {
  tabPanel(
    ...
    div(
      class = "page-shell",
      div(class = "app-heading", h1("Framingham Risk Score Calculator"), div("Select variables and add FRScore, CVD10, G.CVD10, and Heartage to the current data.", class = "app-subtitle")),
      div(
...
frs_setup_ui <- function(file, data, variable_info, input) {
  ...
  div(
    ...
    div(class = "analysis-transfer-column analysis-transfer-panel metabolic-target-panel", analysis_field_label_tag("FRS variables"), div(class = "metabolic-variable-input-grid", variable_inputs)),
    div(class = "analysis-options-column analysis-options-panel metabolic-reference-panel", div(class = "analysis-option-title", "Outputs"), tags$table(class = "hint8-initial-table", tags$tbody(tags$tr(tags$td("Score"), tags$td("FRScore")), tags$tr(tags$td("10-year risk"), tags$td("CVD10")), tags$tr(tags$td("Risk group"), tags$td("G.CVD10")), tags$tr(tags$td("Heart age"), tags$td("Heartage")))))
  )
}
...
register_frs_calculator_handlers <- function(input, output, session, dataset_fn, current_data_file_fn, variable_info_fn, add_calculated_variable_fn) {
  ...
  result <- eventReactive(input$run_frs_calculator, {
    ...
    tryCatch({
      result_data <- frs_result(dataset_fn(), frs_selected_variables(input))
      add_calculated_variable_fn("FRScore", result_data[["FRScore"]], var_label = "Framingham Risk Score", measurement = "continuous")
      add_calculated_variable_fn("CVD10", result_data[["CVD10"]], var_label = "Cardiovascular disease 10 year Risk", measurement = "continuous")
      add_calculated_variable_fn("G.CVD10", result_data[["G.CVD10"]], var_label = "CVD 10 year Risk Grouping", measurement = "category")
      add_calculated_variable_fn("Heartage", result_data[["Heartage"]], var_label = "Heart age", measurement = "continuous")
      showNotification("FRScore, CVD10, G.CVD10, and Heartage were added to the current data.", type = "message", duration = 5)
      result_data
  ...
  output$frs_calculator_preview <- DT::renderDT({
    ...
    selected <- frs_selected_variables(input)
    preview_names <- intersect(c(unname(selected), "FRScore", "CVD10", "G.CVD10", "Heartage"), names(data))
  ...
  output$download_frs_calculator <- downloadHandler(
    ...
    content = function(file) {
      data <- result()
      if (is.null(data)) data <- frs_result(dataset_fn(), frs_selected_variables(input))
