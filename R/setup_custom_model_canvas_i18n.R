...
custom_model_canvas_translation_key <- function(en) {
  keys <- c(
    ...
    "Export" = "export",
    "Format" = "format",
    "Invalid model canvas file." = "invalid_json",
...
custom_model_canvas_builtin_text <- function(language, key) {
  ...
  values <- c(
    ...
    export = "\u66f8\u304d\u51fa\u3057",
    format = "\u5f62\u5f0f",
    invalid_json = "\u30e2\u30c7\u30eb\u30ad\u30e3\u30f3\u30d0\u30b9\u30d5\u30a1\u30a4\u30eb\u304c\u6b63\u3057\u304f\u3042\u308a\u307e\u305b\u3093\u3002",
...
custom_model_canvas_text <- function(language, en, ko) {
  ...
  if (!identical(language, "ko")) analysis_ui_text(en, language) else fallback
}

custom_model_canvas_title <- function(language = statedu_initial_language()) {
  statedu_t("custom_model_canvas.title", language, "Mediation / Moderation Effects")
}

custom_model_canvas_i18n <- function(language = statedu_initial_language()) {
  list(
    path_properties = custom_model_canvas_text(language, "Path properties", "경로 속성"),
    display_label = custom_model_canvas_text(language, "Display label", "표시 라벨"),
    select_canvas_variable = custom_model_canvas_text(language, "Select a variable on the canvas.", "캔버스의 변수를 선택하세요."),
    ...
    invalid_json = custom_model_canvas_text(language, "Invalid model canvas file.", "\uc798\ubabb\ub41c \ubaa8\ub378 \uce94\ubc84\uc2a4 \ud30c\uc77c\uc785\ub2c8\ub2e4."),
