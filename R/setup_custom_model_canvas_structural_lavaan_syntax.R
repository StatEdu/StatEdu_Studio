...
structural_canvas_structural_moderation_terms <- function(snapshot, data, edges, structural_edge_info) {
  ...
  for (moderation in moderations) {
    ...
    product_block_key <- paste(
      as.character(predictor_node$id %||% edge_info$predictor),
      as.character(source$id %||% moderator),
      as.character(source$role %||% ""), method,
      sep = "\r"
    )
    product_block <- product_blocks[[product_block_key]] %||% NULL
    if (is.null(product_block)) {
      product_specs <- list()
      ...
      product_block <- list(
        ...
        moderator_range = moderator_range
      )
      product_blocks[[product_block_key]] <- product_block
      measurement_lines <- c(measurement_lines, paste(interaction_factor, "=~", paste(product_names, collapse = " + ")))
    }
    interaction_factor <- product_block$interaction_factor
    product_names <- product_block$product_names
    product_indicator_pairs <- product_block$product_indicator_pairs
    moderator_range <- product_block$moderator_range
    existing_moderator_edges <- Filter(function(item) {
      identical(as.character(item$predictor %||% ""), moderator) &&
        identical(as.character(item$outcome %||% ""), edge_info$outcome)
    }, structural_edge_info)
    moderator_main_effect_exists <- length(existing_moderator_edges) > 0L
    moderator_label <- if (moderator_main_effect_exists) {
      as.character(existing_moderator_edges[[1L]]$label %||% "")
    } else {
      structural_canvas_structural_effect_label("statedu_mod", moderator, edge_info$outcome)
    }
    interaction_label <- paste(
      structural_canvas_lavaan_safe_label("statedu_jn", "statedu_jn"),
      structural_canvas_lavaan_safe_label(edge_info$predictor, "predictor"),
      structural_canvas_lavaan_safe_label(moderator, "moderator"),
      structural_canvas_lavaan_safe_label(edge_info$outcome, "outcome"),
      sep = "_"
    )
    moderation_predictors <- c(
      if (!moderator_main_effect_exists) paste0(moderator_label, "*", moderator) else character(0),
      paste0(interaction_label, "*", interaction_factor)
    )
    structural_lines <- c(structural_lines, paste(edge_info$outcome, "~", paste(moderation_predictors, collapse = " + ")))
...
# Rebuild latent-product indicators for a joint multi-group analysis.  Product
# indicators created for the pooled single-group fit cannot be reused here:
# centering has to be performed inside each group so that the interaction
# coefficient has the same interpretation in every group.  The same helper is
# also used by the stratified bootstrap, where it is called after each
# within-group resample.
structural_canvas_prepare_group_product_indicators <- function(data, group, moderation_definitions = list()) {
  if (!is.data.frame(data)) stop("Multi-group product indicators require a data frame.")
  group <- as.character(group %||% "")
  if (!nzchar(group) || !group %in% names(data)) stop("A valid grouping variable is required to create multi-group product indicators.")
  definitions <- moderation_definitions %||% list()
  if (!length(definitions)) {
    return(list(data = data, audit = data.frame(), policy = list()))
  }
  group_values <- data[[group]]
  observed_groups <- unique(group_values[!is.na(group_values)])
  if (length(observed_groups) < 2L) stop("At least two non-empty groups are required to create multi-group product indicators.")
  audit_rows <- list()
  product_names <- character(0)
  for (definition_index in seq_along(definitions)) {
    definition <- definitions[[definition_index]] %||% list()
    pairs <- definition$product_indicator_pairs %||% data.frame()
    if (!is.data.frame(pairs) || !nrow(pairs) ||
        !all(c("name", "predictor_indicator", "moderator_indicator") %in% names(pairs))) {
      stop("A latent moderation definition did not contain complete product-indicator specifications.")
    }
    method <- as.character(definition$product_indicator_method %||% "all_pairs_dmc")
    if (!method %in% c("all_pairs_dmc", "matched_pair_dmc", "all_pairs_mean_centered")) {
      stop(paste0("Unsupported multi-group product-indicator method: ", method, "."))
    }
    for (pair_index in seq_len(nrow(pairs))) {
      product_name <- as.character(pairs$name[[pair_index]] %||% "")
      predictor_indicator <- as.character(pairs$predictor_indicator[[pair_index]] %||% "")
      moderator_indicator <- as.character(pairs$moderator_indicator[[pair_index]] %||% "")
      required <- c(predictor_indicator, moderator_indicator)
      if (!nzchar(product_name) || any(!nzchar(required)) || any(!required %in% names(data))) {
        stop("A product-indicator specification referenced a missing or unnamed source variable.")
      }
      if (!is.numeric(data[[predictor_indicator]]) || !is.numeric(data[[moderator_indicator]])) {
        stop(paste0(
          "Multi-group product-indicator SEM requires numeric source indicators. Check: ",
          paste(required[!vapply(data[required], is.numeric, logical(1))], collapse = ", "), "."
        ))
      }
      product_values <- rep(NA_real_, nrow(data))
      for (group_value in observed_groups) {
        in_group <- !is.na(group_values) & group_values == group_value
        predictor_values <- as.numeric(data[[predictor_indicator]][in_group])
        moderator_values <- as.numeric(data[[moderator_indicator]][in_group])
        predictor_mean <- mean(predictor_values, na.rm = TRUE)
        moderator_mean <- mean(moderator_values, na.rm = TRUE)
        if (!is.finite(predictor_mean) || !is.finite(moderator_mean)) {
          stop(paste0("A group contained no finite values for product indicator ", product_name, "."))
        }
        current_product <- (predictor_values - predictor_mean) * (moderator_values - moderator_mean)
        product_mean_before_dmc <- mean(current_product, na.rm = TRUE)
        if (method %in% c("all_pairs_dmc", "matched_pair_dmc")) {
          if (!is.finite(product_mean_before_dmc)) {
            stop(paste0("A group contained no finite product values for ", product_name, "."))
          }
          current_product <- current_product - product_mean_before_dmc
        }
        product_values[in_group] <- current_product
        audit_rows[[length(audit_rows) + 1L]] <- data.frame(
          ...
          `Predictor indicator` = predictor_indicator,
          `Moderator indicator` = moderator_indicator,
          Method = method,
          `Complete pairs` = sum(is.finite(predictor_values) & is.finite(moderator_values)),
          `Predictor center` = predictor_mean,
          `Moderator center` = moderator_mean,
          `Product mean before DMC` = product_mean_before_dmc,
          `Product mean after DMC` = mean(current_product, na.rm = TRUE),
          check.names = FALSE, stringsAsFactors = FALSE
        )
      }
      data[[product_name]] <- product_values
      product_names <- unique(c(product_names, product_name))
    }
  }
  list(
    data = data,
    ...
    policy = list(
      scope = "Within-group centering",
      double_mean_centering = all(vapply(definitions, function(item) {
        as.character(item$product_indicator_method %||% "") %in% c("all_pairs_dmc", "matched_pair_dmc")
      }, logical(1))),
      resampling = "Products must be regenerated after every within-group bootstrap resample.",
      product_indicators = product_names
    )
  )
}

# Build the original-factor-only measurement syntax for diagnostics that need
# to separate the substantive measurement model from an artificial interaction
# factor.  The structural group-comparison gate itself uses the full
# measurement-only syntax, including the interaction factor, so that equal
# product-factor loadings are tested rather than merely imposed.
structural_canvas_base_measurement_syntax <- function(syntax, moderation_definitions = list(), language = NULL) {
  measurement <- structural_canvas_measurement_only_syntax(syntax, language)
  interaction_factors <- unique(vapply(moderation_definitions %||% list(), function(item) {
    as.character(item$interaction_factor %||% "")
  }, character(1)))
  interaction_factors <- interaction_factors[nzchar(interaction_factors)]
  if (!length(interaction_factors)) return(measurement)
  lines <- strsplit(as.character(measurement %||% ""), "\\r?\\n")[[1L]]
  keep <- vapply(lines, function(line) {
    if (grepl("=~", line, fixed = TRUE)) {
      lhs <- trimws(sub("=~.*$", "", line))
      return(!lhs %in% interaction_factors)
    }
    if (grepl("~~", line, fixed = TRUE)) {
      terms <- trimws(strsplit(line, "~~", fixed = TRUE)[[1L]])
      return(!any(terms %in% interaction_factors))
    }
    TRUE
  }, logical(1))
