...
structural_canvas_structural_effect_plan <- function(snapshot, analysis_type, estimator = "ML") {
  ...
  pls_moderation_method <- switch(
    tolower(moderation_method),
    product_indicator = "seminr product-indicator interaction",
    orthogonal = "seminr orthogonalized interaction",
  ...
  rows <- list(
    ...
    data.frame(Effect = "Moderation", Status = if (length(moderations)) "Supported" else "Not requested", Method = if (engine == "CB-SEM") moderation_method_label else pls_moderation_method, Limitation = if (engine == "CB-SEM") "Johnson-Neyman regions require a continuous observed moderator or a latent moderator represented on its factor-score scale." else if (engine == "PLSC") "The interaction is an uncorrected composite; PLSc consistency correction remains selective for declared common-factor constructs." else "Simple slopes are reported on the standardized construct-score scale.", stringsAsFactors = FALSE),
    data.frame(Effect = "Moderated mediation", Status = if (length(moderations)) "Available when an indirect chain contains the moderated path" else "Not requested", Method = if (engine == "CB-SEM") "Conditional indirect effect and index of moderated mediation" else "PLS conditional path components; moderated-mediation inference is compiled separately", Limitation = if (engine == "CB-SEM") "Inference uses the fitted product-indicator parameterization and the observed moderator range." else "The interaction is re-estimated in every accepted whole-draw bootstrap sample; PLSpredict remains unavailable for interaction models.", stringsAsFactors = FALSE)
  )
  do.call(rbind, rows)
}
...
structural_canvas_validate_structural_effects <- function(snapshot, analysis_type, estimator = "ML") {
  ...
  if (any(blocked)) {
    stop(paste(plan$Limitation[blocked], collapse = " "), call. = FALSE)
  }
  invisible(plan)
}

structural_canvas_validate_pls_model_contract <- function(snapshot, analysis_type = "plssem") {
  ...
  node_label <- function(node) {
    ...
    as.character(structural_canvas_name(node) %||% node$id %||% "")
  }

  covariates <- unique(trimws(as.character(snapshot$covariates %||% character(0))))
  covariates <- covariates[nzchar(covariates)]
  covariate_targets <- snapshot$covariateTargets %||% list()
  target_values <- if (is.list(covariate_targets)) {
    unique(trimws(as.character(unlist(covariate_targets, recursive = TRUE, use.names = FALSE))))
  } else {
    unique(trimws(as.character(covariate_targets)))
  }
  target_values <- target_values[nzchar(target_values)]
  target_covariates <- if (is.list(covariate_targets)) {
    names(Filter(function(value) length(trimws(as.character(value %||% character(0)))) > 0L &&
      any(nzchar(trimws(as.character(value %||% character(0))))), covariate_targets)) %||% character(0)
  } else character(0)
  if (length(covariates) || length(target_values)) {
    details <- unique(c(covariates, target_covariates))
    details <- details[nzchar(details)]
    stop(paste0(
      "PLS model contract blocked estimation: observed covariates/control variables and covariateTargets are not supported by the current PLS/PLSc engine and would otherwise be silently omitted.",
      if (length(details)) paste0(" Covariates: ", paste(details, collapse = ", "), ".") else ""
    ), call. = FALSE)
  }

  has_parameter_modifier <- function(item) {
    fixed_value <- suppressWarnings(as.numeric(item$fixedValue %||% NA_real_))
    start_value <- suppressWarnings(as.numeric(item$startValue %||% NA_real_))
    identical(item$free, FALSE) || is.finite(fixed_value) || is.finite(start_value) ||
      nzchar(trimws(as.character(item$parameterName %||% ""))) ||
      nzchar(trimws(as.character(item$equalityLabel %||% "")))
  }
  modified_edges <- Filter(has_parameter_modifier, edges)
  modified_nodes <- Filter(function(node) {
    node$role %in% c("error", "disturbance") && has_parameter_modifier(node)
  }, nodes)
  if (length(modified_edges) || length(modified_nodes)) {
    edge_labels <- vapply(modified_edges, function(edge) {
      from <- node_label(node_for(edge$from))
      to <- node_label(node_for(edge$to))
      endpoints <- paste(c(from, to)[nzchar(c(from, to))], collapse = " -> ")
      if (nzchar(endpoints)) endpoints else as.character(edge$id %||% "unnamed edge")
    }, character(1))
    node_labels <- vapply(modified_nodes, node_label, character(1))
    elements <- unique(c(edge_labels, node_labels))
    stop(paste0(
      "PLS model contract blocked estimation: fixed/free constraints, fixed values, start values, parameter names, and equality labels are not supported by the current PLS/PLSc engine and would otherwise be silently ignored. Modified elements: ",
  ...
  structural_edges <- Filter(function(edge) {
    if (identical(edge$kind %||% "", "covariance") ||
        identical(as.character(edge$pathType %||% ""), "higherOrder")) return(FALSE)
    from <- node_for(edge$from)
    to <- node_for(edge$to)
    !is.null(from) && !is.null(to) && identical(from$role, "latent") && identical(to$role, "latent")
  }, edges)
  adjacency <- split(
    vapply(structural_edges, function(edge) as.character(edge$to %||% ""), character(1)),
    vapply(structural_edges, function(edge) as.character(edge$from %||% ""), character(1))
  )
  ...
  has_cycle <- any(vapply(latent_ids, visit, logical(1)))
  if (has_cycle) {
    stop(
      "PLS model contract blocked estimation: directed structural paths must be acyclic. Reciprocal paths and feedback loops are not supported by the current PLS/PLSc engine.",
      call. = FALSE
    )
  }
  ...
  ownership <- lapply(measurement_edges, function(edge) {
    from <- node_for(edge$from)
    to <- node_for(edge$to)
    latent <- if (identical(from$role, "latent")) from else to
    indicator <- if (identical(from$role, "indicator")) from else to
    c(indicator_id = as.character(indicator$id %||% ""), indicator = node_label(indicator),
      latent_id = as.character(latent$id %||% ""), latent = node_label(latent))
  })
  if (length(ownership)) {
    ownership_matrix <- do.call(rbind, ownership)
    pair_keys <- paste(ownership_matrix[, "indicator_id"], ownership_matrix[, "latent_id"], sep = "|")
    duplicate_pairs <- unique(ownership_matrix[duplicated(pair_keys) | duplicated(pair_keys, fromLast = TRUE), "indicator", drop = TRUE])
    latent_memberships <- split(ownership_matrix[, "latent_id"], ownership_matrix[, "indicator_id"])
    cross_loaded_ids <- names(Filter(function(value) length(unique(value)) > 1L, latent_memberships))
    if (length(duplicate_pairs) || length(cross_loaded_ids)) {
      cross_loaded <- vapply(cross_loaded_ids, function(indicator_id) {
        rows <- ownership_matrix[ownership_matrix[, "indicator_id"] == indicator_id, , drop = FALSE]
        paste0(rows[1L, "indicator"], " -> ", paste(unique(rows[, "latent"]), collapse = ", "))
      }, character(1))
      details <- unique(c(
        if (length(duplicate_pairs)) paste0(duplicate_pairs, " (duplicate measurement edge)") else character(0),
        cross_loaded
      ))
      stop(paste0(
        "PLS model contract blocked estimation: each indicator may belong to only one construct and each measurement edge must be unique. Duplicate indicator ownership: ",
...
run_structural_canvas_analysis <- function(snapshot, data, analysis_type, estimator = "ML", missing = "fiml", std_lv = FALSE, ordered = character(0), nominal = character(0), residual_variance_fixes = numeric(0), ml_likelihood = "normal") {
  if (length(attr(data,"statedu_scope_excluded"))) {
    ...
    nominal <- setdiff(nominal,analysis_scope_excluded(data))
  }
  nodes <- snapshot$nodes %||% list()
  edges <- snapshot$edges %||% list()
  ...
  structural_canvas_validate_pls_model_contract(snapshot, analysis_type)
  ...
  result$plsc_uncorrected_composites <- unique(c(
    estimator_selection$composites,
    result$interaction_constructs %||% character(0)
  ))
