...
root<-'output/category-label-lookup-20260915'
dir.create(root,recursive=TRUE,showWarnings=FALSE)
old<-new.env(parent=.GlobalEnv);new<-new.env(parent=.GlobalEnv)
sys.source('scripts/fixtures/category_label_lookup_reference.R',old)
...
for(n in c(0L,1L,12L,100L))for(kind in c('none','all','partial','duplicate','missing','factor','numeric'))for(pairs in c(1L,3L,11L)){
