...
root<-paste0('output/sem-shared-inverse-final-5000-20260915/',mode,'-',id)
...
eligible<-sem_shared_inverse_eligible(prepared,5000L,4L)
stopifnot(eligible)
...
capture<-function(){
 ...
 stdout<-capture.output(value<-withCallingHandlers(structural_canvas_effect_bootstrap_prepared(
  prepared,reps=5000L,seed=fixture$bootstrap_seed,ci_method='bias_corrected',workers=4L,chunk_size=100L,return_draws=TRUE),
...
for(version in if(id%%2L)c('baseline','profiled')else c('profiled','baseline')) {
 cat('START',mode,id,version,format(Sys.time()),'\n')
 body(structural_canvas_effect_bootstrap_fixed_index_worker)<-if(version=='baseline'||!eligible)original_body else profile_body
 results[[version]]<-capture()
 saveRDS(results[[version]],file.path(root,paste0(version,'-checkpoint.rds')))
 cat('DONE',mode,id,version,format(Sys.time()),'\n')
}
...
stopifnot(identical(baseline,profiled,num.eq=FALSE),length(draws$sample_indices)==5000L,
 length(draws$valid_mask)==5000L,nrow(draws$raw)==5000L,nrow(draws$standardized)==5000L,sum(draws$valid_mask)>0L)
...
cat('PASS: 5000 full draws/results/diagnostics/stdout/RNG exact',id,'\n')
