...
root<-'output/correlation-metadata-profile-20260915/server-initialized';dir.create(root,recursive=TRUE,showWarnings=FALSE)
...
capture<-function(info){
 diagnostics<-list();stdout<-capture.output(value<-withCallingHandlers(
  prepare_correlation_results(data,names(data),variable_info=info,options=list(continuous_method='pearson',normality=TRUE)),
...
baseline_path<-file.path(dirname(root),paste0('result-',run,'.rds'))
if(file.exists(baseline_path))stopifnot(identical(expected,readRDS(baseline_path),num.eq=FALSE))
timings<-list()
for(n in if(run%%2L)c(20L,10000L)else c(10000L,20L)){
 ...
 for(iteration in seq_len(3L)){
  ...
  timings[[length(timings)+1L]]<-data.frame(run,metadata_rows=n,iteration,seconds=elapsed)
 }
}
...
print(aggregate(seconds~metadata_rows,do.call(rbind,timings),median))
