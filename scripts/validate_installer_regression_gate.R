...
pls_mga_engine <- read_text("R/setup_custom_model_canvas_structural_pls_mga_engine.R")
...
pls_specific_indirect_contract <- read_text("scripts/validate_pls_specific_indirect_engine.R")
pls_mga_effect_contract <- read_text("scripts/validate_pls_mga_effect_engine.R")
...
csem_stage <- read_text("scripts/stage_bundled_validation_packages.R")
csem_runtime_contract <- read_text("scripts/validate_bundled_validation_packages.R")
runtime_version_contract <- read_text("scripts/validate_bundled_runtime_package_versions.R")
bundled_pls_focused_regression <- read_text("scripts/run_bundled_pls_focused_regressions.R")
bundled_validation_expected_lock <- read_text("scripts/bundled_validation_packages.expected.csv")
bundled_validation_lock_contract <- read_text("scripts/validate_bundled_validation_lock_contract.R")
runtime_prune <- read_text("scripts/prune_r_runtime.R")
oss_notices <- read_text("scripts/generate_oss_notices.R")
...
release_build <- read_text("scripts/build_electron_release.ps1")
...
for (pattern in c(
  'identical(audit$schema$version, "1.8")',
...
require_contains(
  stabilization,
  'Join-Path $RepositoryRoot "packaging\\electron\\runtime\\R-4.5.3"',
  "bundled R 4.5.3 is the default stabilization runtime"
)
require_contains(
  stabilization,
  "Find-Rscript -RepositoryRoot $RepoRoot",
  "stabilization runner passes the repository root to bundled-R discovery"
)
require_contains(
  stabilization,
  '$env:R_LIBS = $defaultBundledRuntimeLibrary',
  "bundled validation library precedence"
)
require_contains(
  stabilization,
  '$env:R_LIBS_USER = $defaultBundledRuntimeLibrary',
  "bundled user-library isolation"
)
require_contains(
  stabilization,
  '$env:R_LIBS_SITE = $defaultBundledRuntimeLibrary',
  "bundled site-library isolation"
)
...
message("Checking the bundled cSEM validation-runtime packaging contract...")
require_contains(app_bootstrap, "bundled_validation_packages <- c(", "separate bundled validation package contract")
require_contains(app_bootstrap, 'cSEM = "0.6.1"', "pinned bundled cSEM version")
require_contains(app_bootstrap, "bundled_runtime_package_versions <- c(", "separate bundled runtime version contract")
require_contains(app_bootstrap, 'lavaan = "0.7-2"', "pinned bundled lavaan version")
require_absent(app_bootstrap, '  "cSEM",', "cSEM must not become an interactive startup package")
require_contains(csem_stage, "tools::package_dependencies(", "recursive bundled validation dependency discovery")
require_contains(csem_stage, 'which = c("Depends", "Imports", "LinkingTo")', "complete bundled validation dependency types")
require_contains(csem_stage, "Bundled validation dependency closure is incomplete", "source dependency closure fail-closed gate")
require_contains(csem_stage, "Staged runtime validation dependency version mismatch", "source-to-runtime dependency version lock")
require_contains(csem_stage, "library_paths <- unique(c(runtime_library, source_library, .libPaths()))", "runtime dependency precedence protects governed packages")
require_contains(csem_stage, "bundled_validation_packages.lock.csv", "packaged validation dependency lock report")
require_contains(csem_stage, "bundled_validation_read_expected_contract(repo_root)", "approved dependency lock used during staging")
require_contains(csem_stage, "bundled_validation_assert_exact_contract(", "staging exact dependency lock comparison")
require_contains(csem_runtime_contract, "installed.packages(lib.loc = runtime_library", "runtime-only validation package inventory")
require_contains(csem_runtime_contract, "Bundled runtime validation dependency closure is incomplete", "runtime dependency closure fail-closed gate")
require_contains(csem_runtime_contract, "Bundled runtime validation dependency closure has unexpected package(s)", "unexpected runtime dependency rejection")
require_contains(csem_runtime_contract, "Bundled runtime validation dependency version mismatch", "runtime dependency version rejection")
require_contains(csem_runtime_contract, "loadNamespace(package, lib.loc = runtime_library)", "runtime-only namespace loading")
require_contains(csem_runtime_contract, 'file.path(repo_root, "scripts", "bundled_validation_packages.expected.csv")', "approved lock path")
require_contains(csem_runtime_contract, "bundled_validation_expected_lock_rows <- function() 72L", "approved 72-package non-base closure")
require_contains(csem_runtime_contract, '"cf6e4175d47ea943cd6d017951357ee87b1b615f18c6f32799b046b179447674"', "approved lock SHA-256")
require_contains(csem_runtime_contract, "Bundled validation expected lock SHA-256 mismatch", "approved lock checksum fail-closed gate")
expected_lock_connection <- textConnection(bundled_validation_expected_lock)
expected_lock_records <- utils::read.csv(
  expected_lock_connection,
  stringsAsFactors = FALSE,
  check.names = FALSE,
  colClasses = "character"
)
close(expected_lock_connection)
require_contains(
  gitattributes,
  "scripts/bundled_validation_packages.expected.csv text eol=lf",
  "approved lock LF checkout contract"
)
approved_lock_sha256 <- "cf6e4175d47ea943cd6d017951357ee87b1b615f18c6f32799b046b179447674"
actual_lock_sha256 <- tolower(digest::digest(
  charToRaw(enc2utf8(paste0(bundled_validation_expected_lock, "\n"))),
  algo = "sha256",
  serialize = FALSE
))
if (
  !identical(names(expected_lock_records), c("Package", "Version")) ||
    nrow(expected_lock_records) != 72L ||
    anyDuplicated(expected_lock_records$Package) ||
    !identical(
      expected_lock_records$Package,
      expected_lock_records$Package[
        order(enc2utf8(expected_lock_records$Package), method = "radix")
      ]
    ) ||
    !identical(actual_lock_sha256, approved_lock_sha256)
) {
  stop(
    "Approved bundled validation lock must be the canonical, SHA-256-sealed 72-row Package/Version contract.",
    call. = FALSE
  )
}
for (pattern in c(
  ...
  require_contains(
    bundled_validation_lock_contract,
    pattern,
    sprintf("approved-lock mutation regression: %s", pattern)
  )
}
require_contains(runtime_version_contract, "installed.packages(lib.loc = runtime_library", "runtime-only exact package version inventory")
require_contains(runtime_version_contract, "Bundled runtime package version contract failed", "exact runtime package fail-closed message")
require_contains(runtime_prune, "names(bundled_validation_packages)", "runtime prune protection for validation roots")
require_contains(oss_notices, 'return("Bundled validation package")', "OSS validation-package scope")
require_contains(build, "[string]$BundledValidationLibrary", "explicit bundled validation library parameter")
require_contains(build, "STATEDU_BUNDLED_VALIDATION_LIBRARY", "bundled validation library environment override")
require_contains(build, 'Join-Path $env:TEMP "statedu-csem-validation-lib"', "standard TEMP validation library fallback")
require_contains(build, "No package is downloaded by the build", "offline validation-package staging contract")
require_contains(build, "scripts\\stage_bundled_validation_packages.R", "validation package staging invocation")
require_contains(build, "scripts\\validate_bundled_runtime_package_versions.R", "pre-stage exact package version invocation")
require_contains(build, "scripts/run_bundled_pls_focused_regressions.R", "focused PLS driver tracked-source guard")
require_contains(build, '"scripts/bundled_validation_packages.expected.csv"', "approved lock tracked-source guard")
require_contains(build, '(Join-Path $appStage "scripts\\run_bundled_pls_focused_regressions.R")', "staged-app focused PLS driver invocation")
require_contains(build, '"--repo-root=$appStage"', "staged-app source context")
require_contains(build, '"--runtime-root=$runtimeStage"', "exact staged runtime context")
require_contains(build, '$env:R_LIBS = $runtimeLibrary', "staged runtime R_LIBS isolation")
require_contains(build, '$env:R_LIBS_USER = $runtimeLibrary', "bundled runtime library isolation")
require_contains(build, '$env:R_LIBS_SITE = $runtimeLibrary', "staged runtime R_LIBS_SITE isolation")
require_contains(build, '$env:STATEDU_CSEM_VALIDATION_MODE = "required"', "post-stage required cSEM oracle mode")
require_absent(build, '(Join-Path $repoRoot "scripts\\validate_bundled_validation_packages.R")', "redundant direct staged validation-closure invocation")
require_absent(build, '(Join-Path $repoRoot "scripts\\validate_pls_fit_csem.R")', "redundant direct staged cSEM invocation")
require_contains(release_build, "[string]$BundledValidationLibrary", "public build validation-library parameter")
require_contains(release_build, "-BundledValidationLibrary $BundledValidationLibrary", "public build validation-library forwarding")
require_contains(electron_smoke, "bundled_validation_packages.lock.csv", "packaged validation lock smoke check")
require_contains(electron_smoke, '$expectedValidationLock.Count -ne 72', "packaged approved-lock row-count check")
require_contains(electron_smoke, '$actualRecord.Version -cne $expectedRecord.Version', "packaged exact lock comparison")
require_contains(electron_smoke, '$csemLicense[0].Scope -ne "Bundled validation package"', "packaged cSEM OSS scope smoke check")
require_contains(electron_smoke, "scripts\\run_bundled_pls_focused_regressions.R", "packaged focused PLS driver execution")
require_contains(electron_smoke, '"--repo-root=$bundledAppDir"', "packaged app source context")
require_contains(electron_smoke, '"--runtime-root=$runtimeDir"', "exact packaged runtime context")
require_contains(electron_smoke, '$env:R_LIBS = $packagedRuntimeLibrary', "packaged R_LIBS isolation")
require_contains(electron_smoke, '$env:R_LIBS_USER = $packagedRuntimeLibrary', "packaged R_LIBS_USER isolation")
require_contains(electron_smoke, '$env:R_LIBS_SITE = $packagedRuntimeLibrary', "packaged R_LIBS_SITE isolation")
require_absent(electron_smoke, "& $rscript --vanilla scripts\\validate_pls_fit_csem.R", "redundant direct packaged cSEM invocation")

message("Checking the bundled-runtime-only focused PLS/PLSc regression driver...")
bundled_pls_validators <- c(
...
driver_validation_entries <- regmatches(
  bundled_pls_focused_regression,
  gregexpr('script = "scripts/[^"]+\\.R"', bundled_pls_focused_regression)
)[[1L]]
driver_validation_paths <- sub(
  '^script = "([^"]+)"$',
  "\\1",
  driver_validation_entries
)
if (
  length(driver_validation_paths) != length(bundled_pls_validators) ||
    anyDuplicated(driver_validation_paths) ||
    !setequal(driver_validation_paths, bundled_pls_validators)
) {
  stop("Bundled-runtime focused PLS/PLSc driver must contain exactly the approved validators.", call. = FALSE)
}
for (validation in bundled_pls_validators) {
  require_contains(
    bundled_pls_focused_regression,
    paste0('script = "', validation, '"'),
    sprintf("bundled focused validation %s", validation)
  )
}
for (pattern in c(
  "if (!identical(current_r_home, runtime_root))",
  'if (!identical(as.character(getRversion()), "4.5.3"))',
  ".libPaths() contains non-runtime path(s)",
  "the runtime library must be first in .libPaths()",
  ...
  "Bundled PLS/PLSc focused regression PASS"
)) {
  require_contains(
    bundled_pls_focused_regression,
    pattern,
    sprintf("bundled focused runtime isolation contract: %s", pattern)
  )
}
require_contains(stabilization, "scripts\\validate_bundled_runtime_package_versions.R", "full stabilization exact runtime-version validation")
require_contains(stabilization, "scripts\\validate_bundled_validation_packages.R", "full stabilization exact validation-lock check")
require_contains(stabilization, "scripts\\validate_bundled_validation_lock_contract.R", "full stabilization validation-lock mutation regression")
require_contains(
  stabilization,
  paste0(
  ...
  "structural bootstrap gate uses the exact bundled runtime"
)
require_contains(stabilization, '$bundledRscriptPath --vanilla $script', "full stabilization uses bundled Rscript")
require_contains(stabilization, '"--runtime-root=$bundledRuntimeRoot"', "full stabilization bundled runtime context")
require_contains(stabilization, '$env:R_LIBS_SITE = $bundledRuntimeLibrary', "full stabilization bundled library isolation")
...
for (pattern in c(
  ...
  "PLS specific/total indirect/total effect engine validation: PASS"
)) {
  require_contains(
    pls_specific_indirect_contract,
    pattern,
    sprintf("executable PLS indirect-effect regression: %s", pattern)
  )
}
for (pattern in c(
  "structural_canvas_pls_mga_compile(",
  'group_runs[c("G3", "G1", "G2")]',
  "isTRUE(plsc_blocked)",
  'identical(mga$omnibus_status, "not_provided")',
  '"Holm-adjusted p"',
  "structural_canvas_pls_mga_effects(",
  "min_group_n = 60L",
  'integration$group_diagnostics[["Small-group warning"]]',
  "PLS multi-group effect engine validation: PASS"
)) {
  require_contains(
    pls_mga_effect_contract,
    pattern,
    sprintf("executable PLS-MGA effect regression: %s", pattern)
  )
}
for (pattern in c(
  "bootstrap_reps = 5000L",
  "PLSc multi-group effect inference is not supported",
  "Within-group mean replacement",
  "diagnostic warning, not an automatic estimation stop",
  "minimum_valid_ratio = structural_canvas_pls_bootstrap_min_valid_ratio()",
  "BH and Holm adjustments are computed separately within each effect family",
  'omnibus_status = "not_provided"'
)) {
  require_contains(pls_mga_engine, pattern, sprintf("PLS-MGA engine contract: %s", pattern))
}
require_contains(
  app_bootstrap,
  '"setup_custom_model_canvas_structural_pls_mga_engine.R"',
  "PLS-MGA app source registration"
)
for (pattern in c(
...
for (pattern in c(
  ...
  require_contains(structural_settings, pattern, sprintf("fixed independent sampling-design contract: %s", pattern))
}
for (pattern in c(
  "structural_canvas_run_plsc_bootstrap",
