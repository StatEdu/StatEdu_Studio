...
default_profile <- pls_external_benchmark_profile()
blank_profile <- pls_external_benchmark_profile("")
legacy_profile <- pls_external_benchmark_profile("historical301")
stopifnot(
  identical(PLS_EXTERNAL_DEFAULT_PROFILE, "holzinger-swineford-first100-smartpls-student"),
  identical(default_profile$id, PLS_EXTERNAL_DEFAULT_PROFILE),
  identical(blank_profile$id, PLS_EXTERNAL_DEFAULT_PROFILE),
  identical(legacy_profile$id, PLS_EXTERNAL_LEGACY_PROFILE),
  grepl("Legacy historical 301-row reference", legacy_profile$intended_scope, fixed = TRUE)
)
...
stopifnot(
  ...
  identical(benchmark_manifest$profile$id, "holzinger-swineford-first100-smartpls-student"),
  identical(benchmark_manifest$data$rows, 100L),
  identical(basename(benchmark$data_path), "HolzingerSwineford1939_first100_x1_x9.txt"),
...
# The historical 301-row profile is retained as a documented reference, but its
# PLSc correction currently produces an inadmissible standardized outer loading
# (> 1). The release contract is fail-closed: do not silently emit or compare an
# invalid PLSc solution merely to keep the legacy fixture green.
historical_dir <- tempfile(pattern = "pls-external-historical301-")
dir.create(historical_dir, recursive = TRUE)
on.exit(unlink(historical_dir, recursive = TRUE, force = TRUE), add = TRUE)
historical_result <- try(
  generate_pls_external_benchmark(historical_dir, "holzinger-swineford-301"),
  silent = TRUE
)
stopifnot(
  inherits(historical_result, "try-error"),
  grepl("PLSC benchmark fit was unavailable", as.character(historical_result), fixed = TRUE),
  !file.exists(file.path(historical_dir, "statedu_fit.csv")),
  !file.exists(file.path(historical_dir, "external_fit_template.csv")),
  !file.exists(file.path(historical_dir, "benchmark_manifest.json"))
)

mixed_profile_dir <- tempfile(pattern = "pls-external-mixed-profile-")
dir.create(mixed_profile_dir, recursive = TRUE)
on.exit(unlink(mixed_profile_dir, recursive = TRUE, force = TRUE), add = TRUE)
writeLines("stale legacy profile data", file.path(mixed_profile_dir, "HolzingerSwineford1939.csv"), useBytes = TRUE)
mixed_profile_result <- try(generate_pls_external_benchmark(mixed_profile_dir), silent = TRUE)
stopifnot(
  inherits(mixed_profile_result, "try-error"),
  grepl("different PLS external benchmark profile", as.character(mixed_profile_result), fixed = TRUE)
)
...
basic <- prepare_pls_external_handoff(basic_dir)
...
basic_pending_run <- jsonlite::fromJSON(basic$run_record_path, simplifyVector = TRUE)
stopifnot(
  identical(basic_pending_run$profile, PLS_EXTERNAL_DEFAULT_PROFILE),
  identical(basic_pending_run$data_file, basename(basic$data_path)),
  isTRUE(basic_pending_run$artifact_contract_required)
)

package_source <- paste(readLines(file.path("scripts", "package_pls_external_handoff.ps1"), warn = FALSE, encoding = "UTF-8"), collapse = "\n")
stopifnot(
  grepl('StatEdu_1.2.4_PLS_external_handoff.zip', package_source, fixed = TRUE),
  grepl('[string]$Profile = "holzinger-swineford-first100-smartpls-student"', package_source, fixed = TRUE),
  grepl('"--profile=$canonicalProfile"', package_source, fixed = TRUE),
  grepl('external_run.json data_file must be a safe basename.', package_source, fixed = TRUE),
  grepl('PLS external handoff contains stale data from another profile', package_source, fixed = TRUE)
)
...
check_initialization_contract <- function() {
  path <- file.path(basic_dir, artifact_names[["algorithm_settings"]])
  ...
  for (scenario in c("default_unit", "default_nonunit", "unknown")) {
    candidate <- settings
    candidate$plsAlgorithmSettings$initialWeightsStrategy <- if (scenario == "unknown") "UNKNOWN" else "DEFAULT"
    if (scenario == "default_nonunit") candidate$plsAlgorithmSettings$initialWeights[[1L]]$weight <- 2
    jsonlite::write_json(candidate, path, auto_unbox = TRUE)
    record <- basic_run
    record$execution_artifacts$algorithm_settings_bytes <- file.info(path)$size
    record$execution_artifacts$algorithm_settings_sha256 <- pls_evidence_sha256(path)
    result <- try(pls_evidence_validate_student100_artifacts(basic_dir, record), silent = TRUE)
    stopifnot(identical(inherits(result, "try-error"), scenario != "default_unit"))
  }
}
check_initialization_contract()
