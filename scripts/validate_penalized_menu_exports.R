...
out <- "tmp/penalized-menu"
html <- paste(readLines(file.path(out,"results.html"),encoding="UTF-8",warn=FALSE),collapse="\n")
...
for(mode in c("current","accumulated")) {
 entries <- list(list(id="split",title="Ridge / LASSO / Elastic Net",html=html))
 ...
 message("PASS: ",mode," HTML/PDF/Word/HWPX/Excel, three methods, displayed statistics and notes")
}
