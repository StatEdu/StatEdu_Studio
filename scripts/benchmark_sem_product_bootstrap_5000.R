...
release_dmc_fixture <- function() {
  generated <- structural_bootstrap_dmc_fixture()
  data <- generated$data
  snapshot <- list(
  ...
  list(
    id = generated$id,
    label = "StatEdu deterministic 75-row all-pairs DMC",
    data = data,
    snapshot = snapshot,
    expected_rows = 75L,
    ...
    R_version = generated$R_version,
    RNGkind = as.list(generated$RNGkind),
    fixture_seed = generated$seed,
...
code_files <- c(
  ...
  deterministic_fixture = file.path("scripts", "structural_bootstrap_dmc_fixture.R"),
...
for (fixture_index in seq_along(fixture_factories)) {
  ...
  fixture_report <- list(
    ...
    generation_contract = if (!is.null(fixture$sha256)) list(
      R_version = fixture$R_version,
      RNGkind = fixture$RNGkind,
  ...
  if (!is.null(existing_fixture)) {
    immutable_fields <- c(
      ...
      "model_hash_sha256", "generation_contract"
    )
