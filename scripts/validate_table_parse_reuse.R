...
# Verify one quiet parse per table, while preserving repeated warning/message diagnostics.
original_parser<-result_html_table_cells
for(mode in c('quiet','warning','message')) {
  outcomes<-list()
  for(v in c('baseline','current')) {
    select_variant(v);calls<-0L;conditions<-character()
    result_html_table_cells<-function(node) {
      calls<<-calls+1L
      if(mode=='warning')warning('parse diagnostic',call.=FALSE)
      if(mode=='message')message('parse diagnostic')
      original_parser(node)
    }
    set.seed(941)
    value<-withCallingHandlers(result_entry_tables(entries[[1]]),
    ...
    outcomes[[v]]<-list(value=value,conditions=conditions,rng=.Random.seed)
    stopifnot(calls==if(v=='current' && mode=='quiet')1L else 2L)
  }
  stopifnot(identical(outcomes$baseline,outcomes$current,num.eq=FALSE))
}
result_html_table_cells<-original_parser
select_variant('current')
for(html in c('<table></table>','<table><tr></tr></table>',tables)) {
  node<-xml2::xml_find_first(xml2::read_html(html),'.//table')
  select_variant('baseline');before<-result_docx_table_payload(node)
  select_variant('current');after<-result_docx_table_payload(node)
  parsed<-result_html_table_cells(node)
  stopifnot(identical(before,after,num.eq=FALSE),
    identical(after,result_docx_table_payload(node,parsed=parsed),num.eq=FALSE))
}
cat('PASS: complete table payloads, default/explicit parses, quiet parse count 2 -> 1, diagnostic count 2 -> 2, conditions and RNG.\n')
