...
correlation_normality_summary <- function(data, variables, variable_info = NULL, labels = character(0), category_table = NULL, prepared_vectors = NULL, measurement_reader = NULL) {
  measurement_reader <- measurement_reader %||% correlation_measurement_reader(variable_info)
...
prepare_correlation_results <- function(
  ...
  normality_table <- if (isTRUE(normality_checked)) {
    correlation_normality_summary(data, variables, variable_info, labels, category_table, prepared_vectors, measurement_reader)
  } else {
    data.frame()
  }
