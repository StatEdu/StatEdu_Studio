source('scripts/validate_survival_reason_combined.R')
...
for(pattern in c('complete','overlap','disjoint'))for(kind in c('basic','strata','cluster','spline')){
 ...
 for(language in c('en','ko')){
  html_a<-htmltools::renderTags(survival_cox_results_panel(a$value,language=language))
  html_b<-htmltools::renderTags(survival_cox_results_panel(b$value,language=language))
  stopifnot(identical(html_a,html_b,num.eq=FALSE));html_checks<-html_checks+1L
 }
...
cat('PASS:',html_checks,'English/Korean current Cox panel HTML/dependency comparisons\n')
