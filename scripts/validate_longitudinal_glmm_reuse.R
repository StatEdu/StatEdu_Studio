...
if(length(reference_path))sys.source(reference_path[[1]],old) else {
 ...
 old$longitudinal_sensitivity_analysis_results<-function(...,selected_fit=NULL)
  original_sensitivity(...,selected_fit=if(inherits(selected_fit$model,'glmerMod'))NULL else selected_fit)
}
...
check<-function(d,args,signal='quiet'){
 ...
 eligible<-identical(args$model_type,'glmm')&&args$family %in% c('binomial','poisson')&&signal=='quiet'&&a$quiet
...
for(slope in c(FALSE,TRUE)){
 args<-list(model_type='glmm',family='binomial',random_slope=slope)
 ...
 factored<-d;factored$y<-factor(factored$y);check(factored,args)
}
# Also compare the shared LMM path and the Poisson extension.
continuous<-d;continuous$y<-eta+rnorm(n);check(continuous,list(model_type='lmm',family='gaussian',random_slope=FALSE))
counts<-d;counts$y<-rpois(n,exp(eta/3));check(counts,list(model_type='glmm',family='poisson',random_slope=FALSE))
stopifnot(reused>0L,reused_slope>0L,fallback>0L)
cat('PASS:',passed,'GLMM cases;',reused,'reused including',reused_slope,'random slopes;',fallback,'fallback cases\n')
