...
position_after <- function(pattern, source, after) {
  matches <- gregexpr(pattern, source, fixed = TRUE)[[1L]]
  matches <- matches[matches > after]
  if (length(matches)) matches[[1L]] else -1L
}
stale_job_clear_position <- position_after("pls_bootstrap_job(NULL)", handler_source, stale_job_reset_position)
stale_notification_clear_position <- position_after(
  'shiny::removeNotification(paste0(prefix, "-pls-bootstrap-progress"))',
  handler_source,
  stale_job_reset_position
)
new_bootstrap_gate_position <- position_after(
  "if (is.finite(nboot) && nboot > 0L)",
  handler_source,
  stale_job_clear_position
)
stopifnot(
  !grepl("!is.na(bootmatrix[1L, ])", engine_source, fixed = TRUE),
  !grepl("parLapplyLB(cluster, indices, function(index)", engine_source, fixed = TRUE),
  grepl("parLapplyLB(cluster, indices, worker_dispatch)", engine_source, fixed = TRUE),
  grepl("environment(worker_dispatch) <- .GlobalEnv", engine_source, fixed = TRUE),
