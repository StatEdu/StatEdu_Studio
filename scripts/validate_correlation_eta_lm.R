...
restore<-function(node) {
 if(identical(node,quote(structure(stats::lm(values ~ groups),class=c('aov','lm'))))) {replacements<<-replacements+1L;return(quote(stats::aov(values ~ groups)))}
...
options(saved)
cat('PASS',count,'exact captures including',tables,'raw ANOVA summaries\n')
