source('scripts/validate_category_label_role.R')
run<-as.integer(commandArgs(trailingOnly=TRUE)[1L])
if(is.na(run))run<-1L
timings<-list()
for(n in c(1000L,5000L))for(kind in c('small','large')){
 info<-make_info(n)
 saved<-data.frame(name=rev(info$name),reference=rep('2',n))
 for(column in category_label_edit_columns())saved[[column]]<-rep('label',n)
 assignments<-if(kind=='large')info$name else head(info$name,3L)
 args<-list(info=info,selected_names=info$name,saved_values=saved,
   dependent=head(assignments,length(assignments)%/%3L),
   independent=assignments,controls=rev(assignments))
