...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
 const manifest=JSON.parse(fs.readFileSync('tmp/merge-headerless-fixtures/manifest.json','utf8'));
 ...
 await visit('data_editor_merge');await page.locator('#merge_csv_header').uncheck();await page.locator('#merge_dat_has_names').uncheck();await page.locator('#merge_join_type input[value="full"]').check();
 for(const lang of ['ja','zh','es','fr','de','vi','en','ko']) {
  ...
  assert.equal(await page.locator('#merge_csv_header').isChecked(),false);assert.equal(await page.locator('#merge_dat_has_names').isChecked(),false);
  for(const [format,fixture] of Object.entries(manifest)) {
   await page.locator('#merge_dat_delimiter').selectOption(fixture.delimiter);
   await page.locator('#merge_id_variable').fill(fixture.key);await page.locator('#merge_id_variable').blur();
   ...
   assert.deepEqual((await rows()).map(r=>[Number(r[1]),Number(r[2])]),[[2,3],[2,3]],`${lang}/${format}/summary`);
   ...
   assert.ok((await page.locator('#merge_data_message').innerText()).includes(dict['merge.ui.preview'].replace('%s','3').replace('%s','5')));
   const toggle=page.locator(format==='csv'?'#merge_csv_header':'#merge_dat_has_names');
   await toggle.check();await page.waitForTimeout(400);assert.equal((await page.locator('#merge_data_message').innerText()).trim(),'');
   assert.deepEqual((await rows()).map(r=>Number(r[1])),[1,1]);
   await toggle.uncheck();await page.waitForTimeout(400);assert.deepEqual((await rows()).map(r=>Number(r[1])),[2,2]);
  }
  console.log('PASS',lang,'4 headerless paths: false options persist, full values, localized status and header toggle recovery');
 }
