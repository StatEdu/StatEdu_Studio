...
stopifnot(
  ...
  length(holdout_comparison$admissibility_reasons) == nrow(holdout_comparison$table),
  all(lengths(holdout_comparison$admissibility_reasons) == 0L),
...
if (nrow(sequential_mi_candidates)) stopifnot(
  "skipped_records" %in% names(sequential_mi_candidates),
  is.list(sequential_mi_candidates$skipped_records),
  identical(structural_canvas_mi_skipped_text(sequential_mi_candidates, "en"),
    as.character(sequential_mi_candidates$skipped_details))
)
