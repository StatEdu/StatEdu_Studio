...
message("Checking that analysis menus initialize only their own server modules...")
require_contains(
  app_server,
...
require_contains(
  ...
  "dedicated survival server registration"
)
require_contains(
  app_server,
  "statedu_log_timing(\"deferred survival module\"",
  "survival-only registration timing"
)
require_contains(
  app_server,
  "statedu_log_timing(\"deferred structural-equation CFA module\"",
  "structural-equation-only registration timing"
)
require_contains(
  app_server,
  "register_on_first_menu_visit(\"analysis_complex_frequencies\"",
  "dedicated complex-sample frequency registration"
)
require_contains(
  app_server,
  "register_on_first_menu_visit(\"analysis_complex_custom_model\"",
  "dedicated complex-sample custom-model registration"
)
require_absent(
  app_server,
  "statedu_log_timing(\"deferred analysis modules\"",
  "legacy bulk analysis registration"
)
require_absent(
  app_server,
  "statedu_log_timing(\"deferred complex-sample modules\"",
  "legacy bulk complex-sample registration"
)
