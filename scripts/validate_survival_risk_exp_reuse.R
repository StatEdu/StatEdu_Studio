...
baseline <- current
original <- quote(exp(-hazard * risk))
replacement <- quote(if (is.null(risk_index)) exp(-hazard * risk) else exp(-hazard * distinct_risk)[risk_index])
replaced <- 0L
restore <- function(x) {
...
for(risk in risks)for(hazard in c(0,-0,1,-1,Inf,-Inf,NA_real_,NaN,1e-300,1e300)) {
  ...
  after <- capture(function(){index<-NULL;if(all(is.finite(risk))&&anyDuplicated(risk)){distinct<-unique(risk);index<-match(risk,distinct)};values<-if(is.null(index))exp(-hazard*risk)else exp(-hazard*distinct)[index];mean(values,na.rm=anyNA(values))})
  stopifnot(identical(before,after,num.eq=FALSE));checks<-checks+1L
}
