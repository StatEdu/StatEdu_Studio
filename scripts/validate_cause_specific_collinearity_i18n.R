...
out<-'tmp/cause-specific-collinearity-i18n';dir.create(out,recursive=TRUE,showWarnings=FALSE);entries<-list()
...
codes<-c(moderate='cause_specific_elevated_vif',high='cause_specific_high_vif',nonfinite='cause_specific_nonfinite_vif',condition='cause_specific_high_condition_number')
for(kind in names(cases))for(language in c('en','ko','ja','zh','es','fr','de','vi')){
 r<-result;r$cause_specific<-list(collinearity_table=data.frame(VIF=cases[[kind]][1]),condition_number=cases[[kind]][2])
 ...
 if(nrow(rows)){
  ...
  if(language!='en')stopifnot(!grepl('Maximum cause-specific Cox VIF|Cause-specific Cox design condition number|At least one cause-specific Cox design-column|Review exact collinearity|Review instability in cause-specific|Review cause-specific Cox covariate|Review numerical instability',xml2::xml_text(doc)))
