...
stopifnot(
  grepl("Focal X analyses", rendered, fixed = TRUE),
  grepl("Direct X -&gt; Y paths", rendered, fixed = TRUE),
  grepl("PROCESS-compatible OLS", rendered, fixed = TRUE),
  grepl("Complete cases for each focal-X model", rendered, fixed = TRUE)
)
old_app_language <- getOption("statedu.app_language", NULL)
options(statedu.app_language = "ko")
rendered_ko <- htmltools::renderTags(mediation_moderation_result_ui(full_result, language = "ko"))$html
if (is.null(old_app_language)) options(statedu.app_language = NULL) else options(statedu.app_language = old_app_language)
stopifnot(
  grepl('data-result-table-role="main"', rendered_ko, fixed = TRUE),
  grepl('data-result-table-role="appendix"', rendered_ko, fixed = TRUE),
  grepl('data-result-table-language="en"', rendered_ko, fixed = TRUE),
  grepl('data-result-table-language="ko"', rendered_ko, fixed = TRUE),
  grepl("모형 개요", rendered_ko, fixed = TRUE),
  grepl("부트스트랩 진단", rendered_ko, fixed = TRUE),
  grepl("초점 X 분석", rendered_ko, fixed = TRUE),
  grepl("직접 X → Y 경로", rendered_ko, fixed = TRUE),
  grepl("공변량 통제", rendered_ko, fixed = TRUE),
  grepl("PROCESS 호환 OLS", rendered_ko, fixed = TRUE),
  grepl("각 초점 X 모형별 완전 사례", rendered_ko, fixed = TRUE),
  grepl("유효 표본 수", rendered_ko, fixed = TRUE),
  grepl("간접효과", rendered_ko, fixed = TRUE),
  grepl("총 간접효과", rendered_ko, fixed = TRUE),
  grepl("총효과", rendered_ko, fixed = TRUE),
  grepl("충분", rendered_ko, fixed = TRUE),
  !grepl("Focal X analyses", rendered_ko, fixed = TRUE),
  !grepl("Complete cases for each focal-X model", rendered_ko, fixed = TRUE)
)
table_hits <- gregexpr("<table", rendered_ko, fixed = TRUE)[[1L]]
sheet_hits <- gregexpr('data-result-table-sheet="true"', rendered_ko, fixed = TRUE)[[1L]]
table_count <- if (identical(table_hits[[1L]], -1L)) 0L else length(table_hits)
sheet_count <- if (identical(sheet_hits[[1L]], -1L)) 0L else length(sheet_hits)
stopifnot(table_count == sheet_count)
