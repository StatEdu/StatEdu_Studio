...
if(length(reference_path))sys.source(reference_path[[1]],old) else {
 ...
 old$longitudinal_sensitivity_analysis_results<-function(...,selected_fit=NULL)
  original_sensitivity(...,selected_fit=if(inherits(selected_fit$model,'glmerMod') && identical(stats::family(selected_fit$model)$family,'poisson'))NULL else selected_fit)
}
...
check<-function(d,args,signal='quiet'){
 ...
 eligible<-identical(args$model_type,'glmm')&&identical(args$family,'poisson')&&signal=='quiet'&&a$quiet
 if(signal=='error')stopifnot(a$calls==1L,b$calls==1L)else{
  stopifnot(length(a$snapshot$value)==1L,!is.null(a$snapshot$value[[1]]$model$fixed),b$calls==a$calls-as.integer(eligible))
  ...
  if(eligible){stopifnot(a$calls==3L,b$calls==2L,identical(a$metrics[[1]],a$metrics[[2]],num.eq=FALSE));reused<<-reused+1L;if(isTRUE(args$random_slope))reused_slope<<-reused_slope+1L}else fallback<<-fallback+1L
 }
...
d$y<-rpois(n,exp(eta/3))
for(slope in c(FALSE,TRUE)){
 args<-list(model_type='glmm',family='poisson',random_slope=slope)
 ...
 exposed<-d;exposed$exposure<-rep(c(.8,1,1.2),length.out=n);check(exposed,c(args,list(exposure='exposure')))
}
# Compare the shared LMM path while disabling only Poisson reuse in the reference.
continuous<-d;continuous$y<-eta+rnorm(n);check(continuous,list(model_type='lmm',family='gaussian',random_slope=FALSE))
check(continuous,list(model_type='lmm',family='gaussian',random_slope=TRUE))
stopifnot(reused>0L,reused_slope>0L,fallback>0L)
cat('PASS:',passed,'Poisson GLMM cases;',reused,'reused including',reused_slope,'random slopes;',fallback,'fallback cases\n')
