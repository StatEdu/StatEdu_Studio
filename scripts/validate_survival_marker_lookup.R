...
root <- 'output/survival-marker-lookup-20260913'
old <- new.env(parent=.GlobalEnv);sys.source(file.path(root,'baseline.R'),old)
baseline <- old$survival_cell_note_marker;environment(baseline) <- .GlobalEnv
current <- survival_cell_note_marker
...
for (language in c('ko','en')) {
  ...
  for (variant in c('baseline','current')) {
    survival_cell_note_marker <- if(variant=='baseline')baseline else current
    panel <- htmltools::renderTags(survival_simple_table(table,table_language=language))
...
survival_cell_note_marker <- current
