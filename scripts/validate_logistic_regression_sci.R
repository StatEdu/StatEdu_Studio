...
screen_tables <- result_entry_tables(list(html = saved_logistic_results_html(list(binary_result), variable_table = binary_info)))
stopifnot(length(openxlsx::getSheetNames(xlsx)) == length(screen_tables))
for (i in seq_along(screen_tables)) {
  screen <- screen_tables[[i]]$screen
  exported <- openxlsx::read.xlsx(xlsx, sheet = i, colNames = FALSE, skipEmptyRows = FALSE, skipEmptyCols = FALSE)
  for (cell in screen$cells) {
    expected <- screen$values[cell$row, cell$col]
    if (!is.na(expected) && nzchar(expected)) stopifnot(identical(as.character(exported[cell$row + 2L, cell$col]), expected))
  }
}
