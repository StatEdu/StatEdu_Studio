...
stopifnot(
  ...
  grepl("일반 sampling-design gate는 향후 전용 메뉴와 내부 검증을 위해 미지원·알 수 없는 설계 코드를 계속 차단", decision_rules, fixed = TRUE),
...
invalid_sampling_error <- tryCatch({
  structural_canvas_sampling_design_gate("unexpected_design")
  ""
}, error = function(error) conditionMessage(error))
blank_sampling_error <- tryCatch({
  structural_canvas_sampling_design_gate("")
  ""
}, error = function(error) conditionMessage(error))
stopifnot(
  ...
  identical(invalid_sampling$sampling_design, "independent_cross_sectional"),
  grepl("Sampling-design gate blocked estimation", invalid_sampling_error, fixed = TRUE),
  identical(blank_sampling$sampling_design, "independent_cross_sectional"),
  ...
  identical(pls_defaults$pls_bootstrap, 5000L),
  identical(pls_defaults$estimator_recommendation_confirmed, TRUE)
)
...
multigroup_moderation_fingerprint_functions <- c(
...
multigroup_fingerprint_before <- structural_canvas_analysis_code_fingerprint()
original_group_product_preparation <- structural_canvas_prepare_group_product_indicators
assign(
  "structural_canvas_prepare_group_product_indicators",
  function(data, group, moderation_definitions = list()) list(data = data),
  envir = .GlobalEnv
)
multigroup_fingerprint_after <- structural_canvas_analysis_code_fingerprint()
assign(
  "structural_canvas_prepare_group_product_indicators",
  original_group_product_preparation,
  envir = .GlobalEnv
)
stopifnot(
  all(pls_fingerprint_functions %in% pls_fingerprint_before$included_functions),
  all(multigroup_moderation_fingerprint_functions %in% multigroup_fingerprint_before$included_functions),
  ...
  !identical(multigroup_fingerprint_before$sha256, multigroup_fingerprint_after$sha256)
)
