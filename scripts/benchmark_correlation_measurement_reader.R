...
capture<-function(env,info){
 ds<-list();stdout<-capture.output(value<-withCallingHandlers(env$prepare_correlation_results(data,names(data),variable_info=info,options=list(continuous_method='pearson',normality=TRUE)),
