source('scripts/validate_category_label_batch.R')
run <- as.integer(commandArgs(trailingOnly=TRUE)[1L])
if (is.na(run)) run <- 1L
timings <- list()
for (n in c(500L,2000L)) for (kind in c('partial','full')) {
  info <- make_info(n)
  saved <- data.frame(name=rev(info$name),reference=rep('2',n))
  if (kind=='full') for (column in category_label_edit_columns()) {
  ...
  for (iteration in seq_len(3L)) {
    for (version in if ((run+iteration)%%2L) c('old','new') else c('new','old')) {
      gc()
      start <- Sys.time()
      actual <- capture(get(version),args)
