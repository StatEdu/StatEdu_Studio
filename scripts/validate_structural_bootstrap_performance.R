...
installer_exactness_verified <- function(recorded) {
  ...
  is.list(exactness) && all(vapply(
    exactness[c(
      ...
      "sem_fixed_index_missing_fiml_exact",
      "sem_pooled_rng_restored", "sem_pooled_index_memory_bounded",
...
assert_effect_bootstrap_index_memory_contract <- function() {
  requested <- 50000L
  observations <- 5000L
  chunk_size <- 250L
  bounded <- structural_canvas_effect_bootstrap_index_memory_diagnostics(
    requested, observations, chunk_size, return_draws = FALSE
  )
  retained <- structural_canvas_effect_bootstrap_index_memory_diagnostics(
    requested, observations, chunk_size, return_draws = TRUE
  )
  if (!isTRUE(bounded$full_materialization_avoided) ||
      !identical(bounded$retained_replicates, 0L) ||
      !identical(bounded$peak_chunk_replicates, chunk_size) ||
      !identical(bounded$estimated_full_materialization_bytes, 1e9) ||
      !identical(bounded$estimated_peak_sample_index_bytes, 5e6) ||
      bounded$estimated_peak_sample_index_bytes >= bounded$estimated_full_materialization_bytes ||
      isTRUE(retained$full_materialization_avoided) ||
      !identical(retained$retained_replicates, requested) ||
      !identical(
        retained$estimated_peak_sample_index_bytes,
        retained$estimated_full_materialization_bytes
      )) {
    stop("The 50,000-replicate pooled SEM sample-index memory contract is not bounded by chunk size.", call. = FALSE)
  }
  invisible(TRUE)
}

assert_effect_bootstrap_index_memory_contract()
...
assert_pooled_bootstrap_chunk_rng_regression <- function() {
  set.seed(20260831L)
  rows <- 96L
  predictor <- stats::rnorm(rows)
  mediator <- 0.45 * predictor + stats::rnorm(rows, sd = 0.75)
  outcome <- 0.25 * predictor + 0.55 * mediator + stats::rnorm(rows, sd = 0.70)
  fixture <- data.frame(predictor = predictor, mediator = mediator, outcome = outcome)
  model <- paste(
    "mediator ~ predictor",
  ...
  fit <- suppressWarnings(lavaan::sem(
    model, data = fixture, estimator = "ML", missing = "listwise",
  ...
  if (!isTRUE(lavaan::lavInspect(fit, "converged")) ||
    ...
    stop("The focused pooled-bootstrap regression fixture is not admissible.", call. = FALSE)
  }
  ...
  keys <- paste(raw$lhs, raw$op, raw$rhs, sep = "\r")
  all_keys <- paste(estimates$lhs, estimates$op, estimates$rhs, sep = "\r")
  fit@Options$baseline <- FALSE
  fit@Options$h1 <- FALSE
  fit@Options$loglik <- FALSE
  fit@Options$implied <- FALSE
  fit@Options$test <- "none"
  prepared <- list(
    data = fixture, fit_template = fit,
    model_df = as.numeric(lavaan::fitMeasures(fit, "df")[[1L]]),
    raw_original = raw, raw_keys = keys,
    standardized_original_values = estimates$std.all[match(keys, all_keys)],
    moderated_specs = list(), product_specs = list(), preparation_seconds = 0,
    original_fit_gate = list(eligible = TRUE, state = "eligible", reason = "")
  )
  old_options <- options(
    statedu.internal.disable_sem_bootstrap_two_stage = TRUE,
    statedu.internal.disable_sem_bootstrap_fixed_index = TRUE,
    statedu.isolated_lavaan_bootstrap_worker = FALSE
  )
  ...
  run <- function(workers, chunk_size, return_draws) {
    value <- suppressWarnings(structural_canvas_effect_bootstrap_prepared(
      prepared, reps = 6L, seed = 20260902L, workers = workers,
      chunk_size = chunk_size, return_draws = return_draws
    ))
    if (!identical(RNGkind(), caller_kind) ||
      ...
      stop("The focused pooled SEM bootstrap did not restore caller RNG state.", call. = FALSE)
    }
    value
  }
  serial <- run(1L, 2L, TRUE)
  parallel <- run(2L, 3L, TRUE)
  bounded <- run(1L, 2L, FALSE)
  serial_draws <- attr(serial, "bootstrap_draws")
  parallel_draws <- attr(parallel, "bootstrap_draws")
  if (!identical(serial_draws$sample_indices, parallel_draws$sample_indices) ||
      !identical(serial_draws$valid_mask, parallel_draws$valid_mask) ||
      !isTRUE(all.equal(serial_draws$raw, parallel_draws$raw, tolerance = 0)) ||
      !isTRUE(all.equal(serial_draws$standardized, parallel_draws$standardized, tolerance = 0))) {
    stop("Pooled SEM chunking changed samples or estimates across worker/chunk counts.", call. = FALSE)
  }
  attr(serial, "timings") <- attr(serial, "bootstrap_draws") <- NULL
  attr(parallel, "timings") <- attr(parallel, "bootstrap_draws") <- NULL
  bounded_timings <- attr(bounded, "timings") %||% list()
  attr(bounded, "timings") <- NULL
  if (!isTRUE(all.equal(serial, parallel, tolerance = 0, check.attributes = TRUE)) ||
      !isTRUE(all.equal(serial, bounded, tolerance = 0, check.attributes = TRUE)) ||
      !isTRUE(bounded_timings$sample_index_memory$full_materialization_avoided) ||
      as.integer(bounded_timings$sample_index_memory$retained_replicates %||% NA_integer_) != 0L ||
      !identical(
        as.character(bounded_timings$rng$kind %||% character(0)),
        c("Mersenne-Twister", "Inversion", "Rejection")
      )) {
    stop("The focused pooled SEM bootstrap result/RNG/memory contract failed.", call. = FALSE)
  }
  invisible(TRUE)
}

assert_pooled_bootstrap_chunk_rng_regression()
message("Focused pooled SEM chunk/RNG/formal-name regression passed.")

fixture_rng_kind_before <- RNGkind()
fixture_seed_existed_before <- exists(".Random.seed", envir = .GlobalEnv, inherits = FALSE)
fixture_seed_before <- if (fixture_seed_existed_before) {
  get(".Random.seed", envir = .GlobalEnv, inherits = FALSE)
} else {
  NULL
}
source(file.path("scripts", "structural_bootstrap_dmc_fixture.R"), encoding = "UTF-8")
sem_fixture <- structural_bootstrap_dmc_fixture()
fixture_seed_exists_after <- exists(".Random.seed", envir = .GlobalEnv, inherits = FALSE)
fixture_seed_after <- if (fixture_seed_exists_after) {
  get(".Random.seed", envir = .GlobalEnv, inherits = FALSE)
} else {
  NULL
}
if (!identical(RNGkind(), fixture_rng_kind_before) ||
    !identical(fixture_seed_exists_after, fixture_seed_existed_before) ||
    !identical(fixture_seed_after, fixture_seed_before)) {
  stop("The deterministic structural bootstrap fixture changed the caller's RNG state.", call. = FALSE)
}
if (!identical(sem_fixture$id, "statedu_dmc_75_v1") ||
    !identical(sem_fixture$R_version, "4.5.3") ||
    !identical(
      sem_fixture$RNGkind,
      c("Mersenne-Twister", "Inversion", "Rejection")
    ) ||
    !identical(sem_fixture$seed, 8L) ||
    !identical(sem_fixture$generator_version, 1L) ||
    !identical(
      sem_fixture$sha256,
      "8a0788d0a806be4bfb5c556f326d9d1e61a71cc16305f55a454f16602417f10f"
    )) {
  stop("The deterministic structural bootstrap fixture contract changed.", call. = FALSE)
}
sem_data <- sem_fixture$data
...
if (!inherits(sem_original$fit, "lavaan") || !isTRUE(sem_original$converged) ||
    !isTRUE(sem_original$admissible) ||
...
fast_fit_list <- suppressWarnings(lavaan::lavaanList(
  ...
  dataList = list(prepared_sample), cmd = "sem", store.slots = character(0),
  FUN = function(fit) {
    estimates <- lavaan::parameterEstimates(fit, standardized = TRUE, ci = FALSE)
    list(
...
cfa_validated_df <- as.numeric(lavaan::fitMeasures(cfa_fit, "df")[[1L]])
cfa_admissibility_with_validated_df <- structural_canvas_fit_admissibility(
  cfa_fit, validated_model_df = cfa_validated_df
)
if (!identical(
    cfa_admissibility_with_validated_df,
    cfa_admissibility
)) {
  stop("The optional prevalidated model-df path changed public CFA admissibility output.", call. = FALSE)
}
cfa_invalid_df <- structural_canvas_fit_admissibility(
  cfa_fit, validated_model_df = NA_real_
)
if (isTRUE(cfa_invalid_df$admissible) ||
    !"invalid degrees of freedom" %in% cfa_invalid_df$reasons) {
  stop("A malformed prevalidated model df did not fail closed.", call. = FALSE)
}
...
assert_strict_mask_equivalence <- function(
  ...
  compared <- suppressWarnings(lavaan::lavaanList(
    model = fit_template, dataList = data_list, cmd = "sem",
    ...
    FUN = function(fit) {
      ...
      list(
        ...
        reference = isTRUE(structural_canvas_fit_admissibility(
          fit, validated_model_df = model_df
        )$admissible),
...
dmc_strict_mask <- assert_strict_mask_equivalence(
  sem_prepared$fit_template, dmc_strict_data_list, sem_prepared$raw_keys,
  sem_prepared$model_df, "DMC SEM (24 draws)",
  require_valid = TRUE, require_invalid = TRUE
)
expected_dmc_strict_mask <- strsplit(
  "011000101110100011101111", "", fixed = TRUE
)[[1L]] == "1"
if (!identical(dmc_strict_mask, expected_dmc_strict_mask)) {
  stop("The deterministic DMC 24-draw strict mask changed.", call. = FALSE)
}
...
run_sem_fixed_index_exactness <- function(seed = 20260824L, repetitions = 8L) {
  ...
  fixed_fit <- suppressWarnings(lavaan::cfa(
    ...
    missing = "fiml", meanstructure = TRUE, se = "standard",
  ...
  if (!identical(fixed_likelihood, "normal")) {
    stop(sprintf(
      "The complete-data FIML/meanstructure fixture resolved to likelihood '%s', not normal.",
      fixed_likelihood
    ), call. = FALSE)
  }
  ...
  run_path <- function(
    workers, disable_fixed = FALSE, failure_mode = "",
    prepared = fixed_prepared, retain_draws = FALSE
  ) {
    old_options <- options(
    ...
    started <- Sys.time()
    value <- suppressWarnings(structural_canvas_effect_bootstrap_prepared(
      prepared, reps = repetitions, seed = seed,
      workers = workers, chunk_size = repetitions,
      return_draws = isTRUE(retain_draws)
    ))
    list(
      value = value,
  ...
  validation_rng_kind <- RNGkind()
  validation_seed_exists <- exists(".Random.seed", envir = .GlobalEnv, inherits = FALSE)
  if (validation_seed_exists) {
    validation_seed <- get(".Random.seed", envir = .GlobalEnv, inherits = FALSE)
  }
  on.exit({
    do.call(RNGkind, as.list(validation_rng_kind))
    if (validation_seed_exists) {
      assign(".Random.seed", validation_seed, envir = .GlobalEnv)
    } else if (exists(".Random.seed", envir = .GlobalEnv, inherits = FALSE)) {
      rm(".Random.seed", envir = .GlobalEnv)
    }
  }, add = TRUE)
  ...
  expected_rng_seed <- get(".Random.seed", envir = .GlobalEnv, inherits = FALSE)
  assert_caller_rng_restored <- function(label) {
    if (!identical(RNGkind(), expected_rng_kind) ||
        !identical(
          get(".Random.seed", envir = .GlobalEnv, inherits = FALSE),
          expected_rng_seed
        )) {
      stop(sprintf("%s did not restore the caller RNG kind and seed.", label), call. = FALSE)
    }
  }

  legacy <- run_path(1L)
  assert_caller_rng_restored("Serial pooled SEM bootstrap")
  fast_two <- run_path(2L)
  assert_caller_rng_restored("Two-worker pooled SEM bootstrap")
  fast_four <- run_path(4L)
  assert_caller_rng_restored("Four-worker pooled SEM bootstrap")
  item_failure <- run_path(2L, failure_mode = "item")
  assert_caller_rng_restored("Fail-open pooled SEM bootstrap")

  missing_data <- fixed_data
  missing_count <- max(1L, floor(nrow(missing_data) * .1))
  for (variable_index in seq_along(missing_data)) {
    missing_rows <- ((seq_len(missing_count) +
      (variable_index - 1L) * missing_count - 1L) %% nrow(missing_data)) + 1L
    missing_data[missing_rows, variable_index] <- NA_real_
  }
  missing_fit <- suppressWarnings(lavaan::cfa(
    hs_strict_model, data = missing_data, estimator = "ML",
    missing = "fiml", meanstructure = TRUE, se = "standard",
    test = "standard", baseline = FALSE, h1 = FALSE
  ))
  if (!isTRUE(lavaan::lavInspect(missing_fit, "converged"))) {
    stop("The actual-missing FIML fixed-index fixture did not converge.", call. = FALSE)
  }
  missing_estimates <- lavaan::parameterEstimates(
    missing_fit, standardized = TRUE, ci = FALSE
  )
  missing_fit@Options$baseline <- FALSE
  missing_fit@Options$h1 <- FALSE
  missing_fit@Options$loglik <- FALSE
  missing_fit@Options$implied <- FALSE
  missing_fit@Options$test <- "none"
  missing_prepared <- list(
    data = missing_data,
    fit_template = missing_fit,
    model_df = as.numeric(lavaan::fitMeasures(missing_fit, "df")[[1L]]),
    raw_original = missing_estimates[, c("lhs", "op", "rhs", "est"), drop = FALSE],
    raw_keys = paste(
      missing_estimates$lhs, missing_estimates$op, missing_estimates$rhs, sep = "\r"
    ),
    standardized_original_values = missing_estimates$std.all,
    moderated_specs = list(), product_specs = list(), preparation_seconds = 0
  )
  missing_legacy <- run_path(
    2L, disable_fixed = TRUE, prepared = missing_prepared, retain_draws = TRUE
  )
  assert_caller_rng_restored("Actual-missing authoritative FIML SEM bootstrap")
  missing_fast <- run_path(
    2L, prepared = missing_prepared, retain_draws = TRUE
  )
  assert_caller_rng_restored("Actual-missing fixed-index FIML SEM bootstrap")
  ...
  assert_reproducible(
    strip_timing_attributes(missing_legacy$value),
    strip_timing_attributes(missing_fast$value),
    "Actual-missing FIML SEM fixed-index versus authoritative lavaanList bootstrap",
    tolerance = 0
  )
  missing_fast_state <- missing_fast$timings$fixed_index %||% list()
  missing_draws <- attr(missing_fast$value, "bootstrap_draws")
  if (!isTRUE(missing_fast_state$supported) || !isTRUE(missing_fast_state$active) ||
      isTRUE(missing_fast_state$product_aware) ||
      as.integer(missing_fast_state$fallbacks %||% 0L) != 0L ||
      !identical(
        as.character((missing_fast_state$information %||% list())$mode %||% ""),
        "expected"
      ) ||
      !isTRUE((missing_fast_state$information %||% list())$expected_active) ||
      !is.list(missing_draws) ||
      !identical(names(missing_draws), c(
        "sample_indices", "valid_mask", "raw", "standardized"
      ))) {
    stop("Actual-missing FIML SEM did not execute the exact non-product fixed-index path.", call. = FALSE)
  }
  ...
  for (candidate in list(legacy, fast_two, fast_four, item_failure)) {
    memory <- candidate$timings$sample_index_memory %||% list()
    rng <- candidate$timings$rng %||% list()
    if (!isTRUE(memory$full_materialization_avoided) ||
        as.integer(memory$retained_replicates %||% NA_integer_) != 0L ||
        !identical(
          as.character(rng$kind %||% character(0)),
          c("Mersenne-Twister", "Inversion", "Rejection")
        )) {
      stop("Pooled SEM bootstrap omitted bounded index-memory or fixed-RNG diagnostics.", call. = FALSE)
    }
  }
  ...
  list(
    ...
    actual_missing_fiml_exact = TRUE,
    fail_open = TRUE,
    rng_restored = TRUE,
    sample_index_memory_bounded = TRUE,
    ...
    seconds = list(
      ...
      actual_missing_legacy = missing_legacy$seconds,
      actual_missing_fast = missing_fast$seconds
    )
  )
}
...
run_sem_product_index_exactness <- function(seed = 20260826L, repetitions = 5L) {
  ...
  # the bounded 75-row release fixture pay for repeated full fits in the core
  # release gate.
  set.seed(1031L)
  rows <- 240L
...
run_structural_exactness_gate <- function() {
  ...
  sem_first_draws <- attr(sem_first$value, "bootstrap_draws")
  if (!is.list(sem_first_draws) ||
      !identical(sem_first_draws$valid_mask, dmc_strict_mask)) {
    stop("The optimized production SEM valid mask differs from the pinned public strict mask.", call. = FALSE)
  }
  ...
  if (!isTRUE(sem_two_stage$supported) || !isTRUE(sem_two_stage$used) ||
    ...
    stop(sprintf(
      "SEM exactness gate did not exercise the guarded two-stage screening path: %s",
      jsonlite::toJSON(sem_two_stage, auto_unbox = TRUE, null = "null")
    ), call. = FALSE)
  }
  ...
  evidence <- list(
    ...
    sem_production_strict_mask = TRUE,
    ...
    sem_fixed_index_missing_fiml_exact = isTRUE(sem_fixed_index$actual_missing_fiml_exact),
    sem_pooled_rng_restored = isTRUE(sem_fixed_index$rng_restored),
    sem_pooled_index_memory_bounded = isTRUE(sem_fixed_index$sample_index_memory_bounded),
...
metrics <- list(
  fixture = list(
    id = sem_fixture$id,
    R_version = sem_fixture$R_version,
    RNGkind = as.list(sem_fixture$RNGkind),
    seed = sem_fixture$seed,
    generator_version = sem_fixture$generator_version,
    sha256 = sem_fixture$sha256,
    provenance = sem_fixture$provenance,
