...
fixture <- structural_bootstrap_dmc_fixture()
fixture_data <- fixture$data
if (!identical(dim(fixture_data), c(75L, 11L)) || anyNA(fixture_data)) {
  stop("The deterministic DMC release fixture must be complete and 75-by-11.", call. = FALSE)
}
...
if (!inherits(original$fit, "lavaan") || !isTRUE(original$converged)) {
  stop("The deterministic latent DMC release fixture did not converge.", call. = FALSE)
}
...
report <- list(
  ...
  fixture = list(
    ...
    RNGkind = as.list(fixture$RNGkind),
    seed = fixture$seed,
