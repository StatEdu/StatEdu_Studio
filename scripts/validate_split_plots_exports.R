...
for (mode in c("current","accumulated")) {
 ...
 for (ext in c("docx","xlsx","hwpx")) {
  ...
  images <- members[grepl("(word/media/|xl/media/|BinData/).*[.](png|jpg|jpeg)$",members,ignore.case=TRUE)]
  stopifnot(length(images)>=4L)
 }
 doc <- xml2::read_html(paste0(stem,".html"))
 stopifnot(length(xml2::xml_find_all(doc,"//img[starts-with(@src,'data:image')]"))>=4L)
}
cat("PASS: all four group diagnostic images preserved in HTML/Word/Excel/HWPX.\n")
