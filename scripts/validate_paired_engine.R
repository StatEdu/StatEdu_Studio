...
for(e in list(old,new))sys.source('R/analysis_paired.R',e)
replacements<-0L
...
f<-old$paired_analyze_pair;body(f)<-restore(body(f));old$paired_analyze_pair<-f
...
count<-0L;methods<-character()
for(kind in c('continuous','ordered'))for(scenario in c('ordinary','normal','missing','zero','infinite','constantdiff','factor'))for(median_iqr in c(FALSE,TRUE))for(unified in c(FALSE,TRUE))for(assumption in c(FALSE,TRUE)) {
 set.seed(939);d<-data.frame(pre=rlnorm(80),post=rlnorm(80))
 if(scenario=='normal')d<-data.frame(pre=rnorm(80),post=rnorm(80))
 ...
 invoke<-function(e)capture(function() {
  options<-list(median_iqr=median_iqr,add_mean_sd=TRUE,effect_size=TRUE,assumption_check=assumption)
  if(unified)e$prepare_paired_unified_results(d,list(c('pre','post')),variable_info=info,options=options)
  else e$prepare_paired_results(d,'pre','post',variable_info=info,options=options)
 })
 a<-invoke(old);b<-invoke(new);stopifnot(identical(a,b,num.eq=FALSE))
 if(scenario %in% c('ordinary','normal','missing'))stopifnot(is.null(a$value$error))
 if(!unified)methods<-c(methods,a$value$table$Method)
 count<-count+1L
}
stopifnot(all(c('Paired t-test','Wilcoxon signed-rank test') %in% methods))
cat('PASS complete and unified results/conditions/stdout/RNG:',count,'; t-test and Wilcoxon paths covered\n')
candidate<-paired_rm_wilcox_engine();count<-0L
for(kind in c('continuous','ordered'))for(missing_rate in c(0,.1)) {
 set.seed(939);d<-as.data.frame(matrix(if(kind=='continuous')rlnorm(20000*16)else sample(1:5,20000*16,TRUE),20000,16))
