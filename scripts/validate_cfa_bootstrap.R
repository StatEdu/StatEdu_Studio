...
# Keep the production lavaanList contract explicit.  lavaan's public API uses
# snake_case formal names in the bundled 0.7-2 build. Compatibility aliases
# are accepted through `...`, but the production path must call the installed
# API directly so it does not pay adaptation overhead in every chunk.
reliability_bootstrap_body <- paste(
  deparse(body(structural_canvas_reliability_bootstrap), width.cutoff = 500L),
  collapse = "\n"
)
lavaan_list_formals <- names(formals(lavaan::lavaanList))
stopifnot(
  all(c("data_list", "store_slots", "fun") %in% lavaan_list_formals),
  grepl("data_list = frames", reliability_bootstrap_body, fixed = TRUE),
  grepl("store_slots = character(0)", reliability_bootstrap_body, fixed = TRUE),
  grepl("fun = fast_callback", reliability_bootstrap_body, fixed = TRUE),
  !grepl("dataList = frames", reliability_bootstrap_body, fixed = TRUE),
  !grepl("store.slots = character(0)", reliability_bootstrap_body, fixed = TRUE),
  !grepl("FUN = fast_callback", reliability_bootstrap_body, fixed = TRUE)
)
...
missing_continuous <- continuous
missing_continuous$x2[seq.int(5L, nrow(missing_continuous), by = 10L)] <- NA_real_
missing_fit <- lavaan::cfa(
  "eta1 =~ x1 + x2 + x3", data = missing_continuous,
  estimator = "ML", missing = "fiml", auto.cov.lv.x = FALSE
)
old_disable_cfa_fixed_index <- getOption("statedu.internal.disable_cfa_reliability_fixed_index")
options(statedu.internal.disable_cfa_reliability_fixed_index = FALSE)
missing_fixed_draws <- structural_canvas_reliability_bootstrap(
  "eta1 =~ x1 + x2 + x3", missing_continuous, reps = 6L, seed = 20260819L,
  estimator = "ML", missing = "fiml", original_fit = missing_fit,
  workers = 2L, chunk_size = 4L, return_draws = TRUE
)
options(statedu.internal.disable_cfa_reliability_fixed_index = TRUE)
missing_lavaan_list_draws <- structural_canvas_reliability_bootstrap(
  "eta1 =~ x1 + x2 + x3", missing_continuous, reps = 6L, seed = 20260819L,
  estimator = "ML", missing = "fiml", original_fit = missing_fit,
  workers = 2L, chunk_size = 4L, return_draws = TRUE
)
options(statedu.internal.disable_cfa_reliability_fixed_index = old_disable_cfa_fixed_index)
...
stopifnot(
  ...
  isTRUE(fast_parallel_draws$timings$fixed_index$active),
  identical(fast_parallel_draws$timings$fixed_index$fallbacks, 0L),
  identical(missing_fixed_draws$sample_indices, missing_lavaan_list_draws$sample_indices),
  identical(reliability_draw_mask(missing_fixed_draws), reliability_draw_mask(missing_lavaan_list_draws)),
  reliability_draws_equal(missing_fixed_draws, missing_lavaan_list_draws),
  isTRUE(missing_fixed_draws$timings$fixed_index$active),
  !isTRUE(missing_lavaan_list_draws$timings$fixed_index$active),
