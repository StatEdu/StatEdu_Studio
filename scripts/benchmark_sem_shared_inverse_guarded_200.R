...
root<-paste0('output/sem-shared-inverse-guarded-20260915/guarded-200/',mode,'-',id)
...
stopifnot(identical(baseline,profiled,num.eq=FALSE),length(draws$sample_indices)==200L,
 length(draws$valid_mask)==200L,nrow(draws$raw)==200L,nrow(draws$standardized)==200L,sum(draws$valid_mask)>0L)
