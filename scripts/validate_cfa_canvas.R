...
stopifnot(
  ...
  grepl("function measurementErrorEndpoints", edge_source, fixed = TRUE),
  grepl("from: fromBoundary", edge_source, fixed = TRUE),
  grepl("to: rectangularNodeAnchor(instance, toNode, fromNode)", edge_source, fixed = TRUE),
  ...
  grepl('from.role === "error" && to.role === "indicator"', edge_source, fixed = TRUE),
  grepl("return measurementErrorEndpoints(instance, from, to);", edge_source, fixed = TRUE),
  ...
  grepl('nodeRoles[edge.from] === "error" && nodeRoles[edge.to] === "indicator"', state_source, fixed = TRUE),
  grepl('edge.directAnchors = false;\n        edge.fixedCenter = true;', state_source, fixed = TRUE),
