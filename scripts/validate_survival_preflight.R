...
cox_guide <- survival_interpretation_guide(cox_fit, "en")
cox_stability <- survival_stability_review(delayed_cox, "en")
stopifnot(
  identical(cox_guide$Topic, c("HR", "Proportional hazards", "Causal interpretation")),
  any(grepl("not automatic pass/fail rules", cox_guide$Guidance, fixed = TRUE)),
  any(grepl("not automatically a causal treatment effect", cox_guide$Guidance, fixed = TRUE)),
  "low_events_per_parameter" %in% cox_stability$Code
)
stopifnot(grepl("Analysis overview", cox_html, fixed = TRUE))
stopifnot(grepl("3. Supplementary statistics and diagnostics", cox_html, fixed = TRUE))
stopifnot(grepl("proportional-hazards diagnostics", cox_html, fixed = TRUE))
...
stopifnot(grepl("automatic pass/fail rule", cox_html, fixed = TRUE))
