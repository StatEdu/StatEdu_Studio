...
if(length(reference_path))sys.source(reference_path[[1]],old) else {
 ...
 old$longitudinal_sensitivity_analysis_results<-function(...,selected_fit=NULL)
  original_sensitivity(...,selected_fit=if(inherits(selected_fit$model,'glmerMod') && identical(stats::family(selected_fit$model)$family,'Gamma'))NULL else selected_fit)
}
...
check<-function(d,args,signal='quiet'){
 ...
 eligible<-identical(args$model_type,'glmm')&&identical(args$family,'gamma')&&identical(args$random_slope,FALSE)&&signal=='quiet'&&a$quiet
...
d$y<-rgamma(n,shape=3,rate=3/exp(eta/3))
for(slope in c(FALSE,TRUE)){
 args<-list(model_type='glmm',family='gamma',random_slope=slope)
...
# Compare the shared LMM path while disabling only Gamma reuse in the reference.
...
cat('PASS:',passed,'Gamma GLMM cases;',reused,'reused including',reused_slope,'random slopes;',fallback,'fallback cases\n')
