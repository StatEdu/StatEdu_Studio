...
assert_contains(app_server, '"analysis_complex_custom_model"\n  ), function() {', "complex-sample handlers are deferred until a complex-sample menu is opened")
assert_contains(app_server, '"analysis_structural_automation"\n  ), function() {', "analysis handlers are deferred until an analysis menu is opened")
assert_contains(app_bootstrap, "installed_packages <- rownames(utils::installed.packages())", "startup dependency check uses the installed-package index")
