...
set.seed(923);n<-60;rate<-exp(rnorm(n,2,.8))
d<-data.frame(group=rep(c('Review','Normality'),each=n/2),pre=rgamma(n,shape=4,scale=rate/4),post=rgamma(n,shape=4,scale=rate*1.3/4),last=rgamma(n,shape=4,scale=rate*1.6/4))
...
phrases<-c('Use the fitted Gamma GLMM as the ITT mixed-model result.','Use the fitted Gamma GLMM as the ITT result.')
...
if(length(failures))stop(paste('Gamma ITT failures:',paste(failures,collapse=', ')))
out<-'tmp/mixed-rm-gamma-itt-i18n';dir.create(out,recursive=TRUE,showWarnings=FALSE)
jsonlite::write_json(captured,file.path(out,'captured.json'),auto_unbox=TRUE)
cat('PASS actual Gamma GLMM guidance with/without complete cases across eight languages; main/source/user labels preserved\n')
