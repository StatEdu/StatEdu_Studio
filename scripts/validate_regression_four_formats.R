...
out<-'outputs/spss_phase31_20260906';dir.create(out,showWarnings=FALSE)
...
make<-function(data=d,dep='y',pred=c('x1','x2'),diag=FALSE)prepare_regression_analysis_results(data,dep,pred,variable_info=info,residual_diagnostics=diag,auto_method=FALSE)$results
m<-d;m$y[1:7]<-NA;m$x1[8:11]<-NA
cases<-list(simple=make(pred='x1'),multiple=make(),categorical=make(pred=c('x1','group')),diagnostics=make(diag=TRUE),multi_outcome=make(dep=c('y','y2')),wide=make(),missing=make(m),long_coefficients=make(pred=paste0('x',1:25)))
for(name in names(cases)) {
 ...
 do.call(write_analysis_results_html,c(args,list(file=file.path(folder,'result.html'))))
    html<-paste(readLines(file.path(folder,'result.html'),encoding='UTF-8'),collapse='\n');b<-xml2::read_html(html)
 a<-xml2::read_html(as.character(htmltools::renderTags(do.call(regression_results_panel,args))$html))
 ...
 do.call(write_analysis_results_pdf,c(args,list(file=file.path(folder,'result.pdf'))));do.call(save_analysis_excel_file,c(args,list(file=file.path(folder,'result.xlsx'))));write_result_collection_docx(list(e),file.path(folder,'result.docx'))
