(function() {
  ...
  function isPlsCanvas(instance) {
    return !!(instance && String(instance.analysisType || "").toLowerCase() === "plssem");
  }

  function stripResidualNodes(snapshot) {
    if (!snapshot || !Array.isArray(snapshot.nodes) || !Array.isArray(snapshot.edges)) return false;
    var residualIds = snapshot.nodes.filter(function(node) {
      return node && (node.role === "error" || node.role === "disturbance");
    }).map(function(node) { return node.id; });
    if (!residualIds.length) return false;
    var removedEdgeIds = snapshot.edges.filter(function(edge) {
      return residualIds.indexOf(edge.from) >= 0 || residualIds.indexOf(edge.to) >= 0;
    }).map(function(edge) { return edge.id; });
    snapshot.nodes = snapshot.nodes.filter(function(node) { return residualIds.indexOf(node.id) < 0; });
    snapshot.edges = snapshot.edges.filter(function(edge) {
      return residualIds.indexOf(edge.from) < 0 && residualIds.indexOf(edge.to) < 0;
    });
    if (Array.isArray(snapshot.moderations)) {
      snapshot.moderations = snapshot.moderations.filter(function(moderation) {
        return residualIds.indexOf(moderation.from) < 0 && removedEdgeIds.indexOf(moderation.toEdge) < 0;
      });
    }
    if (Array.isArray(snapshot.selectedNodeIds)) {
      snapshot.selectedNodeIds = snapshot.selectedNodeIds.filter(function(id) { return residualIds.indexOf(id) < 0; });
    }
    if (residualIds.indexOf(snapshot.selectedNodeId) >= 0) snapshot.selectedNodeId = null;
    if (removedEdgeIds.indexOf(snapshot.selectedEdgeId) >= 0) snapshot.selectedEdgeId = null;
    return true;
  }

  function sanitizePlsSnapshot(instance, snapshot) {
    if (isPlsCanvas(instance)) stripResidualNodes(snapshot);
    return snapshot;
  }

  function stripPlsResidualNodes(instance) {
    return isPlsCanvas(instance) && instance.state ? stripResidualNodes(instance.state) : false;
  }
  ...
  function paperPoint(instance, event) {
    var rect = instance.paper.getBoundingClientRect();
    var viewZoom = paperViewZoom(instance);
    return {
      x: (event.clientX - rect.left) / viewZoom,
      y: (event.clientY - rect.top) / viewZoom
    };
  }

  function canvasPoint(instance, event) {
    var local = paperPoint(instance, event);
    var structural = isStructuralCanvas(instance);
    if (structural) {
      var scale = Number(instance.state.canvas.modelZoom || 1);
      var centerX = Number(instance.state.canvas.widthPx || instance.paper.offsetWidth || 0) / 2;
      var centerY = Number(instance.state.canvas.heightPx || instance.paper.offsetHeight || 0) / 2;
      return {
        x: (local.x - centerX) / scale + centerX,
        y: (local.y - centerY) / scale + centerY
      };
    }
    return local;
  }
  ...
  function render(instance) {
    if (!isStructuralCanvas(instance) && !window.StatEduModelCanvas.nodes.isViewingResult(instance)) window.StatEduModelCanvas.edges.inferObservedRoles(instance);
    stripPlsResidualNodes(instance);
    ensurePaperFrame(instance);
    ensureMeasurementErrorGap(instance);
    ...
    if (isStructuralCanvas(instance)) {
      ...
      // Keep the SVG viewport fixed to the full paper. The edge renderer
      // scales an inner group so paths and parameter labels remain visible
      // everywhere on the paper when modelZoom is below 1.
      instance.edgeLayer.style.transform = "none";
      instance.edgeLayer.style.transformOrigin = "0 0";
    } else {
  ...
  function updateSelectionSettings(instance) {
    ...
    container.innerHTML = "";
    if (!nodes.length) {
      container.textContent = window.StatEduModelCanvas.state.label(instance, "select_canvas_variable", ko ? "캔버스의 변수를 선택하세요." : "Select a variable on the canvas.");
      return;
    }
    ...
    if (single) {
      ...
      if (!structural) {
        ...
        field(single.autoRole ? (ko ? "역할 (자동)" : "Role (automatic)") : (ko ? "역할" : "Role"), roleInput);
      ...
      if (single.role === "latent" && !window.StatEduModelCanvas.nodes.isViewingResult(instance)) {
        var measurementDistanceSelect = document.createElement("select");
        measurementDistanceSelect.className = "form-control input-sm structural-measurement-distance-select";
        [[1, "1.0×"], [1.5, "1.5×"], [1.7, "1.7×"], [2, "2.0×"]].forEach(function(item) {
          var distanceOption = document.createElement("option");
          distanceOption.value = String(item[0]);
          distanceOption.textContent = item[1];
          measurementDistanceSelect.appendChild(distanceOption);
        });
        measurementDistanceSelect.value = String(normalizeMeasurementDistanceScale(single.measurementDistanceScale));
        measurementDistanceSelect.addEventListener("change", function() {
          setMeasurementDistanceScale(instance, measurementDistanceSelect.value);
        });
        field(ko ? "측정변수 거리" : "Indicator distance", measurementDistanceSelect);
  ...
  function reflowMeasurementModel(instance) {
    ...
    instance.state.nodes.filter(function(node) { return node.role === "latent"; }).forEach(function(latent) {
      ...
      var measurementMode = latent.measurementMode || "reflective";
      var measurementDistance = 50 * normalizeMeasurementDistanceScale(latent.measurementDistanceScale);
      var disturbanceEdge = instance.state.edges.find(function(edge) {
        var source = window.StatEduModelCanvas.nodes.nodeById(instance, edge.from);
        return edge.to === latent.id && source && source.role === "disturbance";
      });
      ...
      indicators.forEach(function(indicator, index) {
        ...
        if (placement === "left") {
          indicator.x = Number(latent.x || 0) - iw - measurementDistance;
          indicator.y = Number(latent.y || 0) + lh / 2 - ih / 2 + (index - (count - 1) / 2) * 48;
          if (errorNode) {
            var leftErrorOffset = Number(errorNode.manualOffset || 0);
            errorNode.x = indicator.x - Number(errorNode.width || 26) - MEASUREMENT_ERROR_GAP - leftErrorOffset;
          ...
          if (errorEdge) Object.assign(errorEdge, {fromSide: "right", toSide: "left", fixedCenter: true, directAnchors: false});
        } else if (placement === "right") {
          indicator.x = Number(latent.x || 0) + lw + measurementDistance;
          indicator.y = Number(latent.y || 0) + lh / 2 - ih / 2 + (index - (count - 1) / 2) * 48;
          if (errorNode) {
            var rightErrorOffset = Number(errorNode.manualOffset || 0);
            errorNode.x = indicator.x + iw + MEASUREMENT_ERROR_GAP + rightErrorOffset;
            errorNode.y = indicator.y + ih / 2 - Number(errorNode.height || 26) / 2;
          }
          ...
          if (errorEdge) Object.assign(errorEdge, {fromSide: "left", toSide: "right", fixedCenter: true, directAnchors: false});
        } else if (placement === "top") {
          var gap = iw + 24;
          indicator.x = Number(latent.x || 0) + lw / 2 - iw / 2 + (index - (count - 1) / 2) * gap;
          indicator.y = Number(latent.y || 0) - ih - measurementDistance;
          if (errorNode) {
            var topErrorOffset = Number(errorNode.manualOffset || 0);
            errorNode.x = indicator.x + iw / 2 - Number(errorNode.width || 26) / 2;
            errorNode.y = indicator.y - Number(errorNode.height || 26) - MEASUREMENT_ERROR_GAP - topErrorOffset;
          }
          ...
          if (errorEdge) Object.assign(errorEdge, {fromSide: "bottom", toSide: "top", fixedCenter: true, directAnchors: false});
        } else {
          var bottomGap = iw + 24;
          indicator.x = Number(latent.x || 0) + lw / 2 - iw / 2 + (index - (count - 1) / 2) * bottomGap;
          indicator.y = Number(latent.y || 0) + lh + measurementDistance;
          if (errorNode) {
            var bottomErrorOffset = Number(errorNode.manualOffset || 0);
            errorNode.x = indicator.x + iw / 2 - Number(errorNode.width || 26) / 2;
            errorNode.y = indicator.y + ih + MEASUREMENT_ERROR_GAP + bottomErrorOffset;
          }
          ...
          if (errorEdge) Object.assign(errorEdge, {fromSide: "top", toSide: "bottom", fixedCenter: true, directAnchors: false});
        }
      });
    });
  }
  ...
  function placeErrorNearIndicator(instance, errorNode, indicator, placement) {
    ...
    if (placement === "left") {
      errorNode.x = Number(indicator.x || 0) - ew - MEASUREMENT_ERROR_GAP;
      errorNode.y = Number(indicator.y || 0) + ih / 2 - eh / 2;
    } else if (placement === "right") {
      errorNode.x = Number(indicator.x || 0) + iw + MEASUREMENT_ERROR_GAP;
      errorNode.y = Number(indicator.y || 0) + ih / 2 - eh / 2;
    } else if (placement === "top") {
      errorNode.x = Number(indicator.x || 0) + iw / 2 - ew / 2;
      errorNode.y = Number(indicator.y || 0) - eh - MEASUREMENT_ERROR_GAP;
    } else {
      errorNode.x = Number(indicator.x || 0) + iw / 2 - ew / 2;
      errorNode.y = Number(indicator.y || 0) + ih + MEASUREMENT_ERROR_GAP;
    }
  }

  function normalizeMeasurementDistanceScale(value) {
    var scale = Number(value || 1);
    return [1, 1.5, 1.7, 2].indexOf(scale) >= 0 ? scale : 1;
  }

  function setMeasurementDistanceScale(instance, scale) {
    if (window.StatEduModelCanvas.nodes.isViewingResult(instance)) return false;
    var latents = selectedLatentNodes(instance);
    if (!latents.length) return false;
    var normalized = normalizeMeasurementDistanceScale(scale);
    var changed = latents.some(function(latent) {
      return normalizeMeasurementDistanceScale(latent.measurementDistanceScale) !== normalized;
    });
    if (!changed) return false;
    window.StatEduModelCanvas.state.pushHistory(instance);
    latents.forEach(function(latent) { latent.measurementDistanceScale = normalized; });
    reflowMeasurementModel(instance);
    render(instance);
    window.StatEduModelCanvas.bridge.sendState(instance);
    return true;
  ...
  function ensureMeasurementErrorGap(instance) {
    if (!isStructuralCanvas(instance) || !instance.state) return;
    instance.state.nodes.filter(function(node) {
      return node.role === "error";
    }).forEach(function(errorNode) {
      var errorEdge = instance.state.edges.find(function(edge) {
        return edge.kind !== "covariance" && edge.from === errorNode.id;
      });
      var indicator = errorEdge ? window.StatEduModelCanvas.nodes.nodeById(instance, errorEdge.to) : null;
      if (!indicator || indicator.role !== "indicator") return;
      ...
      var measurementFrom = window.StatEduModelCanvas.nodes.nodeById(instance, measurementEdge.from);
      var latent = measurementFrom && measurementFrom.role === "latent" ?
        measurementFrom : window.StatEduModelCanvas.nodes.nodeById(instance, measurementEdge.to);
      if (!latent || latent.role !== "latent") return;
      var placement = latent.effectiveMeasurementPlacement || latent.measurementPlacement || "right";
      var iw = Number(indicator.width || instance.state.style.boxWidth || 110);
      var ih = Number(indicator.height || instance.state.style.boxHeight || 38);
      var ew = Number(errorNode.width || 26);
      var eh = Number(errorNode.height || 26);
      var gap = placement === "left" ? Number(indicator.x || 0) - (Number(errorNode.x || 0) + ew) :
        placement === "right" ? Number(errorNode.x || 0) - (Number(indicator.x || 0) + iw) :
        placement === "top" ? Number(indicator.y || 0) - (Number(errorNode.y || 0) + eh) :
        Number(errorNode.y || 0) - (Number(indicator.y || 0) + ih);
      if (Number.isFinite(gap) && gap >= MEASUREMENT_ERROR_GAP - 0.5) return;
      placeErrorNearIndicator(instance, errorNode, indicator, placement);
      errorNode.manualOffset = 0;
  ...
  function bindVariableDrag(instance) {
    ...
    instance.paper.addEventListener("drop", function(event) {
      ...
      var measuredTarget = !isStructural && event.target.closest ? event.target.closest(".custom-model-node") : null;
      if (measuredTarget) {
        var measuredNode = window.StatEduModelCanvas.nodes.nodeById(instance, measuredTarget.getAttribute("data-node-id"));
        var measuredVariable = variableByName(instance, name);
        if (!measuredNode || !measuredVariable || window.StatEduModelCanvas.nodes.variableUsed(instance, name) || instance.state.covariates.indexOf(name) >= 0) return;
        window.StatEduModelCanvas.state.pushHistory(instance);
        measuredNode.variableId = measuredVariable.name;
        measuredNode.name = measuredVariable.name;
        measuredNode.dataLabel = measuredVariable.dataLabel || measuredVariable.name;
        measuredNode.canvasLabel = "";
        measuredNode.unassigned = false;
        measuredNode.autoRole = true;
        render(instance);
        window.StatEduModelCanvas.bridge.sendState(instance);
        return;
      }
      if (isStructural && targetElement) {
        ...
        names.forEach(function(itemName, index) {
          ...
          var errorNode = isPlsCanvas(instance) ? null : window.StatEduModelCanvas.nodes.createErrorNode(instance, indicator, Number(latent.x || 0), Number(latent.y || 0), errorBase + index + 1);
          instance.state.nodes.push(indicator);
          if (errorNode) instance.state.nodes.push(errorNode);
          window.StatEduModelCanvas.edges.createEdge(instance, latent.id, indicator.id);
          if (errorNode) window.StatEduModelCanvas.edges.createEdge(instance, errorNode.id, indicator.id);
        });
        reflowMeasurementModel(instance);
        instance.state.selectedVariables = names;
        instance.state.selectedVariable = names[names.length - 1] || null;
      ...
      var role = isStructural ? window.StatEduModelCanvas.dialogs.chooseRole("independent") : "observed";
      if (!role || instance.state.covariates.indexOf(name) >= 0) return;
      if (isStructural && !canAssignRole(instance, role, name)) return;
      var point = canvasPoint(instance, event);
      window.StatEduModelCanvas.state.pushHistory(instance);
      var placedNode = window.StatEduModelCanvas.nodes.createNodeFromVariable(instance, variable, point.x, point.y, role);
      if (!isStructural) placedNode.autoRole = true;
      instance.state.nodes.push(placedNode);
    if (isStructural && role !== "moderator") {
      window.StatEduModelCanvas.layout.reflowRoleLayout(instance.state.nodes, instance.state.style);
    }
  ...
  function bindCanvasPointer(instance) {
    instance.paper.addEventListener("pointerdown", function(event) {
      ...
      if (instance.state.mode === "addObserved" && !isStructuralCanvas(instance)) {
        event.preventDefault();
        if (!viewingResult && event.button === 0 && !nodeElement && !labelElement && !edgeId && !propertyPanel) {
          var observedPoint = canvasPoint(instance, event);
          window.StatEduModelCanvas.state.pushHistory(instance);
          var sequence = instance.state.nodes.length + 1;
          var placeholder = {name: "", dataLabel: (instance.language === "ko" ? "측정변수 " : "Measured variable ") + sequence};
          var node = window.StatEduModelCanvas.nodes.createNodeFromVariable(instance, placeholder, observedPoint.x, observedPoint.y, "observed");
          node.id = "observed_" + Date.now() + "_" + sequence;
          node.autoRole = true;
          node.unassigned = true;
          instance.state.nodes.push(node);
          instance.state.selectedNodeId = node.id;
          instance.state.selectedNodeIds = [node.id];
          render(instance);
          window.StatEduModelCanvas.bridge.sendState(instance);
        }
        return;
      }
      var latentPlacementMode = instance.state.mode === "addLatent" || instance.state.mode === "addHigherOrderLatent";
      if (latentPlacementMode) {
        event.preventDefault();
        if (!viewingResult && event.button === 0 && !nodeElement && !labelElement && !edgeControlElement && !propertyPanel) {
          var latentPoint = canvasPoint(instance, event);
          window.StatEduModelCanvas.toolbar.placeLatentAt(
            instance,
            latentPoint.x,
            latentPoint.y,
            instance.state.mode === "addHigherOrderLatent"
          );
        }
        return;
      }
      ...
      if (marqueeSelectionMode && !labelElement && !nodeElement && !edgeId && !moderationElement && !propertyPanel) {
        ...
        var marqueeStart = paperPoint(instance, event);
        ...
        function moveMarquee(moveEvent) {
          var point = paperPoint(instance, moveEvent);
          var left = Math.min(marqueeStart.x, point.x);
          var top = Math.min(marqueeStart.y, point.y);
          marquee.style.left = left + "px";
          marquee.style.top = top + "px";
          marquee.style.width = Math.abs(point.x - marqueeStart.x) + "px";
          marquee.style.height = Math.abs(point.y - marqueeStart.y) + "px";
        }
  ...
  function indicatorAlignmentGroups(instance) {
    var selected = selectedNodes(instance);
    var alignAll = !selected.length && !instance.state.selectedEdgeId && !instance.state.selectedModerationId;
    var lookup = function(id) { return window.StatEduModelCanvas.nodes.nodeById(instance, id); };
    return instance.state.nodes.filter(function(node) { return node.role === "latent"; }).map(function(latent) {
      var edges = instance.state.edges.filter(function(edge) {
        var other = lookup(edge.from === latent.id ? edge.to : edge.to === latent.id ? edge.from : null);
        return other && other.role === "indicator";
      });
      var indicators = edges.map(function(edge) { return lookup(edge.from === latent.id ? edge.to : edge.from); })
        .filter(function(node, index, all) {
          return all.indexOf(node) === index;
        });
      var selectedGroup = alignAll || selected.indexOf(latent) >= 0 || indicators.some(function(indicator) {
        return selected.indexOf(indicator) >= 0 || instance.state.edges.some(function(edge) {
          var error = lookup(edge.from);
          return edge.to === indicator.id && error && error.role === "error" && selected.indexOf(error) >= 0;
        });
      });
      if (!selectedGroup) return null;
      return {latent: latent, edges: edges, indicators: indicators};
    }).filter(function(group) { return group && group.indicators.length >= 2; });
  }

  function alignIndicators(instance) {
    var groups = indicatorAlignmentGroups(instance);
    if (!groups.length) return false;
    window.StatEduModelCanvas.state.pushHistory(instance);
    groups.forEach(function(group) {
      var placement = measurementPlacementForLatent(instance, group.latent, group.edges);
      var axis = placement === "top" || placement === "bottom" ? "y" : "x";
      var center = function(node) { return window.StatEduModelCanvas.layout.nodeCenter(node, instance.state.style)[axis]; };
      // The median preserves an existing aligned majority and resists one outlier.
      var positions = group.indicators.map(center).sort(function(a, b) { return a - b; });
      var mid = Math.floor(positions.length / 2);
      var target = positions.length % 2 ? positions[mid] : (positions[mid - 1] + positions[mid]) / 2;
      group.indicators.forEach(function(indicator) {
        var delta = target - center(indicator);
        indicator[axis] = Number(indicator[axis] || 0) + delta;
        instance.state.edges.forEach(function(edge) {
          if (edge.to !== indicator.id) return;
          var error = window.StatEduModelCanvas.nodes.nodeById(instance, edge.from);
          if (error && error.role === "error") error[axis] = Number(error[axis] || 0) + delta;
        });
      });
    });
    render(instance);
  ...
  function addContinuousModeratorBlock(instance) {
    ...
    names.forEach(function(name, index) {
      ...
      var errorNode = isPlsCanvas(instance) ? null : window.StatEduModelCanvas.nodes.createErrorNode(instance, indicator, indicator.x, indicator.y - 46, errorBase + index + 1);
      instance.state.nodes.push(latent, indicator);
      if (errorNode) instance.state.nodes.push(errorNode);
      window.StatEduModelCanvas.edges.createEdge(instance, latent.id, indicator.id);
      if (errorNode) window.StatEduModelCanvas.edges.createEdge(instance, errorNode.id, indicator.id);
  ...
  function centerModelOnPaper(instance) {
    ...
    var bounds = instance.state.nodes.reduce(function(result, node) {
      ...
      var height = Math.max(0, Number(node.height || 0));
      result.left = Math.min(result.left, x);
      result.top = Math.min(result.top, y);
      result.right = Math.max(result.right, x + width);
      result.bottom = Math.max(result.bottom, y + height);
      return result;
    }, {left: Infinity, top: Infinity, right: -Infinity, bottom: -Infinity});
    if (![bounds.left, bounds.top, bounds.right, bounds.bottom].every(Number.isFinite)) return false;

    var paperWidth = Number(instance.state.canvas.widthPx || (instance.paper && instance.paper.offsetWidth) || 0);
    var paperHeight = Number(instance.state.canvas.heightPx || (instance.paper && instance.paper.offsetHeight) || 0);
    if (!(paperWidth > 0) || !(paperHeight > 0)) return false;
    var offsetX = paperWidth / 2 - (bounds.left + bounds.right) / 2;
    var offsetY = paperHeight / 2 - (bounds.top + bounds.bottom) / 2;
    if (Math.abs(offsetX) < 0.01 && Math.abs(offsetY) < 0.01) return false;
    ...
    (instance.state.edges || []).forEach(function(edge) {
      if (!edge.controlPoint || !Number.isFinite(Number(edge.controlPoint.x)) || !Number.isFinite(Number(edge.controlPoint.y))) return;
      edge.controlPoint.x = Number(edge.controlPoint.x) + offsetX;
      edge.controlPoint.y = Number(edge.controlPoint.y) + offsetY;
    });
    return true;
  }
  ...
  function showResult(instance) {
    ...
    if (window.StatEduModelCanvas.bridge && window.StatEduModelCanvas.bridge.updateResultGroupControl) {
      window.StatEduModelCanvas.bridge.updateResultGroupControl(instance);
    }
    render(instance);
  ...
  function init(root) {
    ...
    sanitizePlsSnapshot(instance, initialSnapshot);
    sanitizePlsSnapshot(instance, initialResultSnapshot);
    instance.sourceSnapshot = initialSnapshot || null;
    instance.resultSnapshots = cachedCanvas && Array.isArray(cachedCanvas.results) ? cachedCanvas.results : null;
    instance.activeResultGroupKey = cachedCanvas && cachedCanvas.activeResultGroupKey ? cachedCanvas.activeResultGroupKey : "overall";
    instance.resultSnapshot = initialResultSnapshot || null;
    if (Array.isArray(instance.resultSnapshots) && instance.resultSnapshots.length) {
      var cachedResultEntry = instance.resultSnapshots.find(function(entry) {
        return String(entry.key || "") === String(instance.activeResultGroupKey || "overall");
      }) || instance.resultSnapshots[0];
      if (cachedResultEntry && cachedResultEntry.result) instance.resultSnapshot = cachedResultEntry.result;
    }
  ...
  window.StatEduModelCanvas.canvas = {
    ...
    zoom: zoom,
    fit: fit,
    centerModelOnPaper: centerModelOnPaper,
    ...
    stripPlsResidualNodes: stripPlsResidualNodes,
    sanitizePlsSnapshot: sanitizePlsSnapshot,
    setMeasurementPlacement: setMeasurementPlacement,
    setMeasurementDistanceScale: setMeasurementDistanceScale,
    ...
    indicatorAlignmentGroups: indicatorAlignmentGroups,
    alignIndicators: alignIndicators,
    alignSelected: alignSelected,
    autoLayout: autoLayoutModel,
    covariateTypeForVariable: covariateTypeForVariable,
    addSelectedVariableAsRole: addSelectedVariableAsRole
  };
