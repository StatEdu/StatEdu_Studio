(function() {
  ...
  function validDirectedEdge(fromNode, toNode) {
    if (fromNode && toNode && fromNode.autoRole && toNode.autoRole) return true;
  ...
  function validModerationSource(node) {
    return node && (node.autoRole || node.role === "moderator" || node.role === "latent");
  }

  function inferObservedRoles(instance) {
    if (!instance.state.nodes.some(function(node) { return node.autoRole; })) return;
    var paths = instance.state.edges.filter(function(edge) { return edge.kind !== "covariance"; });
    instance.state.nodes.forEach(function(node) {
      node.autoRole = true;
      var incoming = paths.some(function(edge) { return edge.to === node.id; });
      var outgoing = paths.some(function(edge) { return edge.from === node.id; });
      var moderator = instance.state.moderations.some(function(link) { return link.from === node.id && edgeById(instance, link.toEdge); });
      node.role = moderator ? "moderator" : incoming ? (outgoing ? "mediator" : "dependent") : outgoing ? "independent" : "observed";
    });
  }
  ...
  function nodeAnchor(instance, node, side, slot, gap) {
    ...
    var ellipse = ["latent", "error", "disturbance"].indexOf(node.role) >= 0;
    var centerX = Number(node.x || 0) + width / 2;
    var centerY = Number(node.y || 0) + height / 2;
    var x = 0;
    var y = 0;
    if (side === "top" || side === "bottom") {
      x = Number(node.x || 0) + anchorOffset(width, slot.count, slot.index, gap);
      if (ellipse) {
        var normalizedX = width > 0 ? Math.max(-1, Math.min(1, (x - centerX) / (width / 2))) : 0;
        var ellipseY = (height / 2) * Math.sqrt(Math.max(0, 1 - normalizedX * normalizedX));
        y = centerY + (side === "bottom" ? ellipseY : -ellipseY);
      } else {
    ...
    if (ellipse) {
      var normalizedY = height > 0 ? Math.max(-1, Math.min(1, (y - centerY) / (height / 2))) : 0;
      var ellipseX = (width / 2) * Math.sqrt(Math.max(0, 1 - normalizedY * normalizedY));
      x = centerX + (side === "right" ? ellipseX : -ellipseX);
    } else {
  ...
  function rectangularNodeAnchor(instance, node, targetNode) {
    var center = window.StatEduModelCanvas.layout.nodeCenter(node, instance.state.style);
    var target = window.StatEduModelCanvas.layout.nodeCenter(targetNode, instance.state.style);
    var halfWidth = Number(node.width || instance.state.style.boxWidth) / 2;
    var halfHeight = Number(node.height || instance.state.style.boxHeight) / 2;
    var dx = target.x - center.x;
    var dy = target.y - center.y;
    if (!dx && !dy) return center;
    var scale = 1 / Math.max(Math.abs(dx) / halfWidth, Math.abs(dy) / halfHeight);
    return {x: center.x + dx * scale, y: center.y + dy * scale};
  }

  function measurementErrorEndpoints(instance, fromNode, toNode) {
    var fromBoundary = radialNodeAnchor(instance, fromNode, toNode);
    return {
      // Start exactly at the error-node ellipse boundary. Extending the shaft
      // toward the center remains visible through the node's transparent fill.
      from: fromBoundary,
      // Recompute the indicator rectangle intersection from current node
      // centers on every render; do not reuse saved side overrides.
      to: rectangularNodeAnchor(instance, toNode, fromNode)
    };
  }
  ...
  function edgeEndpoints(instance, edge) {
    ...
    if (edge.kind !== "covariance" && from.role === "error" && to.role === "indicator") {
      return measurementErrorEndpoints(instance, from, to);
    }
  ...
  function applyModelZoomToEdgeContent(instance, svg) {
    if (!instance || !svg || ["cfa", "cbsem", "sem", "plssem"].indexOf(instance.analysisType) < 0) return;
    var group = document.createElementNS(SVG_NS, "g");
    group.setAttribute("class", "custom-model-edge-content");
    Array.prototype.slice.call(svg.children).forEach(function(child) {
      if (String(child.tagName || "").toLowerCase() !== "defs") group.appendChild(child);
    });
    var zoom = Number(instance.state.canvas.modelZoom || 1);
    var centerX = Number(instance.nodeLayer && instance.nodeLayer.offsetWidth || instance.state.canvas.widthPx || 0) / 2;
    var centerY = Number(instance.nodeLayer && instance.nodeLayer.offsetHeight || instance.state.canvas.heightPx || 0) / 2;
    if (zoom !== 1) {
      group.setAttribute("transform", "translate(" + centerX + " " + centerY + ") scale(" + zoom + ") translate(" + (-centerX) + " " + (-centerY) + ")");
    }
    svg.appendChild(group);
  }
  ...
  function renderEdges(instance) {
    ...
    function nonsignificantResult(item) {
      // Older result files can have p/significant without dashEligible.
      // Explicitly suppressed inference must never be treated as non-significant.
      return instance.state.dashNonsignificant !== false && item &&
        item.dashEligible !== false && item.resultMatched !== false &&
        typeof item.p === "number" && Number.isFinite(item.p) &&
        item.p >= 0.05 && item.p <= 1;
    }
    ...
    function applyEdgeStyle(element, selected, widthOffset, item) {
      ...
      if (nonsignificantResult(item) && !edgeTouchesErrorNode(item)) {
    ...
    instance.state.moderations.forEach(function(moderation) {
      ...
      if (Object.prototype.hasOwnProperty.call(moderation, "significant")) {
        if (nonsignificantResult(moderation)) {
    ...
    applyModelZoomToEdgeContent(instance, svg);
  }
  ...
  function createEdge(instance, fromId, toId) {
    ...
    if (fromNode.autoRole) {
      var visited = {};
      function reaches(id) {
        if (id === fromId) return true;
        if (visited[id]) return false;
        visited[id] = true;
        return instance.state.edges.some(function(edge) { return edge.kind !== "covariance" && edge.from === id && reaches(edge.to); });
      }
      if (reaches(toId)) return false;
    }
  ...
  function createModeration(instance, fromId, edgeId, edgePosition) {
    ...
    var targetPath = edgeById(instance, edgeId);
    if (fromNode.autoRole && (targetPath.kind === "covariance" || targetPath.from === fromId || targetPath.to === fromId)) return false;
    if (fromNode.autoRole) {
      var otherModerators = Array.from(new Set(instance.state.moderations.map(function(link) { return link.from; }).filter(function(id) { return id !== fromId; })));
      if (otherModerators.length >= 2) return false;
    }
  ...
  function deleteEdge(instance, edgeId) {
    var changed = instance.state.edges.some(function(edge) { return edge.id === edgeId; });
    instance.state.edges = instance.state.edges.filter(function(edge) {
    ...
    if (changed) window.StatEduModelCanvas.bridge.invalidateResultAfterModelEdit(instance);
  ...
  function deleteModeration(instance, moderationId) {
    var changed = instance.state.moderations.some(function(item) { return item.id === moderationId; });
    instance.state.moderations = instance.state.moderations.filter(function(moderation) {
  ...
  window.StatEduModelCanvas.edges = {
    inferObservedRoles: inferObservedRoles,
    render: renderEdges,
