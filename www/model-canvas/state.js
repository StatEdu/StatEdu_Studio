(function() {
  ...
  function normalizeNode(node) {
    ...
    var measurementDistanceScale = Number(node.measurementDistanceScale || 1);
    node.measurementDistanceScale = [1, 1.5, 1.7, 2].indexOf(measurementDistanceScale) >= 0 ? measurementDistanceScale : 1;
  ...
  function restore(state, snap) {
    ...
    state.edges.forEach(function(edge) {
      ...
      if (edge.kind !== "covariance" && nodeRoles[edge.from] === "error" && nodeRoles[edge.to] === "indicator") {
  ...
  function roleLabel(instance, role) {
    if (role === "observed") return instance.language === "ko" ? "측정변수 (연결 대기)" : "Measured variable (unconnected)";
