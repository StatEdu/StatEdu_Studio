(function() {
  ...
  var STRUCTURAL_ALIGN_GUIDE_THRESHOLD = 10;
  ...
  function captureErrorOffset(instance, errorNode) {
    ...
    var defaultPosition;
    if (placement === "left") {
      defaultPosition = Number(indicator.x || 0) - ew - MEASUREMENT_ERROR_GAP;
      errorNode.manualOffset = defaultPosition - Number(errorNode.x || 0);
    } else if (placement === "right") {
      defaultPosition = Number(indicator.x || 0) + iw + MEASUREMENT_ERROR_GAP;
      errorNode.manualOffset = Number(errorNode.x || 0) - defaultPosition;
    } else if (placement === "top") {
      defaultPosition = Number(indicator.y || 0) - eh - MEASUREMENT_ERROR_GAP;
      errorNode.manualOffset = defaultPosition - Number(errorNode.y || 0);
    } else {
      defaultPosition = Number(indicator.y || 0) + ih + MEASUREMENT_ERROR_GAP;
  ...
  function usesObservedAlignmentGuides(instance) {
    return !!(instance && instance.root && instance.root.classList.contains("mediation-moderation-canvas-root"));
  }

  function alignmentNode(instance, node) {
    if (usesObservedAlignmentGuides(instance)) {
      return ["observed", "independent", "mediator", "dependent", "moderator", "covariate"].indexOf(node.role) >= 0;
    }
    return node.role === "latent";
  }

  function structuralAlignmentReference(instance, draggedNodes) {
    var latents = (draggedNodes || []).filter(function(node) { return alignmentNode(instance, node); });
    return latents.length === 1 ? latents[0] : null;
  }

  function structuralAlignmentCandidate(instance, reference, selectedIds, threshold) {
    if ((!usesStructuralMovePolicy(instance) && !usesObservedAlignmentGuides(instance)) || !reference) return null;
    var selected = selectedIds || [];
    var referenceCenterX = Number(reference.x || 0) + Number(reference.width || 90) / 2;
    var referenceCenterY = Number(reference.y || 0) + Number(reference.height || 44) / 2;
    var candidate = {x: null, y: null};
    var targets = instance.state.nodes.filter(function(target) {
      return alignmentNode(instance, target) && selected.indexOf(target.id) < 0;
    });
    targets.forEach(function(target) {
      var targetCenterX = Number(target.x || 0) + Number(target.width || 90) / 2;
      var targetCenterY = Number(target.y || 0) + Number(target.height || 44) / 2;
      var distanceX = Math.abs(targetCenterX - referenceCenterX);
      var distanceY = Math.abs(targetCenterY - referenceCenterY);
      if (distanceX <= threshold && (!candidate.x || distanceX < candidate.x.distance)) {
        candidate.x = {target: target, center: targetCenterX, distance: distanceX};
      }
      if (distanceY <= threshold && (!candidate.y || distanceY < candidate.y.distance)) {
        candidate.y = {target: target, center: targetCenterY, distance: distanceY};
      }
    });
    // Compare peer centers, not the bounds of a selected measurement bundle.
    // Only peers on the same row/column define a midpoint guide.
    targets.forEach(function(first, index) {
      targets.slice(index + 1).forEach(function(second) {
        var ax = Number(first.x || 0) + Number(first.width || 90) / 2;
        var ay = Number(first.y || 0) + Number(first.height || 44) / 2;
        var bx = Number(second.x || 0) + Number(second.width || 90) / 2;
        var by = Number(second.y || 0) + Number(second.height || 44) / 2;
        function consider(axis, center, referenceCenter) {
          var distance = Math.abs(center - referenceCenter);
          if (distance <= threshold && (!candidate[axis] || distance < candidate[axis].distance)) {
            candidate[axis] = {midpoint: true, center: center, distance: distance};
          }
        }
        if (Math.abs(ay - by) <= threshold && Math.abs(ax - bx) > threshold * 2) {
          consider("x", (ax + bx) / 2, referenceCenterX);
        }
        if (Math.abs(ax - bx) <= threshold && Math.abs(ay - by) > threshold * 2) {
          consider("y", (ay + by) / 2, referenceCenterY);
        }
      });
    });
    return candidate.x || candidate.y ? candidate : null;
  }

  function applyStructuralAlignmentSnap(alignment, reference, draggedNodes, structuralAxis) {
    if (!alignment || !reference) return {x: 0, y: 0};
    var canAlignX = structuralAxis !== "y" && structuralAxis !== "none";
    var canAlignY = structuralAxis !== "x" && structuralAxis !== "none";
    var referenceCenterX = Number(reference.x || 0) + Number(reference.width || 90) / 2;
    var referenceCenterY = Number(reference.y || 0) + Number(reference.height || 44) / 2;
    var snapX = alignment.x && canAlignX ? alignment.x.center - referenceCenterX : 0;
    var snapY = alignment.y && canAlignY ? alignment.y.center - referenceCenterY : 0;
    if (snapX || snapY) {
      (draggedNodes || []).forEach(function(node) {
        node.x = Number(node.x || 0) + snapX;
        node.y = Number(node.y || 0) + snapY;
      });
    }
    return {x: snapX, y: snapY};
  }

  function ensureStructuralAlignmentGuideLayer(instance) {
    var layer = instance.paper.querySelector(".structural-alignment-guide-layer");
    if (layer) return layer;
    layer = document.createElement("div");
    layer.className = "structural-alignment-guide-layer";
    instance.paper.appendChild(layer);
    return layer;
  }

  function clearStructuralAlignmentGuides(instance) {
  ...
  function modelCoordinateOnPaper(instance, value, axis) {
    if (usesObservedAlignmentGuides(instance)) return Number(value || 0);
    var size = axis === "x" ? Number(instance.state.canvas.widthPx || 0) : Number(instance.state.canvas.heightPx || 0);
    var zoom = Number(instance.state.canvas.modelZoom || 1);
    return size / 2 + (Number(value || 0) - size / 2) * zoom;
  }

  function showStructuralAlignmentGuides(instance, candidate) {
    clearStructuralAlignmentGuides(instance);
    if (!candidate) return;
    var layer = ensureStructuralAlignmentGuideLayer(instance);
    function appendLine(className, property, value) {
      var line = document.createElement("div");
      line.className = "structural-alignment-guide " + className;
      line.style[property] = value + "px";
      layer.appendChild(line);
    }
    if (candidate.x && candidate.x.midpoint) {
      appendLine("is-vertical is-midpoint", "left", modelCoordinateOnPaper(instance, candidate.x.center, "x"));
    } else if (candidate.x) {
      var xTarget = candidate.x.target;
      appendLine("is-vertical", "left", modelCoordinateOnPaper(instance, Number(xTarget.x || 0), "x"));
      appendLine("is-vertical", "left", modelCoordinateOnPaper(instance, Number(xTarget.x || 0) + Number(xTarget.width || 90), "x"));
    }
    if (candidate.y && candidate.y.midpoint) {
      appendLine("is-horizontal is-midpoint", "top", modelCoordinateOnPaper(instance, candidate.y.center, "y"));
    } else if (candidate.y) {
      var yTarget = candidate.y.target;
      appendLine("is-horizontal", "top", modelCoordinateOnPaper(instance, Number(yTarget.y || 0), "y"));
      appendLine("is-horizontal", "top", modelCoordinateOnPaper(instance, Number(yTarget.y || 0) + Number(yTarget.height || 44), "y"));
    }
  ...
  function startNodeDrag(instance, event, element) {
    ...
    if (!node) return;
    event.preventDefault();
    var historyBeforeDrag = instance.state.history.slice();
    var redoBeforeDrag = instance.state.redoStack.slice();
    window.StatEduModelCanvas.state.pushHistory(instance);
    ...
    var alignmentReference = structuralAlignmentReference(instance, draggedNodes);

    function move(moveEvent) {
      // Structural canvases scale the paper and its model independently.
      var dragScale = instance.analysisType ?
        Number(instance.state.canvas.viewZoom || 1) * Number(instance.state.canvas.modelZoom || 1) :
        Number(instance.state.canvas.zoom || 1);
      ...
      var alignment = null;
      if (alignmentReference && (usesStructuralMovePolicy(instance) || usesObservedAlignmentGuides(instance))) {
        var guideThreshold = STRUCTURAL_ALIGN_GUIDE_THRESHOLD / Math.max(0.1, dragScale);
        alignment = structuralAlignmentCandidate(instance, alignmentReference, selectedIds, guideThreshold);
        var canAlignX = structuralAxis !== "y" && structuralAxis !== "none";
        var canAlignY = structuralAxis !== "x" && structuralAxis !== "none";
        if (alignment && instance.state.autoAlign !== false) {
          applyStructuralAlignmentSnap(alignment, alignmentReference, draggedNodes, structuralAxis);
        }
        if (alignment) {
          if (!canAlignX) alignment.x = null;
          if (!canAlignY) alignment.y = null;
        }
      }
      showStructuralAlignmentGuides(instance, alignment);
      renderNodes(instance);
    ...
    function up(upEvent) {
      ...
      clearStructuralAlignmentGuides(instance);
      if (!moved) {
        instance.state.history = historyBeforeDrag;
        instance.state.redoStack = redoBeforeDrag;
      ...
      if (!structuralMovePolicy && !usesObservedAlignmentGuides(instance) && instance.state.autoAlign !== false && initialPositions.length === 1 && node.role !== "error" && node.role !== "disturbance") {
    ...
    function cancelOnEscape(keyEvent) {
  ...
  function positionPropertyPanel(instance, panel, anchorX, anchorY, anchorLeftX) {
    if (!instance || !instance.paper || !panel) return;
    var margin = 8;
    var canvasWidth = Number(instance.state.canvas.widthPx || instance.paper.offsetWidth || 0);
    var canvasHeight = Number(instance.state.canvas.heightPx || instance.paper.offsetHeight || 0);
    var maxHeight = Math.max(120, canvasHeight - margin * 2);
    panel.style.maxHeight = maxHeight + "px";

    var panelWidth = Number(panel.offsetWidth || 230);
    var panelHeight = Math.min(Number(panel.offsetHeight || panel.scrollHeight || 230), maxHeight);
    var nextX = Number(anchorX || 0);
    if (nextX + panelWidth > canvasWidth - margin) {
      nextX = Number(anchorLeftX === undefined ? anchorX : anchorLeftX) - panelWidth - 10;
    }
  ...
  function showNodeProperties(instance, nodeId) {
    ...
    panel.className = "custom-model-property-popover custom-model-node-property-popover";
    ...
    var propertyFields = [
      '<div class="custom-model-property-scroll">',
    ...
    if (!isStructuralModel) {
      roleSelect.appendChild(option("observed", window.StatEduModelCanvas.state.roleLabel(instance, "observed")));
      roleSelect.disabled = true;
    }
    ...
    panel.querySelector(".custom-model-property-apply").addEventListener("click", function() {
      ...
      fixedRole = fixedRole || node.role === "disturbance" || !isStructuralModel;
      ...
      node.role = nextRole;
      if (!node.autoRole && nextRole === "dependent") {
      ...
      if (!node.autoRole && nextRole !== "moderator") {
  ...
  function showEdgeProperties(instance, edgeId) {
    ...
    var propertyFields = [
      '<div class="custom-model-property-title">' + (ko && edge.kind === "covariance" ? "공분산 속성" : window.StatEduModelCanvas.state.label(instance, "path_properties", "Path properties")) + '</div>',
      '<label class="custom-model-property-label">' + window.StatEduModelCanvas.state.label(instance, "display_label", "Display label") + '</label>',
    ...
    propertyFields.push(
      '<div class="custom-model-property-actions">',
      '<button type="button" class="btn btn-primary btn-sm edge-property-apply">' + window.StatEduModelCanvas.state.label(instance, "apply", "Apply") + '</button>',
      '<button type="button" class="btn btn-default btn-sm edge-property-close">' + window.StatEduModelCanvas.state.label(instance, "close", "Close") + '</button>',
      '</div>'
    );
    panel.innerHTML = propertyFields.join("");
  ...
  window.StatEduModelCanvas.nodes = {
    ...
    structuralAlignmentCandidate: structuralAlignmentCandidate,
    applyStructuralAlignmentSnap: applyStructuralAlignmentSnap,
    clearStructuralAlignmentGuides: clearStructuralAlignmentGuides,
    render: renderNodes,
    nodeById: nodeById,
